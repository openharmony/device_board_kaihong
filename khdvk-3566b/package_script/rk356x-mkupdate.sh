...
ROOTFS_LAST=$(grep "system:grow" Image/parameter.txt)
if [ -z "${ROOTFS_LAST}" ]
then
echo "Resize system partition size"
FILE_P=$(readlink -f Image/system.img)
...
ORIGIN=$(grep -Eo "0x[0-9a-fA-F]*@0x[0-9a-fA-F]*\(system" $PARA_FILE)
