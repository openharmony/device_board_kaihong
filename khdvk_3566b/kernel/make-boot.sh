...
source ../khdvk_3566b/device/rockchip/.BoardConfig.mk
if [ -f "../khdvk_3566b/device/rockchip/$RK_TARGET_PRODUCT/$RK_KERNEL_FIT_ITS" ]; then
	../khdvk_3566b/device/rockchip/common/mk-fitimage.sh ./kernel/$RK_BOOT_IMG \
		../khdvk_3566b/device/rockchip/$RK_TARGET_PRODUCT/$RK_KERNEL_FIT_ITS \
		${BOOT_LINUX}/kernel/ramdisk.img
fi
#bash ./build.sh kernel
cp -arfpL ${BOOT_LINUX}/../khdvk_3566b/kernel/boot.img ${IMAGES_OUT_PATH}
