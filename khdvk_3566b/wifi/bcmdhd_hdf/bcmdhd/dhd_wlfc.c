                               ...
                               uint8 hcnt, uint8 prec, void **pktout)
{
    ...
    if (hslot < WLFC_MAC_DESC_TABLE_SIZE) {
        ...
        entry = &ctx->destination_entries
                     .interfaces[hslot - WLFC_MAC_DESC_TABLE_SIZE];
    } else {
        entry = &ctx->destination_entries.other;
    }
    ...
    if (!b) {
        ...
        DHD_INFO(
            ("%s: out of order, seq(%d), head_seq(%d)\n", __FUNCTION__, hcnt,
                                ...
                                uint16 htodseq, bool skip_wlfc_hdr)
{
    ...
    fillers = ROUNDUP(dataOffset, 0x4) - dataOffset;
    dataOffset += fillers;
    ...
    if (tim_signal_len) {
        ...
        wlh[dataOffset - fillers - tim_signal_len + 0x2] = mac_handle;
        wlh[dataOffset - fillers - tim_signal_len + 0x3] = tim_bmp;
    }
    if (fillers) {
...
static int _dhd_wlfc_pullheader(athost_wl_status_info_t *ctx, void *pktbuf)
{
    ...
    if (PKTLEN(ctx->osh, pktbuf) < (uint)(h->dataOffset << 0x2)) {
        DHD_ERROR(("%s: rx data too short (%d < %d)\n", __FUNCTION__,
                   PKTLEN(ctx->osh, pktbuf), (h->dataOffset << 0x2)));
        return BCME_ERROR;
    }

    /* pull wl-header */
    PKTPULL(ctx->osh, pktbuf, (h->dataOffset << 0x2));
    return BCME_OK;
}
...
static int _dhd_wlfc_prec_drop(dhd_pub_t *dhdp, int prec, void *p, bool bPktInQ)
{
    ...
    if (!WLFC_GET_AFQ(dhdp->wlfc_mode) && (prec & 1)) {
        ...
        _dhd_wlfc_hanger_poppkt(
            ctx->hanger, WL_TXSTATUS_GET_HSLOT(DHD_PKTTAG_H2DTAG(PKTTAG(p))),
            &pout, TRUE);
        ASSERT(p == pout);
    }
                                         ...
                                         uint32 hslot)
{
    ...
    if (entry != NULL) {
        ...
        */
        ...
        if (_dhd_wlfc_prec_enq_with_drop(
                ctx->dhdp, &entry->psq, p, fifo_id, TRUE,
                WLFC_SEQCOUNT(entry, fifo_id >> 1)) == FALSE) {
            /* enque failed */
                                         ...
                                         struct pktq *pq, uint8 if_id)
{
    ...
    if ((ctx->hostif_flow_state[if_id] == OFF) &&
        !_dhd_wlfc_allow_fc(ctx, if_id)) {
        return;
    }
                                            ...
                                            uint8 ta_bmp)
{
    ...
    if (p) {
        ...
        _dhd_wlfc_pushheader(ctx, &p, TRUE, ta_bmp, entry->mac_handle, 0, 0,
                             FALSE);
                                      ...
                                      void *p)
{
    ...
    order of delivery.
    */
    if (_dhd_wlfc_prec_enq_with_drop(ctx->dhdp, &entry->psq, p,
                                     ((prec << 1) + 1), FALSE,
                                      ...
                                      uint32 *slot)
{
    ...
    *slot = hslot;
    ...
    rc = _dhd_wlfc_pushheader(ctx, &p, send_tim_update,
                              ...
                              entry->mac_handle, htod, htodseq, FALSE);
    if (rc == BCME_OK) {
                                      ...
                                      bool only_no_credit)
{
    ...
    *ac_credit_spent = credit_spent;
    ...
    for (i = 0; i < total_entries; i++) {
        ...
        if (entry->occupied &&
            _dhd_wlfc_is_destination_open(ctx, entry, prec) &&
            ...
            *needs_hdr = 0;
            ...
            if (p != NULL) {
                ...
                *entry_out = entry;
                ...
                /*
                 * A packet has been picked up, update traffic availability
                 * bitmap, if applicable.
                 */
                _dhd_wlfc_traffic_pending_check(ctx, entry, prec);
                return p;
            }
    ...
    if (item->pkt) {
        ...
        if (item->pkt_state == WLFC_HANGER_PKT_STATE_COMPLETE) {
            ...
            if (old_state == WLFC_HANGER_ITEM_STATE_INUSE_SUPPRESSED) {
                printf(
                    "ERROR: free a suppressed pkt %p state %d pkt_state %d\n",
        ...
        if (item->state == WLFC_HANGER_ITEM_STATE_FREE) {
            DHD_ERROR(
                ("Error: %s():%d Multiple TXSTATUS or BUSRETURNED: %d (%d)\n",
    ...
    for (i = 0; i < total_entries; i++) {
        if (table[i].occupied) {
            ...
            if (WLFC_GET_AFQ(dhd->wlfc_mode) && (table[i].afq.n_pkts_tot)) {
                _dhd_wlfc_pktq_flush(wlfc, &table[i].afq, TRUE, fn, arg,
                                     Q_TYPE_AFQ);
            }

            if ((fn == NULL) &&
                (&table[i] != &wlfc->destination_entries.other)) {
                table[i].occupied = 0;
                                      ...
                                      uint8 *ea, f_processpkt_t fn, void *arg)
{
    ...
    if ((action == eWLFC_MAC_ENTRY_ACTION_ADD) ||
        ...
        /* When the entry is deleted, the packets that are queued in the entry
           must be cleanup. The cleanup action should be before the occupied is
           set as 0.
        */
                                   ...
                                   bool bBorrowAll)
{
    ...
    for (lender_ac = 0; lender_ac <= highest_lender_ac; lender_ac++) {
        if (!bBorrowAll) {
            borrow_limit =
                ctx->Init_FIFO_credit[lender_ac] / WLFC_BORROW_LIMIT_RATIO;
        } else {
            borrow_limit = 0;
        }
...
static int _dhd_wlfc_interface_entry_update(void *state, uint8 action,
                                            uint8 ifid, uint8 iftype, uint8 *ea)
{
...
static int _dhd_wlfc_FIFOcreditmap_update(void *state, uint8 *credits)
{
    ...
    ctx->FIFO_credit[0x2] += (credits[0x2] - ctx->Init_FIFO_credit[0x2]);
    ctx->FIFO_credit[0x3] += (credits[0x3] - ctx->Init_FIFO_credit[0x3]);
    ctx->FIFO_credit[0x4] += (credits[0x4] - ctx->Init_FIFO_credit[0x4]);

    ctx->Init_FIFO_credit[0] = credits[0];
    ctx->Init_FIFO_credit[1] = credits[1];
    ctx->Init_FIFO_credit[0x2] = credits[0x2];
    ctx->Init_FIFO_credit[0x3] = credits[0x3];
    ctx->Init_FIFO_credit[0x4] = credits[0x4];

    /* credit for ATIM FIFO is not used yet. */
    ctx->Init_FIFO_credit[0x5] = ctx->FIFO_credit[0x5] = 0;
                                          ...
                                          void *commit_ctx)
{
    ...
    */
    ...
    rc =
        _dhd_wlfc_pretx_pktprocess(ctx, commit_info->mac_entry, &commit_info->p,
    ...
    if (rc != BCME_OK) {
        ...
        */
        _dhd_wlfc_rollback_packet_toq(ctx, commit_info->p,
...
{
    wlfc_mac_descriptor_t *table = ((athost_wl_status_info_t *)dhdp->wlfc_state)
                                                ...
                                                uint8 len, void **p_mac)
{
    ...
    while (count < len) {
        ...
        if (DHD_PKTTAG_CREDITCHECK(PKTTAG(pktbuf))) {
            ...
            /*
            if this packet did not count against FIFO credit, it must have
            taken a requested_credit from the destination entry (for pspoll
            etc.)
            */
...
static int _dhd_wlfc_fifocreditback_indicate(dhd_pub_t *dhd, uint8 *credits)
{
    ...
    for (i = 0; i < WLFC_CTL_VALUE_LEN_FIFO_CREDITBACK; i++) {
        ...
        if (dhd->proptxstatus_mode == WLFC_FCMODE_EXPLICIT_CREDIT) {
            ...
            /* Return credits to highest priority lender first */
            for (lender = AC_COUNT; (lender >= 0) && (credits[i] > 0);
                 lender--) {
...
#ifndef BCMDBUS
/** !BCMDBUS specific function */
static void _dhd_wlfc_suppress_txq(dhd_pub_t *dhd, f_processpkt_t fn, void *arg)
{
    athost_wl_status_info_t *wlfc = (athost_wl_status_info_t *)dhd->wlfc_state;
    wlfc_mac_descriptor_t *entry;
    int prec;
...
static int _dhd_wlfc_dbg_senum_check(dhd_pub_t *dhd, uint8 *value)
{
    ...
    bcopy(&value[0x2], &timestamp, sizeof(uint32));
...
static int _dhd_wlfc_mac_table_update(dhd_pub_t *dhd, uint8 *value, uint8 type)
{
    int rc;
    athost_wl_status_info_t *wlfc = (athost_wl_status_info_t *)dhd->wlfc_state;
    wlfc_mac_descriptor_t *table;
    ...
    if (type == WLFC_CTL_TYPE_MACDESC_ADD) {
        existing_index = _dhd_wlfc_find_mac_desc_id_from_mac(dhd, &value[0x2]);
        if ((existing_index != WLFC_MAC_DESC_ID_INVALID) &&
            ...
            */
            rc = _dhd_wlfc_mac_entry_update(
                ...
                table[existing_index].interface_id,
        ...
        if (!table[table_index].occupied) {
            ...
            rc = _dhd_wlfc_mac_entry_update(
                ...
                wlfc->destination_entries.interfaces[ifid].iftype, ea, NULL,
                NULL);
        } else {
...
static int _dhd_wlfc_credit_request(dhd_pub_t *dhd, uint8 *value)
{
    ...
    if (desc->occupied) {
        ...
        desc->ac_bitmap = value[0x2] & (~(1 << AC_COUNT));
...
{
    if (info_len) {
        /* Check copy length to avoid buffer overrun. In case of length
         * exceeding WLHOST_REORDERDATA_TOTLEN, return failure instead sending
         * incomplete result of length WLHOST_REORDERDATA_TOTLEN
         */
                               ...
                               uchar *reorder_info_buf, uint *reorder_info_len)
{
    ...
    if (remainder) {
        while ((processed < (WLFC_MAX_PENDING_DATALEN * 0x2)) &&
               (remainder > 0)) {
            ...
            value = &tmpbuf[processed + 0x2];

            if (remainder < (0x2 + len)) {
                break;
            }
            ...
            if (type == WLFC_CTL_TYPE_TXSTATUS) {
                ...
                _dhd_wlfc_compressed_txstatus_update(
                    dhd, value, value[compcnt_offset], &entry);
        ...
        */
        ...
        for (ac = AC_COUNT; ac >= 0; ac--) {
            ...
            while ((FALSE == dhdp->proptxstatus_txoff) &&
                ...
                commit_info.p = _dhd_wlfc_deque_delayedq(
                    ...
                    &(commit_info.needs_hdr), &(commit_info.mac_entry),
                    no_credit);
                ...
                if (!dhdp->proptxstatus_credit_ignore && (lender == -1)) {
                    ASSERT(ctx->FIFO_credit[ac] >=
                           commit_info.ac_fifo_credit_spent);
                }
                /* here we can ensure have credit or no credit needed */
                rc = _dhd_wlfc_handle_packet_commit(
                    ctx, ac, &commit_info, ctx->fcommit, ctx->commit_ctx);
...
#ifdef LIMIT_BORROW
                    else if (!commit_info.ac_fifo_credit_spent &&
                             (lender != -1) && dhdp->wlfc_borrow_allowed) {
                        _dhd_wlfc_return_credit(ctx, lender, ac);
                    }
#endif // endif
                } else {
        ...
        if (((tx_map & (tx_map - 1)) == 0) && (tx_map >= rx_map)) {
            ...
            if ((single_ac == ctx->single_ac) && ctx->allow_credit_borrow) {
                ...
                if (single_ac != ctx->single_ac) {
                    /* new single ac traffic (first single ac or different
                     * single ac) */
            ...
            /* If we have multiple AC traffic, turn off borrowing, mark time and
             * bail out */
            ctx->allow_credit_borrow = 0;
        ...
        while (FALSE == dhdp->proptxstatus_txoff) {
            ...
            if (dhdp->wlfc_borrow_allowed) {
                if ((lender = _dhd_wlfc_borrow_credit(ctx, AC_COUNT, ac,
                                                      TRUE)) == -1) {
                    break;
                }
            } else {
            ...
            if (commit_info.p == NULL) {
                /* before borrow only one ac exists and now this only ac is
                 * empty */
#ifdef LIMIT_BORROW
                _dhd_wlfc_return_credit(ctx, lender, ac);
#endif // endif
...
int dhd_wlfc_txcomplete(dhd_pub_t *dhd, void *txp, bool success)
{
    ...
    if (!success || dhd->proptxstatus_txstatus_ignore) {
        ...
        if (!WLFC_GET_AFQ(dhd->wlfc_mode)) {
            _dhd_wlfc_hanger_poppkt(
                wlfc->hanger,
                WL_TXSTATUS_GET_HSLOT(DHD_PKTTAG_H2DTAG(PKTTAG(txp))), &pout,
                TRUE);
            ASSERT(txp == pout);
        }
...
int dhd_wlfc_init(dhd_pub_t *dhd)
{
    ...
    */
    ...
    if (!ret) {
        ...
        ret = dhd_wl_ioctl_set_intiovar(dhd, "wlfc_mode", mode, WLC_SET_VAR,
                                        TRUE, 0);
    }
    ...
    DHD_INFO(
        ("dhd_wlfc_init(): wlfc_mode=0x%x, ret=%d\n", dhd->wlfc_mode, ret));
#ifdef LIMIT_BORROW
                             ...
                             uint8 iftype, uint8 *ea)
{
    ...
    rc = _dhd_wlfc_interface_entry_update(dhdp->wlfc_state, action, ifid,
                                          iftype, ea);
...
int dhd_wlfc_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
    ...
    if (!WLFC_GET_AFQ(dhdp->wlfc_mode)) {
        ...
        if (h == NULL) {
            ...
            bcm_bprintf(strbuf,
                        ...
                        h->pushed, h->popped, h->failed_to_push,
                        h->failed_to_pop, h->failed_slotfind,
                        (h->pushed - h->popped));
        }
    }
    ...
    bcm_bprintf(
        ...
        wlfc->Init_FIFO_credit[0], wlfc->FIFO_credit[0],
        wlfc->stats.send_pkts[0], wlfc->stats.drop_pkts[0],
        wlfc->stats.drop_pkts[1], wlfc->stats.ooo_pkts[0],
        wlfc->Init_FIFO_credit[1], wlfc->FIFO_credit[1],
        wlfc->stats.send_pkts[1], wlfc->stats.drop_pkts[0x2],
        wlfc->stats.drop_pkts[0x3], wlfc->stats.ooo_pkts[1],
        wlfc->Init_FIFO_credit[0x2], wlfc->FIFO_credit[0x2],
        wlfc->stats.send_pkts[0x2], wlfc->stats.drop_pkts[0x4],
        wlfc->stats.drop_pkts[0x5], wlfc->stats.ooo_pkts[0x2],
        wlfc->Init_FIFO_credit[0x3], wlfc->FIFO_credit[0x3],
        wlfc->stats.send_pkts[0x3], wlfc->stats.drop_pkts[0x6],
        wlfc->stats.drop_pkts[0x7], wlfc->stats.ooo_pkts[0x3],
        wlfc->Init_FIFO_credit[0x4], wlfc->FIFO_credit[0x4],
        wlfc->stats.send_pkts[0x4], wlfc->stats.drop_pkts[0x8],
        wlfc->stats.drop_pkts[0x9], wlfc->stats.ooo_pkts[0x4]);
    ...
    for (i = 0; i < WLFC_MAX_IFNUM; i++) {
        if (interfaces[i].occupied) {
            ...
            bcm_bprintf(strbuf,
                        ...
                        i, MAC2STRDBG(ea), interfaces[i].interface_id,
                        iftype_desc,
            ...
            bcm_bprintf(
                ...
                i, interfaces[i].psq.q[0].n_pkts, interfaces[i].psq.q[1].n_pkts,
                interfaces[i].afq.q[0].n_pkts, interfaces[i].psq.q[0x2].n_pkts,
                interfaces[i].psq.q[0x3].n_pkts, interfaces[i].afq.q[1].n_pkts,
                interfaces[i].psq.q[0x4].n_pkts, interfaces[i].psq.q[0x5].n_pkts,
                interfaces[i].afq.q[0x2].n_pkts, interfaces[i].psq.q[0x6].n_pkts,
                interfaces[i].psq.q[0x7].n_pkts, interfaces[i].afq.q[0x3].n_pkts,
                interfaces[i].psq.q[0x8].n_pkts, interfaces[i].psq.q[0x9].n_pkts,
                interfaces[i].afq.q[0x4].n_pkts);
        }
    }
    ...
    for (i = 0; i < WLFC_MAC_DESC_TABLE_SIZE; i++) {
        if (mac_table[i].occupied) {
            ...
            bcm_bprintf(
                ...
                mac_table[i].afq.q[0].n_pkts, mac_table[i].psq.q[0x2].n_pkts,
                mac_table[i].psq.q[0x3].n_pkts, mac_table[i].afq.q[1].n_pkts,
                mac_table[i].psq.q[0x4].n_pkts, mac_table[i].psq.q[0x5].n_pkts,
                mac_table[i].afq.q[0x2].n_pkts, mac_table[i].psq.q[0x6].n_pkts,
                mac_table[i].psq.q[0x7].n_pkts, mac_table[i].afq.q[0x3].n_pkts,
                mac_table[i].psq.q[0x8].n_pkts, mac_table[i].psq.q[0x9].n_pkts,
                mac_table[i].afq.q[0x4].n_pkts);
        }
    }
...
#ifdef PROP_TXSTATUS_DEBUG
    {
        ...
        if (wlfc->stats.latency_sample_count) {
            ...
            avg = (0x64 * wlfc->stats.total_status_latency) /
                  wlfc->stats.latency_sample_count;
            bcm_bprintf(strbuf,
                        "txstatus latency (average, last, moving[%d]) = "
                        "(%d.%d, %03d, %03d)\n",
                        moving_samples, avg / 0x64, (avg - (avg / 0x64) * 0x64),
    ...
    bcm_bprintf(
        ...
        wlfc->stats.fifo_credits_sent[0x2], wlfc->stats.fifo_credits_sent[0x3],
        wlfc->stats.fifo_credits_sent[0x4], wlfc->stats.fifo_credits_sent[0x5],

        wlfc->stats.fifo_credits_back[0], wlfc->stats.fifo_credits_back[1],
        wlfc->stats.fifo_credits_back[0x2], wlfc->stats.fifo_credits_back[0x3],
        wlfc->stats.fifo_credits_back[0x4], wlfc->stats.fifo_credits_back[0x5]);
    {
        ...
        for (i = 0; i < WLFC_MAX_IFNUM; i++) {
            if (wlfc->destination_entries.interfaces[i].occupied) {
                request_cr_sent += wlfc->destination_entries.interfaces[i]
    ...
    for (i = 0; i < WLFC_MAX_IFNUM; i++) {
        bcm_bprintf(strbuf,
                    ...
                    i, wlfc->pkt_cnt_in_q[i][0], wlfc->pkt_cnt_in_q[i][1],
                    wlfc->pkt_cnt_in_q[i][0x2], wlfc->pkt_cnt_in_q[i][0x3],
                    wlfc->pkt_cnt_in_q[i][0x4]);
    }
    bcm_bprintf(strbuf, "\n");
