...
const auth_name_map_t auth_name_map[] = {
    ...
    {WL_AUTH_OPEN_SYSTEM,
     WPA3_AUTH_SAE_PSK | WPA2_AUTH_PSK_SHA256 | WPA2_AUTH_PSK, "wpa3psksha256"},
                               ...
                               s32 bsscfg_idx, struct mutex *buf_sync)
{
    ...
    ret = wldev_iovar_setbuf_bsscfg(dev, iovar_name, param, paramlen, buf,
                                    buflen, bsscfg_idx, buf_sync);
    if (ret < 0) {
        AEXT_ERROR(dev->name, "iovar=%s, ret=%d\n", iovar_name, ret);
    }
...
static void wl_ext_ch_to_chanspec(int ioctl_ver, int ch,
                                  ...
                                  size_t *join_params_size)
{
    ...
    if (ch != 0) {
        ...
        *join_params_size +=
        ...
        join_params->params.chanspec_list[0] = wl_ext_chspec_host_to_driver(
            ioctl_ver, join_params->params.chanspec_list[0]);
...
void wl_ext_wait_event_complete(struct dhd_pub *dhd, int ifidx)
{
    ...
    timeout = wait_event_interruptible_timeout(
        dhd->conf->event_complete, wl_ext_event_complete(dhd, ifidx),
        msecs_to_jiffies(0x2710));
...
int wl_ext_set_chanspec(struct net_device *dev, int ioctl_ver, uint16 channel,
    ...
    if (wf_chspec_valid(chspec)) {
        ...
        if (fw_chspec != INVCHANSPEC) {
            if ((err = wl_ext_iovar_setint(dev, "chanspec", fw_chspec)) ==
                ...
                err = wl_ext_ioctl(dev, WLC_SET_CHANNEL, &_chan, sizeof(_chan),
                                   1);
...
static int wl_ext_channel(struct net_device *dev, char *command, int total_len)
{
    int ret;
    int channel = 0;
    channel_info_t ci;
    ...
    if (channel > 0) {
        ...
        if (!(ret = wl_ext_ioctl(dev, WLC_GET_CHANNEL, &ci,
            ...
            bytes_written = snprintf(command, sizeof(channel_info_t) + 0x2,
...
static int wl_ext_channels(struct net_device *dev, char *command, int total_len)
{
    int ret, i;
    int bytes_written = -1;
...
static int wl_ext_monitor(struct net_device *dev, char *command, int total_len)
{
    int val = -1, ret = -1;
    int bytes_written = -1;
...
s32 wl_ext_connect(struct net_device *dev, struct wl_conn_info *conn_info)
{
    ...
    ext_join_params->ssid.SSID_len = min(
        (uint32)sizeof(ext_join_params->ssid.SSID), conn_info->ssid.SSID_len);
    ...
    if (chan_cnt) {
        ...
        ext_join_params->assoc.chanspec_list[0] = wl_ext_chspec_host_to_driver(
            ioctl_ver, ext_join_params->assoc.chanspec_list[0]);
    }
...
set_ssid:
    ...
    wl_ext_ch_to_chanspec(ioctl_ver, conn_info->channel, &join_params,
                          &join_params_size);
                    ...
                    int total_len, bool dump)
{
...
#endif /* WL_EXT_IAPSTA */
    {
        ...
        if (!match) {
            bytes_written +=
                snprintf(sec + bytes_written, total_len, "/0x%x", wsec);
        }
    }
    if (dump) {
    ...
    *chan_5g = 0;
    ...
    if (ret == 0) {
        for (i = 0; i < dtoh32(list->count); i++) {
            ...
            if (chan_tmp <= 0xD && !*chan_2g) {
                *chan_2g = chan_tmp;
            } else if (chan_tmp >= 0x24 && chan_tmp <= 0xA1 && !*chan_5g) {
...
static int wl_ext_easymesh(struct net_device *dev, char *command, int total_len)
{
    int ret = 0, wlc_down = 1, wlc_up = 1, map = 0x4, dwds = 1;
    ...
    } else if (strncmp(command, EASYMESH_MASTER, strlen(EASYMESH_MASTER)) ==
               0) {
        map = dwds = 0;
        WL_MSG(dev->name, "try to set map %d, dwds %d\n", map, dwds);
        ret = wl_ext_ioctl(dev, WLC_DOWN, &wlc_down, sizeof(wlc_down), 1);
                      ...
                      const char *add_del_cmd)
{
    ...
    tot_len = (int)(sizeof(vndr_ie_setbuf_t) + ((strlen(ie_data) - 0x2) / 0x2));
    ...
    if (data) {
        ...
        if (!ret) {
            ...
            bytes_written += snprintf(command + bytes_written, total_len,
                                      "ip_serv %s\n", ip_serv_str);
            bytes_written +=
                snprintf(command + bytes_written, total_len, "lease_time %d\n",
                       ...
                       int total_len)
{
    ...
    if (data) {
        ...
        AEXT_TRACE(dev->name, "id=%d, period=%d, packet=%s", id, period,
                   packet);
        if (period >= 0) {
            ...
            if (!ret) {
                ...
                for (i = 0; i < mkeep_alive_pktp->len_bytes; i++) {
                    bytes_written +=
                        snprintf(command + bytes_written, total_len, "%02x",
                                 ...
                                 char *command, int total_len)
{
    ...
    if (data) {
        ...
        ka_payload_len = strlen(ka_payload) / 0x2;
        ...
        AEXT_INFO(dev->name,
                  "tcpka_conn_add %d %pM %pM %pM %d %d %d %u %u %d %u %u %u %u "
                  "\"%s\"\n",
                  tcpka->sess_id, &tcpka->dst_mac, &tcpka->src_ip,
                  &tcpka->dst_ip, tcpka->ipid, tcpka->srcport, tcpka->dstport,
                  tcpka->seq, tcpka->ack, tcpka->tcpwin, tcpka->tsval,
                  tcpka->tsecr, tcpka->len, tcpka->ka_payload_len,
                  tcpka->ka_payload);

        ret = wl_ext_iovar_setbuf(
            dev, "tcpka_conn_add", (char *)tcpka,
            (sizeof(tcpka_conn_t) + tcpka->ka_payload_len - 1), iovar_buf,
            sizeof(iovar_buf), NULL);
    }
                                    ...
                                    char *command, int total_len)
{
    ...
    if (data) {
        sscanf(data, "%d %d %d %d %d", &sess_id, &flag, &interval,
                                  ...
                                  char *command, int total_len)
{
    ...
    if (data) {
        ...
        ret = wl_ext_iovar_getbuf(dev, "tcpka_conn_sess_info", (char *)&sess_id,
                                  sizeof(uint32), iovar_buf, sizeof(iovar_buf),
                                  NULL);
        if (!ret) {
                        ...
                        int total_len)
{
    ...
    if (data) {
        ...
        params->channel_list[1] = wf_channel2chspec(0x2, WL_CHANSPEC_BW_20);
    ...
    if (data) {
        ...
        if (add == -1) {
            if (isset(eventmask, event)) {
                bytes_written +=
                    snprintf(command + bytes_written, total_len, "1");
            } else {
                bytes_written +=
                    snprintf(command + bytes_written, total_len, "0");
            }
            AEXT_INFO(dev->name, "%s\n", command);
            goto eventmsg_out;
        }
                                 ...
                                 char *command, int total_len)
{
    ...
    if (data) {
        ...
        new_id = simple_strtol(data, NULL, 0xA);
        if (new_id <= 0) {
        ...
        for (i = 0; i < cnt; i++) {
            ...
            filter_id = simple_strtol(pktfilter[i], NULL, 0xA);
            if (new_id == filter_id) {
        ...
        for (i = 0; i < cnt; i++) {
            filter_id = simple_strtol(filter_add->filter[i], NULL, 0xA);
            if (new_id == filter_id) {
                AEXT_ERROR(dev->name, "filter id %d already in list\n",
                           filter_id);
                return -1;
                               ...
                               char *command, int total_len)
{
    ...
    if (data) {
        ...
        err =
            wl_ext_iovar_setbuf(dev, "sendprb", (char *)&params, sizeof(params),
                               ...
                               char *command, int total_len)
{
    ...
    if (data) {
        ...
        memcpy(&keyinfo, data,
               RSN_KCK_LENGTH + RSN_KEK_LENGTH + RSN_REPLAY_LEN);
        err =
            wl_ext_iovar_setbuf(dev, "gtk_key_info", &keyinfo, sizeof(keyinfo),
                                iovar_buf, sizeof(iovar_buf), NULL);
        if (err) {
                               ...
                               char *command, int total_len)
{
    ...
    uint buf_len = 0;
    int offset;
    char mask[128] = "\0", pattern[128] = "\0", add[0x4] = "\0", mask_tmp[128],
         *pmask_tmp;
    ...
    if (data) {
        ...
        if (!ret) {
            ...
            ret =
                snprintf(command, total_len, "#of patterns :%d\n", list->count);
            ...
            for (i = 0; i < list->count; i++) {
                ...
                ret += snprintf(command + ret, total_len,
                                ...
                                "Masksize   :%d\n"
                                "Mask       :0x",
                                i + 1, (uint32)wowl_pattern->id,
                                wowl_pattern->offset, wowl_pattern->masksize);
                pattern = ((uint8 *)wowl_pattern + sizeof(wl_wowl_pattern_t));
                for (j = 0; j < wowl_pattern->masksize; j++) {
                    ret +=
                        snprintf(command + ret, total_len, "%02x", pattern[j]);
                }
                ret += snprintf(command + ret, total_len, "\n");
                ret += snprintf(command + ret, total_len,
                                "PatternSize:%d\n"
                               ...
                               char *command, int total_len)
{
    ...
    if (data) {
        ...
        if (!ret) {
            ...
            ret = snprintf(command, total_len, "wakeind=0x%x",
                           wake->ucode_wakeind);
                              ...
                              int total_len)
{
    ...
    char frame_str[WLC_IOCTL_SMLEN + 0x3];
...
static int wl_ether_atoe(const char *a, struct ether_addr *n)
{
    char *c = NULL;
    int i = 0;
    ...
    for (;;) {
        n->octet[i++] = (uint8)strtoul(a, &c, 0x10);
                      ...
                      int total_len)
{
    ...
    if (data) {
        ...
        ret = wl_ext_iovar_getbuf(dev, "csi_list", NULL, 0, buf,
                                  WLC_IOCTL_SMLEN, NULL);
        if (!ret) {
...
#ifdef WL_CFG80211
        bytes_written = wl_cfg80211_set_country_code(dev, country_code, true,
                                                     true, revinfo);
#else
        bytes_written =
            wldev_set_country(dev, country_code, true, true, revinfo);
#endif /* WL_CFG80211 */
    } else {
                            ...
                            int total_len, int *bytes_written)
{
    ...
    if (strnicmp(command, CMD_CHANNELS, strlen(CMD_CHANNELS)) == 0) {
        ...
        *bytes_written = wl_ext_monitor(net, command, total_len);
        ...
        int bcn_li_dtim;
        bcn_li_dtim = (int)simple_strtol(
            (command + strlen(CMD_SET_SUSPEND_BCN_LI_DTIM) + 1), NULL, 0xA);
    ...
    } else if (strnicmp(command, CMD_IAPSTA_CONFIG,
                        strlen(CMD_IAPSTA_CONFIG)) == 0 ||
               strnicmp(command, CMD_ISAM_CONFIG, strlen(CMD_ISAM_CONFIG)) == 0) {
        *bytes_written = wl_ext_iapsta_config(net, command, total_len);
    } else if (strnicmp(command, CMD_IAPSTA_ENABLE,
                        strlen(CMD_IAPSTA_ENABLE)) == 0 ||
    ...
    else if (strnicmp(command, CMD_ISAM_PEER_PATH,
                      strlen(CMD_ISAM_PEER_PATH)) == 0) {
                            ...
                            int ioctl_ver, int *best_2g_ch, int *best_5g_ch)
{
    ...
    wl_bss_cache_t *node;
#endif /* BSSCACHE */
    int b_band[CH_MAX_2G_CHANNEL] = {0}, a_band1[0x4] = {0}, a_band4[0x5] = {0};
    s32 cen_ch, distance, distance_2g, distance_5g, ch, min_ap = 0x3E7;
    u8 valid_chan_list[sizeof(u32) * (WL_NUMCHANNELS + 1)];
    wl_uint32_list_t *list;
    int ret;
    chanspec_t chanspec;
    ...
    if (ret < 0) {
        ...
        for (i = 0; i < dtoh32(list->count); i++) {
            ...
            if (ch < CH_MAX_2G_CHANNEL) {
                ...
                a_band1[(ch - 0x24) / 0x4] = 0;
            } else if (ch >= 0x95 && ch <= 0xA1) {
                a_band4[(ch - 0x95) / 0x4] = 0;
            }
        }
    }
        ...
        if (CHSPEC_IS2G(chanspec)) {
            ...
            if (cen_ch <= 0x30) {
                for (j = 0; j < ARRAYSIZE(a_band1); j++) {
                    if (a_band1[j] >= 0 &&
                        abs(cen_ch - (0x24 + j * 0x4)) <= distance) {
                        a_band1[j] += 1;
                    }
                }
            } else if (cen_ch >= 0x95) {
                for (j = 0; j < ARRAYSIZE(a_band4); j++) {
                    if (a_band4[j] >= 0 &&
                        abs(cen_ch - (0x95 + j * 0x4)) <= distance) {
                        a_band4[j] += 1;
                    }
                }
            }
    ...
    *best_5g_ch = 0;
    ...
    if (ohos_msg_level & OHOS_INFO_LEVEL) {
        ...
        bcm_bprintf(&strbuf, "\n");
        for (j = 0; j < ARRAYSIZE(a_band1); j++) {
            bcm_bprintf(&strbuf, "%d/%d, ", a_band1[j], 0x24 + j * 0x4);
        }
        bcm_bprintf(&strbuf, "\n");
        for (j = 0; j < ARRAYSIZE(a_band4); j++) {
            bcm_bprintf(&strbuf, "%d/%d, ", a_band4[j], 0x95 + j * 0x4);
        }
        bcm_bprintf(&strbuf, "\n");
...
static int wl_ext_fw_apcs(struct net_device *dev, uint32 band)
{
...
#ifdef WL_ESCAN
int wl_ext_drv_scan(struct net_device *dev, uint32 band, bool fast_scan)
{
    int ret = -1, i, cnt = 0;
    ...
    if (band == WLC_BAND_5G || band == WLC_BAND_AUTO) {
        for (i = 0; i < 0x4; i++) {
            scan_info.channels.channel[i + cnt] = 0x24 + i * 0x4;
        }
        cnt += 0x4;
        for (i = 0; i < 0x4; i++) {
            scan_info.channels.channel[i + cnt] = 0x95 + i * 0x4;
        }
        cnt += 0x4;
    }
    scan_info.channels.count = cnt;
    if (fast_scan) {
        scan_info.scan_time = 0x28;
    }
...
int wl_ext_drv_apcs(struct net_device *dev, uint32 band)
{
    int ret = 0, channel = 0;
    struct dhd_pub *dhd = dhd_get_pub(dev);
...
int wl_ext_autochannel(struct net_device *dev, uint acs, uint32 band)
{
    ...
    if (acs & ACS_FW_BIT) {
        int ret = 0;
        ret = wldev_ioctl_get(dev, WLC_GET_CHANNEL_SEL, &channel,
                              sizeof(channel));
        channel = 0;
                                   ...
                                   int *rssi_avg)
{
    ...
    if (timeout.tv_sec < now.tv_sec) {
         ...
         */
        AEXT_TRACE(net->name,
                   "Too long timeout (secs=%d) to ever happen - now=%lu, "
                   "timeout=%lu\n",
    ...
    for (i = 0; i < ss_list->count; i++) {
        ...
        for (; node;) {
            if (!memcmp(&node->BSSID, &bi->BSSID, ETHER_ADDR_LEN)) {
                AEXT_INFO("wlan",
                          "Update %d with BSSID %pM, RSSI=%3d, SSID \"%s\"\n",
    ...
    wl_bss_cache_t *node)
{
    ...
    for (; node;) {
        ...
        k++;
        AEXT_TRACE(
            "wlan", "dump %d with cached BSSID %pM, RSSI=%3d, SSID \"%s\"\n", k,
            &node->results.bss_info->BSSID, rssi, node->results.bss_info->SSID);
        node = node->next;
    }
}
                         ...
                         wl_scan_results_t *ss_list)
{
    ...
    for (i = 0; i < ss_list->count; i++) {
        ...
        for (; node;) {
            if (!memcmp(&node->results.bss_info->BSSID, &bi->BSSID,
                        ETHER_ADDR_LEN)) {
                if (node == *bss_head) {
                    *bss_head = node->next;
                } else {
