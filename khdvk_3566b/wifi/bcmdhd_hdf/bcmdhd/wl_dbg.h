...
extern int wl_print_backtrace(const char *prefix, void *i_backtrace,
                              int i_backtrace_depth);
#else
...
/* Ideally this should be some include file that vendors can include to
 * conditionalize logging */

/* DBGONLY() macro to reduce ifdefs in code for statements that are only needed
 * when BCMDBG is defined.
 */
#define DBGONLY(x) x
...
#define WL_RATE(args)                                                          \
    do {                                                                       \
        if (wl_msg_level & WL_RATE_VAL)                                        \
            WL_PRINT(args);                                                    \
    } while (0)
#define WL_ASSOC(args)                                                         \
    do {                                                                       \
        if (wl_msg_level & WL_ASSOC_VAL)                                       \
            WL_PRINT(args);                                                    \
        else                                                                   \
            WIFICC_LOGDEBUG(args);                                             \
    } while (0)
...
#define WL_REGULATORY(args)                                                    \
    do {                                                                       \
        if (wl_msg_level & WL_REGULATORY_VAL)                                  \
            WL_PRINT(args);                                                    \
        else                                                                   \
            WIFICC_LOGDEBUG(args);                                             \
    } while (0)
...
#define WL_WSEC_DUMP(args)                                                     \
    EVENT_LOG_FAST_CAST_PAREN_ARGS(EVENT_LOG_TAG_WL_WSEC_DUMP, args)
...
#else
#define WL_BAM_ERR(args)                                                       \
    EVENT_LOG_COMPACT_CAST_PAREN_ARGS(EVENT_LOG_TAG_BAM, args)
#endif /* USE_EVENT_LOG_RA */
#else
...
#define WL_BSSTRANS_INFO(args)                                                 \
    EVENT_LOG_RA(EVENT_LOG_TAG_WNM_BSSTRANS_INFO, args)
#else
#define WL_BSSTRANS_INFO(args)                                                 \
