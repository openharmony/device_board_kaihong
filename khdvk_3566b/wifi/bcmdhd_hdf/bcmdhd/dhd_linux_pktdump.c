...
#define TX_FATE_STR(fate)                                                      \
    (((*fate) <= (WLFC_CTL_PKTFLAG_MKTFREE)) ? (tx_pktfate[(*fate)])           \
...
#define EAP_PRINT(x, args...)                                                  \
    do {                                                                       \
        if (dump_msg_level & DUMP_EAPOL_VAL) {                                 \
            if (tx) {                                                          \
                DHD_PKTDUMP((DHD_LOG_PREFIX "[%s] 802_1X " x                   \
                                            " [TX] : (%s) %s (%s)" TXFATE_FMT  \
                                            "\n",                              \
                             ifname, ##args, tx ? seabuf : deabuf,             \
                             tx ? "->" : "<-", tx ? deabuf : seabuf,           \
...
#define EAP_PRINT_OTHER(x, args...)                                            \
    do {                                                                       \
        if (dump_msg_level & DUMP_EAPOL_VAL) {                                 \
            if (tx) {                                                          \
                DHD_PKTDUMP(                                                   \
                    (DHD_LOG_PREFIX "[%s] 802_1X " x " [TX] : (%s) %s (%s) "   \
...
msg_eapol_t dhd_is_4way_msg(uint8 *pktdata)
{
    ...
    if (eap_key->type == EAPOL_WPA2_KEY) {
        if (pair && !install && ack && !mic && !sec && !kerr && !req) {
            ...
            type = EAPOL_OTHER;
            if (dump_msg_level & DUMP_EAPOL_VAL) {
                printf("WPA2: key_info=0x%x, pair=%d, ack=%d, mic=%d, sec=%d, "
                       "kerr=%d, req=%d\n",
        ...
        if (pair && !install && ack && !mic && !sec && !kerr && !req) {
            ...
            type = EAPOL_OTHER;
            if (dump_msg_level & DUMP_EAPOL_VAL) {
                printf("WPA: key_info=0x%x, pair=%d, ack=%d, mic=%d, sec=%d, "
                       "kerr=%d, req=%d\n",
                       key_info, pair, ack, mic, sec, kerr, req);
            }
        }
        ...
        type = EAPOL_OTHER;
        if (dump_msg_level & DUMP_EAPOL_VAL) {
            printf("OTHER: key_info=0x%x, pair=%d, ack=%d, mic=%d, sec=%d, "
                   "kerr=%d, req=%d\n",
                   key_info, pair, ack, mic, sec, kerr, req);
        }
    }
                  ...
                  bool tx, uint32 *pkthash, uint16 *pktfate)
{
    ...
    if (ether_type == ETHER_TYPE_802_1X) {
        dhd_dump_eapol_message(dhdp, ifidx, pktdata, pktlen, tx, pkthash,
                               pktfate);
    }
...
void dhd_dump_pkt_init(dhd_pub_t *dhdp)
{
    ...
    if (!pktcnts) {
        DHD_ERROR(
            ("%s: failed to allocate memory for pktcnts\n", __FUNCTION__));
        return;
    }
...
static INLINE bool dhd_dump_pkt_enabled(dhd_pub_t *dhdp)
{
    return FALSE;
}
                                 ...
                                 uint16 *pktfate)
{
    ...
    bcm_ether_ntoa((struct ether_addr *)(pktdata + 0x6), seabuf);
    ...
    if (eap_wsc->opcode == WSC_OPCODE_MSG) {
        ...
        uint16 msglen;
        uint16 wsc_data_len =
            (uint16)(eap_len - EAP_HDR_LEN - EAP_WSC_DATA_OFFSET);
    ...
    if (eap_hdr->code == EAP_CODE_REQUEST ||
        ...
        switch (eap_hdr->type) {
            ...
            case EAP_TYPE_AKAP:
                ...
                break;
            case EAP_TYPE_EXP:
                dhd_dump_wsc_message(dhd, ifidx, pktdata, pktlen, tx, pkthash,
                                     pktfate);
                break;
            default:
...
#define DHCP_PRINT(str)                                                        \
    do {                                                                       \
        if (tx) {                                                              \
            DHD_PKTDUMP((DHD_LOG_PREFIX                                        \
                         "[%s] " str                                           \
                         " %8s[%8s] [TX] : %s(%s) %s %s(%s)" TXFATE_FMT "\n",  \
                         ifname, typestr, opstr, tx ? sabuf : dabuf,           \
                         tx ? seabuf : deabuf, tx ? "->" : "<-",               \
                         tx ? dabuf : sabuf, tx ? deabuf : seabuf,             \
                         TX_PKTHASH(pkthash), TX_FATE(pktfate)));              \
        } else {                                                               \
            DHD_PKTDUMP((DHD_LOG_PREFIX "[%s] " str                            \
                                        " %8s[%8s] [RX] : %s(%s) %s %s(%s)\n", \
                         ifname, typestr, opstr, tx ? sabuf : dabuf,           \
                         tx ? seabuf : deabuf, tx ? "->" : "<-",               \
                         tx ? dabuf : sabuf, tx ? deabuf : seabuf));           \
...
static char dhcp_ops[][10] = {"NA", "REQUEST", "REPLY"};
static char dhcp_types[][10] = {"NA",      "DISCOVER", "OFFER",
                                "REQUEST", "DECLINE",  "ACK",
                                "NAK",     "RELEASE",  "INFORM"};
        ...
        while (ptr < end && *ptr != 0xff) {
            ...
            if (*opt == DHCP_OPT_MSGTYPE) {
                if (opt[1]) {
                    ...
                    opstr = dhcp_ops[b->op];
                    DHD_STATLOG_DATA(dhdp, dhcp_types_stat[dhcp_type], ifidx,
                                     tx, cond);
                    DHCP_PRINT("DHCP");
                    break;
                }
        ...
        if (tx) {                                                              \
            DHD_PKTDUMP_MEM(                                                   \
                (DHD_LOG_PREFIX                                                \
                 ...
                 seqnum, TX_PKTHASH(pkthash), TX_FATE(pktfate)));              \
        } else {                                                               \
            DHD_PKTDUMP_MEM((DHD_LOG_PREFIX                                    \
                             "[%s] " str                                       \
                             " [RX] : %s(%s) %s %s(%s) SEQNUM=%d\n",           \
                             ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf, \
                             tx ? "->" : "<-", tx ? dabuf : sabuf,             \
    ...
    do {                                                                       \
        if (tx) {                                                              \
            DHD_PKTDUMP_MEM(                                                   \
                (DHD_LOG_PREFIX                                                \
                 "[%s] " str                                                   \
                 " [TX] : %s(%s) %s %s(%s) TYPE=%d, CODE=%d" TXFATE_FMT "\n",  \
                 ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf,             \
                 tx ? "->" : "<-", tx ? dabuf : sabuf, tx ? deabuf : seabuf,   \
                 type, code, TX_PKTHASH(pkthash), TX_FATE(pktfate)));          \
        } else {                                                               \
            DHD_PKTDUMP_MEM((DHD_LOG_PREFIX                                    \
                             "[%s] " str " [RX] : %s(%s) %s %s(%s) TYPE=%d,"   \
                             " CODE=%d\n",                                     \
                             ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf, \
                             tx ? "->" : "<-", tx ? dabuf : sabuf,             \
    ...
    do {                                                                       \
        if (tx) {                                                              \
            if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) {          \
                ...
                DHD_PKTDUMP_MEM((DHD_LOG_PREFIX                                \
                                 "[%s] " str                                   \
                                 " [TX] : %s(%s) %s %s(%s)" TXFATE_FMT "\n",   \
                                 ifname, tx ? sabuf : dabuf,                   \
                                 tx ? seabuf : deabuf, tx ? "->" : "<-",       \
                                 tx ? dabuf : sabuf, tx ? deabuf : seabuf,     \
                                 TX_PKTHASH(pkthash), TX_FATE(pktfate)));      \
            }                                                                  \
            ...
            DHD_PKTDUMP_MEM(                                                   \
                (DHD_LOG_PREFIX "[%s] " str " [RX] : %s(%s) %s %s(%s)\n",      \
                 ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf,             \
                 tx ? "->" : "<-", tx ? dabuf : sabuf, tx ? deabuf : seabuf)); \
        }                                                                      \
    } while (0)
    ...
    do {                                                                       \
        if (tx) {                                                              \
            if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) {          \
                DHD_PKTDUMP((DHD_LOG_PREFIX                                    \
                             "[%s] " str                                       \
                             " [TX] : %s(%s) %s %s(%s) op_code=%d" TXFATE_FMT  \
                             "\n",                                             \
                             ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf, \
                             tx ? "->" : "<-", tx ? dabuf : sabuf,             \
                ...
                DHD_PKTDUMP_MEM(                                               \
                    (DHD_LOG_PREFIX                                            \
                     ...
                     ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf,         \
                     tx ? "->" : "<-", tx ? dabuf : sabuf,                     \
                     tx ? deabuf : seabuf, opcode, TX_PKTHASH(pkthash),        \
                     TX_FATE(pktfate)));                                       \
            }                                                                  \
        } else {                                                               \
            DHD_PKTDUMP_MEM((DHD_LOG_PREFIX                                    \
                             "[%s] " str                                       \
                             " [RX] : %s(%s) %s %s(%s) op_code=%d\n",          \
                             ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf, \
                             tx ? "->" : "<-", tx ? dabuf : sabuf,             \
                             tx ? deabuf : seabuf, opcode));                   \
        }                                                                      \
...
#define DNS_REQ_PRINT(str)                                                     \
    do {                                                                       \
        if (tx) {                                                              \
            if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) {          \
                DHD_PKTDUMP(                                                   \
                    (DHD_LOG_PREFIX                                            \
                     ...
                     ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf,         \
                     tx ? "->" : "<-", tx ? dabuf : sabuf,                     \
                     tx ? deabuf : seabuf, id, DNSOPCODE(opcode),              \
                     TX_PKTHASH(pkthash), TX_FATE(pktfate)));                  \
            } else {                                                           \
                DHD_PKTDUMP_MEM(                                               \
                    (DHD_LOG_PREFIX                                            \
            ...
            DHD_PKTDUMP_MEM((DHD_LOG_PREFIX                                    \
                             "[%s] " str                                       \
                             " [RX] : %s(%s) %s %s(%s) ID:0x%04X OPCODE:%s\n", \
                             ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf, \
                             tx ? "->" : "<-", tx ? dabuf : sabuf,             \
                             tx ? deabuf : seabuf, id, DNSOPCODE(opcode)));    \
        }                                                                      \
...
#define DNS_RESP_PRINT(str)                                                    \
    do {                                                                       \
        if (tx) {                                                              \
            if (dump_enabled && pktfate && !TX_FATE_ACKED(pktfate)) {          \
                DHD_PKTDUMP((DHD_LOG_PREFIX                                    \
                             "[%s] " str " [TX] : %s(%s) %s %s(%s) ID:0x%04X " \
                                         "OPCODE:%s RCODE:%d" TXFATE_FMT "\n", \
                             ifname, tx ? sabuf : dabuf, tx ? seabuf : deabuf, \
                             tx ? "->" : "<-", tx ? dabuf : sabuf,             \
                             tx ? deabuf : seabuf, id, DNSOPCODE(opcode),      \
                ...
                DHD_PKTDUMP_MEM((DHD_LOG_PREFIX                                \
                                 "[%s] " str                                   \
                                 " [TX] : %s(%s) %s %s(%s) ID:0x%04X "         \
                                 "OPCODE:%s RCODE:%d" TXFATE_FMT "\n",         \
                                 ifname, tx ? sabuf : dabuf,                   \
                                 tx ? seabuf : deabuf, tx ? "->" : "<-",       \
                                 tx ? dabuf : sabuf, tx ? deabuf : seabuf, id, \
                                 DNSOPCODE(opcode), GET_DNS_RCODE(flags),      \
                                 TX_PKTHASH(pkthash), TX_FATE(pktfate)));      \
            }                                                                  \
        } else {                                                               \
            DHD_PKTDUMP_MEM(                                                   \
                      ...
                      bool tx)
{
    ...
    if (protocol != ETHER_TYPE_BRCM) {
        if (pktdata[0] == 0xFF) {
            ...
            DHD_PKTDUMP((DHD_LOG_PREFIX "[%s] %s MULTICAST DUMP " MACDBG
                                        " - %s\n",
