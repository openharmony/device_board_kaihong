...
static int dhd_wlan_set_power(int on, wifi_adapter_info_t *adapter)
{
    ...
    LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 0)
        if (adapter->sdio_func && adapter->sdio_func->card &&
            adapter->sdio_func->card->host) {
            mdelay(0x64);
...
static int dhd_wlan_get_mac_addr(unsigned char *buf, int ifidx)
{
    ...
    /* EXAMPLE code */
    {
        char macpad[56] = {0x00, 0xaa, 0x9c, 0x84, 0xc7, 0xbc, 0x9b, 0xf6,
        ...
        bcopy(macpad, buf + 0x6, sizeof(macpad));
    }
...
int dhd_wlan_init_gpio(wifi_adapter_info_t *adapter)
{
    ...
    if (root_node) {
        gpio_wl_reg_on =
            of_get_named_gpio(root_node, GPIO_WL_REG_ON_PROPNAME, 0);
#ifdef CUSTOMER_OOB
        gpio_wl_host_wake =
