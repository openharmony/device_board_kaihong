...
typedef struct dhd_bus {
    ...
    void *bus_info;
    dbus_intf_t *drvintf; /* callbacks to lower level, e.g. dbus_usb.c or
                             dbus_usb_linux.c */
    uint8 *fw;
    int fwlen;
    uint32 errmask;
                          ...
                          int size_irb)
{
    ...
    for (i = 0; i < nq; i++) {
        /* MALLOC dbus_irb_tx or dbus_irb_rx, but cast to simple dbus_irb_t
         * linkedlist */
...
static int dbus_rxirbs_fill(dhd_bus_t *dhd_bus)
{
     ...
     * is triggered outside, below while loop has to check and abort posting
     * more to avoid RPC rxq overflow.
     */
    ...
    if (dhd_bus->pub.busstate == DBUS_STATE_UP ||
        ...
        if (dhd_bus->drvintf && dhd_bus->drvintf->send_irb) {
            ...
            if (err == DBUS_ERR_TXDROP) {
                /* tx fail and no completion routine to clean up, reclaim irb
                 * NOW */
                DBUSERR(
                    ("%s: send_irb failed, status = %d\n", __FUNCTION__, err));
                ...
                if (txirb_pending > (dhd_bus->tx_low_watermark * 0x3)) {
...
static int dbus_get_fw_nvram(dhd_bus_t *dhd_bus, char *pfw_path, char *pnv_path)
{
    int bcmerror = -1, i;
    uint len, total_len;
    ...
    if (len > 0 && len < MAX_NVRAMBUF_SIZE) {
        ...
        if (dhd_bus->nvram_len % 0x4) {
            nvram_words_pad = 0x4 - dhd_bus->nvram_len % 0x4;
        }
    } else {
 ...
 */
static int dbus_do_download(dhd_bus_t *dhd_bus, char *pfw_path, char *pnv_path)
{
    int err = DBUS_OK;
...
static int dbus_jumbo_nvram(dhd_bus_t *dhd_bus)
{
    ...
    ret =
        dbus_select_nvram(dhd_bus, dhd_bus->extdl.vars, dhd_bus->extdl.varslen,
...
static int dbus_do_download(dhd_bus_t *dhd_bus)
{
    ...
    dhd_bus->firmware = dbus_get_fw_nvfile(
        ...
        &dhd_bus->fwlen, DBUS_FIRMWARE, 0, 0);
    if (!dhd_bus->firmware) {
        return DBUS_ERR;
    }
#endif
    ...
    nonfwnvram = NULL;
    nonfwnvramlen = 0;
    dhd_bus->nvfile = dbus_get_fw_nvfile(
        dhd_bus->pub.attrib.devid, dhd_bus->pub.attrib.chiprev,
 ...
 * not, the higher level (e.g. dhd_linux.c) has to be notified, and transmit
 * flow control has to be evaluated.
 */
static void BCMFASTPATH dbus_if_send_irb_complete(void *handle,
                                                  dbus_irb_tx_t *txirb,
                                                  int status)
{
    dhd_bus_t *dhd_bus = (dhd_bus_t *)handle;
 ...
 */
static void BCMFASTPATH dbus_if_recv_irb_complete(void *handle,
                                                  dbus_irb_rx_t *rxirb,
                                                  int status)
{
    dhd_bus_t *dhd_bus = (dhd_bus_t *)handle;
...
/**
 * When lower DBUS level signals control IRB completed, higher level (e.g.
...
/**
 * Called as part of DBUS bus registration. Calls back into higher level (e.g.
 * dhd_linux.c) probe function.
 */
static void *dbus_probe(void *arg, const char *desc, uint32 bustype,
                        uint16 bus_no, uint16 slot, uint32 hdrlen)
{
...
/**
 * As part of initialization, higher level (e.g. dhd_linux.c) requests DBUS to
 * prepare for action.
 */
int dhd_bus_register(void)
{
    ...
    err = dbus_bus_register(
        0xa5c, 0x48f, dbus_probe, /* call lower DBUS level register function */
                       ...
                       dbus_extdl_t *extdl, struct shared_info *sh)
{
    ...
    dhd_bus->rx_low_watermark = nrxq / 0x2; /* keep enough posted rx urbs */
    dhd_bus->pub.ntxq = ntxq;
    dhd_bus->tx_low_watermark =
        ntxq / 0x4; /* flow control when too many tx urbs posted */
    ...
    if (dhd_bus->tx_q == NULL) {
        ...
        err =
            dbus_irbq_init(dhd_bus, dhd_bus->tx_q, ntxq, sizeof(dbus_irb_tx_t));
        if (err != DBUS_OK) {
            goto error;
        }
    }
    ...
    if (dhd_bus->rx_q == NULL) {
        ...
        err =
            dbus_irbq_init(dhd_bus, dhd_bus->rx_q, nrxq, sizeof(dbus_irb_rx_t));
        if (err != DBUS_OK) {
            goto error;
        }
    }
...
int dbus_send_pkt(dbus_pub_t *pub, void *pkt, void *info)
{
    return dbus_send_irb(pub, NULL, 0, pkt, info);
}

int dbus_send_ctl(struct dhd_bus *pub, uint8 *buf, int len)
{
    dhd_bus_t *dhd_bus = (dhd_bus_t *)pub;
...
int dbus_recv_ctl(struct dhd_bus *pub, uint8 *buf, int len)
{
    dhd_bus_t *dhd_bus = (dhd_bus_t *)pub;
...
int dbus_poll_intr(dbus_pub_t *pub)
{
    dhd_bus_t *dhd_bus = (dhd_bus_t *)pub;
    ...
    if (dhd_bus->pub.busstate == DBUS_STATE_UP) {
        if (dhd_bus->drvintf && dhd_bus->drvintf->recv_irb_from_ep) {
            status = dhd_bus->drvintf->recv_irb_from_ep(dhd_bus->bus_info, NULL,
                                                        0xff);
        }
    }
    return status;
                     ...
                     void *arg, int len, bool set)
{
    ...
    if (dhd_bus->drvintf && dhd_bus->drvintf->iovar_op) {
        err = dhd_bus->drvintf->iovar_op(dhd_bus->bus_info, name, params, plen,
                                         arg, len, set);
    }
...
void *dhd_dbus_txq(const dbus_pub_t *pub)
{
    return NULL;
}

uint dhd_dbus_hdrlen(const dbus_pub_t *pub)
{
    return 0;
}

void *dbus_get_devinfo(dbus_pub_t *pub)
{
    return pub->dev_info;
}
...
static int dbus_otp(dhd_bus_t *dhd_bus, uint16 *boardtype, uint16 *boardrev)
{
    ...
    cis = MALLOC(dhd_bus->pub.osh, otp_size * 0x2);
    if (cis == NULL) {
        return DBUS_ERR;
    }
    ...
    for (i = 0; i < otp_size; i++) {
        ret = dhd_bus->drvintf->readreg(
            dhd_bus->bus_info, otp_addr + ((otp_sw_rgn + i) << 1), 0x2, &value);
        if (ret != DBUS_OK) {
            MFREE(dhd_bus->pub.osh, cis, otp_size * 0x2);
            return ret;
        }
    ...
    for (i = 0; i < (otp_size << 1);) {
        ...
        if (tup == CISTPL_BRCM_HNBU) {
            switch (cis[i]) {
                case HNBU_BOARDTYPE:
                    btype = (uint16)((cis[i + 0x2] << 0x8) + cis[i + 1]);
                    btype_present = TRUE;
                ...
                case HNBU_BOARDREV:
                    if (tlen == 0x2) {
                        ...
                        brev = (uint16)((cis[i + 0x2] << 0x8) + cis[i + 1]);
                    }
                    brev_present = TRUE;
...
#ifdef PROP_TXSTATUS
    wlfc_enabled =
        (dhd_wlfc_flowcontrol(dhd, onoff, !onoff) != WLFC_UNSUPPORTED);
#endif
    if (!wlfc_enabled) {
                        ...
                        uint16 bus_no, uint16 slot, uint32 hdrlen)
{
    ...
    if (bus) {
        ...
        if (dlneeded >= 0) {
            if (!g_pub) {
                dhd_conf_reset(pub);
                dhd_conf_set_chiprev(pub, bus->pub.attrib.devid,
                                     bus->pub.attrib.chiprev);
        ...
        if (g_pub || dhd_download_fw_on_driverload) {
            if (dlneeded == 0) {
                ...
                if (dbus_download_firmware(bus, bus->fw_path, bus->nv_path) !=
                    DBUS_OK) {
                    goto fail;
                }
#endif
