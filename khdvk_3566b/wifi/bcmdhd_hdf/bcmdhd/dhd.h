...
enum dhd_op_flags {
    ...
    DHD_FLAG_CONCURR_STA_HOSTAP_MODE =
        (DHD_FLAG_STA_MODE | DHD_FLAG_HOSTAP_MODE),
...
enum {
     ...
     * 2. TCPACKs that don't need to hurry delivered remains longer in TXQ so
     * can be suppressed.
     */
    TCPACK_SUP_DELAYTX,
    TCPACK_SUP_HOLD,
...
typedef struct dhd_pub {
    ...
    ulong
        wd_dpc_sched; /* Number of times dhd dpc scheduled by watchdog timer */
    ulong rx_pktgetfail;    /* Number of PKTGET failures in DHD on RX */
    ulong tx_pktgetfail;    /* Number of PKTGET failures in DHD on TX */
    ulong rx_readahead_cnt; /* Number of packets where header read-ahead was
                               used. */
    ulong tx_realloc; /* Number of tx packets we had to realloc for headroom */
    ulong fc_packets; /* Number of flow control pkts recvd */
    ulong tx_big_packets; /* Dropped data packets that are larger than
                             MAX_MTU_SZ */
#ifdef DMAMAP_STATS
...
#define MAXSKBPEND 1024
    ...
    bool
        fw_4way_handshake; /* Whether firmware will to do the 4way handshake. */
#endif                     // endif
#ifdef DEBUG_DPC_THREAD_WATCHDOG
    bool dhd_bug_on;
#endif /* DEBUG_DPC_THREAD_WATCHDOG */
...
#define WIFI_FEATURE_SCAN_RAND                                                 \
    0x2000000                           /* Support MAC & Prb SN randomization */
...
#ifdef KEEP_ALIVE
extern int dhd_dev_start_mkeep_alive(dhd_pub_t *dhd_pub, uint8 mkeep_alive_id,
                                     uint8 *ip_pkt, uint16 ip_pkt_len,
                                     uint8 *src_mac_addr, uint8 *dst_mac_addr,
                                     uint32 period_msec);
...
#else
static INLINE int dhd_set_macaddr_from_file(dhd_pub_t *dhdp)
{
    return 0;
}
#endif /* READ_MACADDR */
...
#else
static INLINE int dhd_check_module_cid(dhd_pub_t *dhdp)
{
    return 0;
}
#endif /* USE_CID_CHECK */
...
#else
static INLINE int dhd_check_module_mac(dhd_pub_t *dhdp)
{
    return 0;
}
...
#else
static INLINE int dhd_read_cis(dhd_pub_t *dhdp)
{
    return 0;
}
...
#define DHD_PERIM_LOCK(dhdp)                                                   \
    do {                                                                       \
    } while (0)
#define DHD_PERIM_UNLOCK(dhdp)                                                 \
    do {                                                                       \
    } while (0)
#define DHD_PERIM_LOCK_ALL(processor_id)                                       \
    do {                                                                       \
    } while (0)
#define DHD_PERIM_UNLOCK_ALL(processor_id)                                     \
    do {                                                                       \
    } while (0)
...
#define DHD_FLOWRING_LIST_UNLOCK(lock, flags)                                  \
    dhd_os_spin_unlock((lock), (flags))
...
#define DHD_DBG_RING_LOCK_DEINIT(osh, lock)                                    \
    dhd_os_dbgring_lock_deinit(osh, (lock))
...
#define DHD_LB_STATS_UPDATE_TXC_HISTO(dhdp, x)                                 \
    dhd_lb_stats_update_txc_histo(dhdp, x)
#define DHD_LB_STATS_UPDATE_RXC_HISTO(dhdp, x)                                 \
    dhd_lb_stats_update_rxc_histo(dhdp, x)
#define DHD_LB_STATS_TXC_PERCPU_CNT_INCR(dhdp)                                 \
    dhd_lb_stats_txc_percpu_cnt_incr(dhdp)
#define DHD_LB_STATS_RXC_PERCPU_CNT_INCR(dhdp)                                 \
    dhd_lb_stats_rxc_percpu_cnt_incr(dhdp)
#else /* !DHD_LB_STATS */
