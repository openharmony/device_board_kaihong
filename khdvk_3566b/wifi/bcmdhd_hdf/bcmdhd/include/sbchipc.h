...
/**
 * In chipcommon rev 49 the pmu registers have been moved from chipc to the pmu
 * core if the 'AOBPresent' bit of 'CoreCapabilitiesExt' is set. If this field
 * is set, the traditional chipc to [pmu|gci|sreng] register interface is
 * deprecated and removed. These register blocks would instead be assigned their
 * respective chipc-specific address space and connected to the Always On
...
typedef volatile struct {
    ...
    /* Note: all timers driven by ILP clock are updated asynchronously to
     * HT/ALP. The CPU must read them twice, compare, and retry if different.
     */
    uint32 pmucontrol; /* 0x600 */
...
#define CC_CAP_UINTCLK                                                         \
    0x00000008 /**< UARTs are driven by internal divided clock */
...
#define CC_ASYNCGPIO                                                           \
    0x00000004 /**< 1=generate GPIO interrupt without backplane clock */
...
#define SCC_LP                                                                 \
    0x00000400 /**< LPOPowerDown, 1: LPO is disabled,                          \
                * 0: LPO is enabled                                            \
                */
#define SCC_FS                                                                 \
    0x00000800 /**< ForceSlowClk, 1: sb/cores running on slow clock,           \
                * 0: power logic control                                       \
                */
#define SCC_IP                                                                 \
    0x00001000 /**< IgnorePllOffReq, 1/0: power logic ignores/honors           \
                * PLL clock disable requests from core                         \
                */
#define SCC_XC                                                                 \
    0x00002000            /**< XtalControlEn, 1/0: power logic does/doesn't    \
...
#define WD_ENABLE_MASK                                                         \
    (WD_SSRESET_PCIE_F0_EN | WD_SSRESET_PCIE_F1_EN | WD_SSRESET_PCIE_F2_EN |   \
...
#define SFLASH_ACT_OP3A4X4D                                                    \
    0x0500 /**< opcode + 3 addr, 4 don't care & 4 data bytes */
#define SFLASH_ACT_OP3A1X4D                                                    \
    0x0700 /**< opcode + 3 addr, 1 don't care & 4 data bytes */
...
#define UART_FCR_FIFO_ENABLE                                                   \
    1 /**< FIFO control register bit controlling FIFO enable/disable */
...
/* pllcontrol registers */
/* ndiv_pwrdn, pwrdn_ch<x>, refcomp_pwrdn, dly_ch<x>, p1div, p2div,
 * _bypass_sdmod */
...
#define DOT11MAC_880MHZ_CLK_DIVISOR_MASK                                       \
    (0xFF << DOT11MAC_880MHZ_CLK_DIVISOR_SHIFT)
#define DOT11MAC_880MHZ_CLK_DIVISOR_VAL                                        \
    (0xE << DOT11MAC_880MHZ_CLK_DIVISOR_SHIFT)
...
#define CST4360_RSRC_INIT_MODE(cs)                                             \
    ((cs & CST4360_RSRC_INIT_MODE_MASK) >> CST4360_RSRC_INIT_MODE_SHIFT)
...
#define PMU43602_CC2_PCIE_CLKREQ_L_WAKE_EN                                     \
    (1 << 1) /* creates gated_pcie_wake, pmu_wakeup logic */
#define PMU43602_CC2_PCIE_PERST_L_WAKE_EN                                      \
    (1 << 2) /* creates gated_pcie_wake, pmu_wakeup logic */
#define PMU43602_CC2_ENABLE_L2REFCLKPAD_PWRDWN (1 << 3)
#define PMU43602_CC2_PMU_WAKE_ALP_AVAIL_EN                                     \
    (1 << 5) /* enable pmu_wakeup to request for ALP_AVAIL */
#define PMU43602_CC2_PERST_L_EXTEND_EN                                         \
    (1 << 9) /* extend perst_l until rsc PERST_OVR comes up */
#define PMU43602_CC2_FORCE_EXT_LPO                                             \
    (1 << 19) /* 1=ext LPO clock is the final LPO clock */
...
#define MUXENAB4349_HOSTWAKE_MASK                                              \
    (0x000000f0) /* configure GPIO for SDIO host_wake */
...
#define CST4347_JTAG_STRAP_ENABLED(cs)                                         \
    (((cs) & (1 << 20)) != 0) /* JTAG strap st */
...
#define PMU32_MAC_MAIN_RSRC_REQ_TIMER                                          \
    ((1 << MAC_RSRC_REQ_TIMER_INT_ENAB_SHIFT) |                                \
...
#define PMU32_MAC_AUX_RSRC_REQ_TIMER                                           \
    ((1 << MAC_RSRC_REQ_TIMER_INT_ENAB_SHIFT) |                                \
     (1 << MAC_RSRC_REQ_TIMER_FORCE_ALP_SHIFT) |                               \
     (1 << MAC_RSRC_REQ_TIMER_FORCE_HT_SHIFT) |                                \
     (1 << MAC_RSRC_REQ_TIMER_FORCE_HQ_SHIFT) |                                \
...
#define CST4335_CHIPMODE_USB20D(cs)                                            \
    (((cs) & (1 << 2)) != 0)                               /**< HSIC || USBDA */
...
#define CCTRL1_4335_SDIO_HOST_WAKE                                             \
    (1 << 2) /* SDIO: 1=configure GPIO0 for host wake */
...
#define CST4350_PKG_MODE(cs)                                                   \
    ((cs & CST4350_PACKAGE_OPTION) >> CST4350_PACKAGE_SHIFT)
...
#define CST4350_PKG_USB_HSIC(cs)                                               \
    (CST4350_PKG_MODE(cs) == (CST4350_PACKAGE_USB_HSIC))
...
#define CST4350_CHIPMODE_SDIOD(cs)                                             \
    (CST4350_IFC_MODE(cs) == (CST4350_IFC_MODE_SDIOD))
#define CST4350_CHIPMODE_USB20D(cs)                                            \
    ((CST4350_IFC_MODE(cs)) == (CST4350_IFC_MODE_USB20D))
#define CST4350_CHIPMODE_HSIC20D(cs)                                           \
    (CST4350_IFC_MODE(cs) == (CST4350_IFC_MODE_HSIC20D))
#define CST4350_CHIPMODE_HSIC30D(cs)                                           \
    (CST4350_IFC_MODE(cs) == (CST4350_IFC_MODE_HSIC30D))
#define CST4350_CHIPMODE_USB30D(cs)                                            \
    (CST4350_IFC_MODE(cs) == (CST4350_IFC_MODE_USB30D))
#define CST4350_CHIPMODE_USB30D_WL(cs)                                         \
    (CST4350_IFC_MODE(cs) == (CST4350_IFC_MODE_USB30D_WL))
#define CST4350_CHIPMODE_PCIE(cs)                                              \
    (CST4350_IFC_MODE(cs) == (CST4350_IFC_MODE_PCIE))
...
#define CST4350_IFC_MODE(cs)                                                   \
    ((cs & CST4350_HOST_IFC_MASK) >> CST4350_HOST_IFC_SHIFT)
...
#define MUXENAB4350_HOSTWAKE_MASK                                              \
    (0x000000f0) /**< configure GPIO for host_wake */
...
#define CC_GCI_04_SDIO_DRVSTR_MASK                                             \
    (0x0f << CC_GCI_04_SDIO_DRVSTR_SHIFT) /* 0x00078000 */
...
#define MUXENAB43430_HOSTWAKE_MASK                                             \
    (0x000000f0) /* configure GPIO for SDIO host_wake */
...
#define PMU_SLEEP_MODE_0                                                       \
    (LHL_PWRSEQCTL_SLEEP_EN | LHL_PWRSEQCTL_PMU_FINAL_PMU_SLEEP_EN)

#define PMU_SLEEP_MODE_1                                                       \
    (LHL_PWRSEQCTL_SLEEP_EN | LHL_PWRSEQCTL_PMU_SLEEP_MODE |                   \
     LHL_PWRSEQCTL_PMU_FINAL_PMU_SLEEP_EN | LHL_PWRSEQCTL_PMU_TOP_ISO_EN |     \
     LHL_PWRSEQCTL_PMU_TOP_SLB_EN | LHL_PWRSEQCTL_PMU_TOP_PWRSW_EN |           \
     LHL_PWRSEQCTL_PMU_CLDO_PD | LHL_PWRSEQCTL_PMU_RSRC6_EN)

#define PMU_SLEEP_MODE_2                                                       \
    (LHL_PWRSEQCTL_SLEEP_EN | LHL_PWRSEQCTL_PMU_SLEEP_MODE |                   \
     LHL_PWRSEQCTL_PMU_FINAL_PMU_SLEEP_EN | LHL_PWRSEQCTL_PMU_TOP_ISO_EN |     \
     LHL_PWRSEQCTL_PMU_TOP_SLB_EN | LHL_PWRSEQCTL_PMU_TOP_PWRSW_EN |           \
     LHL_PWRSEQCTL_PMU_CLDO_PD | LHL_PWRSEQCTL_PMU_LPLDO_PD |                  \
...
#define LHL_PWRUP_CTL                                                          \
    (LHL_PWRUP_ISOLATION_CNT | LHL_PWRUP_RETENTION_CNT | LHL_PWRUP_PWRSW_CNT)
...
#define LHL_PWRUP_CTL_4347                                                     \
    (LHL_PWRUP_ISOLATION_CNT_4347 | LHL_PWRUP_RETENTION_CNT_4347 |             \
...
#define LHL_PWRUP2_CTL                                                         \
    (LHL_PWRUP2_CLDO_DN_CNT | LHL_PWRUP2_LPLDO_DN_CNT |                        \
     LHL_PWRUP2_RSRC6_DN_CN | LHL_PWRUP2_RSRC7_DN_CN)

/* LHL Top Level Power Down Control Register (lhl_top_pwrdn_ctl_adr, Offset
 * 0xE74) */
#define LHL_PWRDN_SLEEP_CNT (0x4)
#define LHL_PWRDN_CTL_MASK (0x3F)

/* LHL Top Level Power Down Control 2 Register (lhl_top_pwrdn2_ctl_adr, Offset
 * 0xE80) */
...
#define LHL_PWRDN2_CTL                                                         \
    (LHL_PWRDN2_CLDO_DN_CNT | LHL_PWRDN2_LPLDO_DN_CNT |                        \
     LHL_PWRDN2_RSRC6_DN_CN | LHL_PWRDN2_RSRC7_DN_CN)
...
/* 4335 MUX options. each nibble belongs to a setting. Non-zero value specifies
 * a logic for now only UART for bootloader.
 */
#define MUXENAB4335_UART_MASK (0x0000000f)

#define MUXENAB4335_UART_SHIFT 0
#define MUXENAB4335_HOSTWAKE_MASK                                              \
    (0x000000f0) /**< configure GPIO for SDIO host_wake */
...
#define CC_SR_CTL0_RSRC_TRIGGER_SHIFT                                          \
    2 /* Rising edge resource trigger 0 to sr_engine  */
#define CC_SR_CTL0_MIN_DIV_SHIFT                                               \
    6 /* Min division value for fast clk in sr_engine */
...
#define SRPWR_DMN1_ARMBPSD_MASK                                                \
    (1 << SRPWR_DMN1_ARMBPSD_SHIFT)                           /* ARM/BP/SDIO */
...
#define SRPWR_DMN3_MACMAIN_MASK                                                \
    (1 << SRPWR_DMN3_MACMAIN_SHIFT) /* MAC/Phy Main */

#define SRPWR_DMN4_MACSCAN (4)                        /* MAC/Phy Scan */
#define SRPWR_DMN4_MACSCAN_SHIFT (SRPWR_DMN4_MACSCAN) /* MAC/Phy Scan */
#define SRPWR_DMN4_MACSCAN_MASK                                                \
    (1 << SRPWR_DMN4_MACSCAN_SHIFT) /* MAC/Phy Scan */
