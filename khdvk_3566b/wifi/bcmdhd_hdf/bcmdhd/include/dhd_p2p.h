...
#define P2P_ADVT_SERV_SE_FIXED_LEN 3 /* Includes only the element ID and len   \
                                      */
#define P2P_ADVT_SERV_INFO_FIXED_LEN                                           \
    7 /* Per ADV Service Instance advt_id +                                    \
...
/* Stop using these obsolete aliases for P2P_PAF_GON_* */
...
BWL_PRE_PACKED_STRUCT struct wifi_p2psd_adp_tpl {
    uint8 llm_pamebi; /* Query Response Length Limit bit 0-6, set to 0 plus
                       * Pre-Associated Message Exchange BSSID Independent bit
                       * 7, set to 0
                       */
    uint8
        adp_id; /* Advertisement Protocol ID: 0 for NQP Native Query Protocol */
} BWL_POST_PACKED_STRUCT;
...
BWL_PRE_PACKED_STRUCT struct wifi_p2psd_adp_ie {
    ...
    uint8 len; /* IE length */
    wifi_p2psd_adp_tpl_t
        adp_tpl; /* Advertisement Protocol Tuple field. Only one
...
BWL_PRE_PACKED_STRUCT struct wifi_p2psd_qreq_frame {
    ...
    uint16 len;     /* Length of service request TLV, 5 plus the size of request
                       data */
    uint8 oui[3];   /* WFA OUI: 0x0050F2 */
    uint8 qreq_vsc[1]; /* Vendor-specific Content: wifi_p2psd_nqp_query_vsc_t
                          type for NQP */
} BWL_POST_PACKED_STRUCT;
...
BWL_PRE_PACKED_STRUCT struct wifi_p2psd_qresp_frame {
    uint16 info_id; /* Info ID: 0xDDDD */
    uint16
        len; /* Lenth of service response TLV, 6 plus the size of resp data */
    uint8 oui[3];       /* WFA OUI: 0x0050F2 */
    uint8 qresp_vsc[1]; /* Vendor-specific Content: wifi_p2psd_qresp_tlv_t type
                           for NQP */
} BWL_POST_PACKED_STRUCT;
