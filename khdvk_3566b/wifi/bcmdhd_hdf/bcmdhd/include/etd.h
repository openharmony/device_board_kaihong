 ...
 */
#define ETD_TAG_JOIN_CLASSIFICATION_INFO                                       \
    10 /* general information about join request */
#define ETD_TAG_JOIN_TARGET_CLASSIFICATION_INFO                                \
    11 /* per target (AP) join information */
#define ETD_TAG_ASSOC_STATE                                                    \
    12 /* current state of the Device association state machine */
#define ETD_TAG_CHANNEL                                                        \
    13 /* current channel on which the association was performed */
#define ETD_TAG_TOTAL_NUM_OF_JOIN_ATTEMPTS                                     \
    14 /* number of join attempts (bss_retries) */
...
/* Ignore chunks for which there are fewer than this many instances,
 * irrespective of size */
#define HEAP_HISTOGRAM_INSTANCE_MIN 4

/*
 * Use the last two length values for chunks larger than this, or when we run
 * out of histogram entries (because we have too many different sized chunks) to
 * store "other"
 */
...
/* Structure to collect debug info of descriptor entry for dma channel on
 * encountering AXI Error */
/* Below three structures are dependant, any change will bump version of all the
 * three */

typedef struct hnd_ext_trap_desc_entry_v1 {
    uint32 ctrl1;  /* descriptor entry at din < misc control bits > */
    uint32 ctrl2;  /* descriptor entry at din <buffer count and address
                      extension> */
    uint32 addrlo; /* descriptor entry at din <address of data buffer, bits
                      31:0> */
    uint32 addrhi; /* descriptor entry at din <address of data buffer, bits
                      63:32> */
} dma_dentry_v1_t;
...
typedef struct hnd_ext_trap_dma_fifo_v1 {
    ...
    uint32 desc_lo; /* Low Address of Descriptor table programmed in DMA
                       register */
    uint32 desc_hi; /* High Address of Descriptor table programmed in DMA
                       register */
    uint16 din;     /* rxin / txin */
    uint16 dout;    /* rxout / txout */
    dma_dentry_v1_t
        dentry[MAX_DMAFIFO_DESC_ENTRIES_V1]; /* Descriptor Entires */
} dma_fifo_v1_t;
...
typedef struct hnd_ext_trap_macphytxerr_v2 {
    ...
    uint16 pad;
    uint32 recv_fifo_status[3]
                           [2]; /* Rcv Status0 & Rcv Status1 for 3 Rx fifos */
