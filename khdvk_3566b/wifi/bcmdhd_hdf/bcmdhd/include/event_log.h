...
typedef struct {
    ...
    /* Keep version and magic last since "header" is appended to the end of
     * logstrs file. */
 ...
 *
 * The FAST versions check the enable of the tag before evaluating the arguments
 * and calling the event_log function.  This adds 5 instructions.  The COMPACT
 * versions evaluate the arguments and call the event_log function
 * unconditionally.  The event_log function will then skip logging if this tag
 * is disabled.
 *
 * To support easy usage of existing debugging (e.g. msglevel) via macro
 * re-definition there are two variants of these macros to help.
 *
 * First there are the CAST versions.  The event_log function normally logs
 * uint32 values or else they have to be cast to uint32.  The CAST versions
 * blindly cast for you so you don't have to edit any existing code.
 *
 * Second there are the PAREN_ARGS versions.  These expect the logging format
 * string and arguments to be enclosed in parentheses.  This allows us to make
 * the following mapping of an existing msglevel macro: #define WL_ERROR(args)
 * EVENT_LOG_CAST_PAREN_ARGS(EVENT_LOG_TAG_WL_ERROR, args)
 *
 * The versions of the macros without FAST or COMPACT in their name are just
 * synonyms for the COMPACT versions.
 *
 * You should use the COMPACT macro (or its synonym) in cases where there is
 * some preceding logic that prevents the execution of the macro, e.g. WL_ERROR
 * by definition rarely gets executed. Use the FAST macro in performance
 * sensitive paths. The key concept here is that you should be assuming that
 * your macro usage is compiled into ROM and can't be changed ... so choose
 * wisely.
 *
 */
 ...
 */
#define _EVENT_LOG0(tag, fmt_num) event_log0(tag, fmt_num)
#define _EVENT_LOG1(tag, fmt_num, t1) event_log1(tag, fmt_num, t1)
#define _EVENT_LOG2(tag, fmt_num, t1, t2) event_log2(tag, fmt_num, t1, t2)
...
#define _EVENT_LOG_CAST0(tag, fmt_num) event_log0(tag, fmt_num)
...
#define _EVENT_LOG_CAST4(tag, fmt_num, t1, t2, t3, t4)                         \
    event_log4(tag, fmt_num, (uint32)(t1), (uint32)(t2), (uint32)(t3),         \
               (uint32)(t4))

/* The rest call the generic routine that takes a count */
#define _EVENT_LOG_CAST5(tag, fmt_num, ...)                                    \
    _EVENT_LOG5(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CAST6(tag, fmt_num, ...)                                    \
    _EVENT_LOG6(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CAST7(tag, fmt_num, ...)                                    \
    _EVENT_LOG7(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CAST8(tag, fmt_num, ...)                                    \
    _EVENT_LOG8(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CAST9(tag, fmt_num, ...)                                    \
    _EVENT_LOG9(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CASTA(tag, fmt_num, ...)                                    \
    _EVENT_LOGA(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CASTB(tag, fmt_num, ...)                                    \
    _EVENT_LOGB(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CASTC(tag, fmt_num, ...)                                    \
    _EVENT_LOGC(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CASTD(tag, fmt_num, ...)                                    \
    _EVENT_LOGD(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CASTE(tag, fmt_num, ...)                                    \
    _EVENT_LOGE(tag, fmt_num, __VA_ARGS__)
#define _EVENT_LOG_CASTF(tag, fmt_num, ...)                                    \
    _EVENT_LOGF(tag, fmt_num, __VA_ARGS__)
...
#define _EVENT_LOG_VA_NUM_ARGS(F, _1, _2, _3, _4, _5, _6, _7, _8, _9, _A, _B,  \
                               _C, _D, _E, _F, N, ...)                         \
    F##N
...
#define _EVENT_LOG(cast, tag, fmt, ...)                                        \
    ...
    static uint32 fmtnum __attribute__((section(".lognums"))) =                \
        (uint32)&logstr;                                                       \
    _EVENT_LOG_VA_NUM_ARGS(cast, ##__VA_ARGS__, F, E, D, C, B, A, 9, 8, 7, 6,  \
                           5, 4, 3, 2, 1, 0)                                   \
...
#define EVENT_LOG_CAST(tag, fmt, ...)                                          \
    EVENT_LOG_COMPACT_CAST(tag, fmt, ##__VA_ARGS__)
...
#define EVENT_LOG_IS_ON(tag)                                                   \
    (*(event_log_tag_sets + (tag)) & ~EVENT_LOG_TAG_FLAG_SET_MASK)
#define EVENT_LOG_IS_LOG_ON(tag)                                               \
    (*(event_log_tag_sets + (tag)) & EVENT_LOG_TAG_FLAG_LOG)
...
void event_log_set_logtrace_trigger_fn(event_log_logtrace_trigger_fn_t fn,
                                       void *ctx);
...
extern void event_log4(int tag, int fmtNum, uint32 t1, uint32 t2, uint32 t3,
                       uint32 t4);
...
extern int event_log_set_destination_set(int set,
                                         event_log_set_destination_t dest);
