...
#else
#define bcm_strncat_s(dst, noOfElements, src, count)                           \
...
/*
 * #define bcm_strcpy_s(dst, count, src)            strncpy((dst), (src),
 * (count)) Use bcm_strcpy_s instead as it is a safer option bcm_strcat_s: Use
 * bcm_strncat_s as a safer option
 *
 */
...
#define bcm_isgraph(c)                                                         \
    ((bcm_ismask(c) & (_BCM_P | _BCM_U | _BCM_L | _BCM_D)) != 0)
...
#define UP_TABLE_MAX                                                           \
    ((IPV4_TOS_DSCP_MASK >> IPV4_TOS_DSCP_SHIFT) + 1) /* 64 max */
...
extern int get_uint8_vararray_slicespecific(osl_t *osh, char *vars,
                                            char *vars_table_accessor,
                                            const char *name, uint8 *dest_array,
                                            uint dest_size);
extern int get_int16_vararray_slicespecific(osl_t *osh, char *vars,
                                            char *vars_table_accessor,
...
extern const bcm_iovar_t *bcm_iovar_lookup(const bcm_iovar_t *table,
                                           const char *name);
extern int bcm_iovar_lencheck(const bcm_iovar_t *table, void *arg, int len,
                              bool set);
...
extern char *bcmstrnstr(const char *s, uint s_len, const char *substr,
                        uint substr_len);
...
#define BCM_IOVT_IS_INT(type)                                                  \
    ((type == IOVT_BOOL) || (type == IOVT_INT8) || (type == IOVT_UINT8) ||     \
     (type == IOVT_INT16) || (type == IOVT_UINT16) || (type == IOVT_INT32) ||  \
     (type == IOVT_UINT32))
...
#ifdef DBG_BUS
/** tracks non typical execution paths, use gdb with arm sim + firmware dump to
 * read counters */
...
/* This error code is *internal* to the driver, and is not propogated to users.
 * It should only be used by IOCTL patch handlers as an indication that it did
 * not handle the IOCTL. (Since the error code is internal, an entry in
 * 'BCMERRSTRINGTABLE' is not required, nor does it need to be part of any OSL
 * driver-to-OS error code mapping).
 */
...
#define BCMERRSTRINGTABLE                                                      \
    {                                                                          \
        "OK", "Undefined error", "Bad Argument", "Bad Option", "Not up",       \
            "Not down", "Not AP", "Not STA", "Bad Key Index", "Radio Off",     \
            "Not band locked", "No clock", "Bad Rate valueset", "Bad Band",    \
            "Buffer too short", "Buffer too long", "Busy", "Not Associated",   \
            "Bad SSID len", "Out of Range Channel", "Bad Channel",             \
            "Bad Address", "Not Enough Resources", "Unsupported",              \
            "Bad length", "Not Ready", "Not Permitted", "No Memory",           \
            "Associated", "Not In Range", "Not Found", "WME Not Enabled",      \
            "TSPEC Not Found", "ACM Not Supported", "Not WME Association",     \
            "SDIO Bus Error", "Dongle Not Accessible", "Incorrect version",    \
            "TX Failure", "RX Failure", "Device Not Present",                  \
            "NMODE Disabled", "Host Offload in device", "Scan Rejected",       \
            "WLCMD usage error", "WLCMD ioctl error", "RWL serial port error", \
            "Disabled", "Decrypt error", "Encrypt error", "MIC error",         \
            "Replay", "IE not found", "Data not found", "NOT GC",              \
            "PRS REQ FAILED", "NO P2P SubElement", "NOA Pending",              \
            "FRAG Q FAILED", "GET ActionFrame failed", "scheduler not ready",  \
            "Last IOV batched sub-cmd", "Mini PMU Cal failed", "R-cal failed", \
            "LPF RC Cal failed", "DAC buf RC Cal failed", "VCO Cal failed",    \
            "band locked", "Dongle Devreset",                                  \
    }
...
/* limit to  max */
#ifndef LIMIT_TO_MAX
#define LIMIT_TO_MAX(x, max) (((x) > (max) ? (max) : (x)))
#endif /* LIMIT_TO_MAX */

/* limit to min */
#ifndef LIMIT_TO_MIN
#define LIMIT_TO_MIN(x, min) (((x) < (min) ? (min) : (x)))
#endif /* LIMIT_TO_MIN */

#define DELTA(curr, prev)                                                      \
    ((curr) > (prev) ? ((curr) - (prev)) : (0xffffffff - (prev) + (curr) + 1))
#define CEIL(x, y) (((x) + ((y)-1)) / (y))
#define ROUNDUP(x, y) ((((x) + ((y)-1)) / (y)) * (y))
#define ROUNDDN(p, align) ((p) & ~((align)-1))
#define ISALIGNED(a, x) (((uintptr)(a) & ((x)-1)) == 0)
#define ALIGN_ADDR(addr, boundary)                                             \
    (void *)(((uintptr)(addr) + (boundary)-1) & ~((boundary)-1))
#define ALIGN_SIZE(size, boundary) (((size) + (boundary)-1) & ~((boundary)-1))
...
/* GCC 4.8+ complains when using our OFFSETOF macro in array length
 * declarations. */
...
/* Reference a function; used to prevent a static function from being optimized
 * out */
...
#define isclr(a, i)                                                            \
    ((((const uint8 *)a)[(i) / NBBY] & (1 << ((i) % NBBY))) == 0)
#endif // endif
#endif /* setbit */
...
#define ETHERP_TO_MACF(ea)                                                     \
    ((struct ether_addr *)(ea))->octet[0],                                     \
...
#define CONST_ETHERP_TO_MACF(ea)                                               \
    ((const struct ether_addr *)(ea))->octet[0],                               \
...
#define ETHER_TO_MACF(ea)                                                      \
    (ea).octet[0], (ea).octet[1], (ea).octet[2], (ea).octet[3], (ea).octet[4], \
        (ea).octet[5]
...
#define MAC2STRDBG(ea)                                                         \
    ((uint8 *)(ea))[0], (((uint8 *)(ea))[4] & 0xf), ((uint8 *)(ea))[5]
#endif /* SIMPLE_MAC_PRINT */

#define MACOUIDBG "%02x:%x:%02x"
#define MACOUI2STRDBG(ea)                                                      \
    ((uint8 *)(ea))[0], ((uint8 *)(ea))[1] & 0xf, ((uint8 *)(ea))[2]

#define MACOUI "%02x:%02x:%02x"
#define MACOUI2STR(ea)                                                         \
    ((uint8 *)(ea))[0], ((uint8 *)(ea))[1], ((uint8 *)(ea))[2]
...
/* 128-bit xor: *dst = *src1 xor *src2. dst1, src1 and src2 may have any
 * alignment */
static INLINE void xor_128bit_block(const uint8 *src1, const uint8 *src2,
                                    uint8 *dst)
{
    if (
#ifdef __i386__
        ...
        ((uint32 *)dst)[0] =
            ((const uint32 *)src1)[0] ^ ((const uint32 *)src2)[0];
        ((uint32 *)dst)[1] =
            ((const uint32 *)src1)[1] ^ ((const uint32 *)src2)[1];
        ((uint32 *)dst)[2] =
            ((const uint32 *)src1)[2] ^ ((const uint32 *)src2)[2];
        ((uint32 *)dst)[3] =
            ((const uint32 *)src1)[3] ^ ((const uint32 *)src2)[3];
    } else {
...
/* format/print */
    ...
    defined(WLMSG_ASSOC)
/* print out the value a field has: fields may have 1-32 bits and may hold any
 * value */
extern int bcm_format_field(const bcm_bit_desc_ex_t *bd, uint32 field,
                            char *buf, int len);
/* print out which bits in flags are set */
extern int bcm_format_flags(const bcm_bit_desc_t *bd, uint32 flags, char *buf,
                            int len);
...
extern uint bcmdumpfields(bcmutl_rdreg_rtn func_ptr, void *arg0, uint arg1,
                          struct fielddesc *str, char *buf, uint32 bufsize);
...
extern uint bcm_mkiovar(const char *name, const char *data, uint datalen,
                        char *buf, uint len);

unsigned int process_nvram_vars(char *varbuf, unsigned int len);
extern bool replace_nvram_variable(char *varbuf, unsigned int buflen,
                                   const char *variable, unsigned int *datalen);

/* trace any object allocation / free, with / without features (flags) set to
 * the object */
...
#define bcm_pkt_validate_chk(obj)                                              \
    do {                                                                       \
        void *pkttag;                                                          \
        bcm_object_trace_chk(obj, 0, 0, __FUNCTION__, __LINE__);               \
        if ((pkttag = PKTTAG(obj))) {                                          \
            bcm_object_trace_chk(obj, 1, DHD_PKTTAG_SN(pkttag), __FUNCTION__,  \
                                 __LINE__);                                    \
        }                                                                      \
    } while (0)
extern void bcm_object_trace_opr(void *obj, uint32 opt, const char *caller,
                                 int line);
 ...
 *
 *    Code Unused memory   Random numbers   Random number    Magic number NVRAM
 * NVRAM byte Count       0xFEEDC0DE                 Size
 *   |<-----Variable---->|<---Variable--->|<-----4 bytes-->|<---4
 * bytes---->|<---V--->|<--4B--->|
...
static INLINE int bcm_count_leading_zeros(uint32 u32arg)
{
...
extern void id16_map_clear(void *id16_map_hndl, uint16 total_ids,
                           uint16 start_val16);
...
static INLINE void dll_init(dll_t *node_p)
{
...
static INLINE dll_t *dll_head_p(dll_t *list_p)
{
    return list_p->next_p;
}

static INLINE dll_t *dll_tail_p(dll_t *list_p)
{
    return (list_p)->prev_p;
}

static INLINE dll_t *dll_next_p(dll_t *node_p)
{
    return (node_p)->next_p;
}

static INLINE dll_t *dll_prev_p(dll_t *node_p)
{
    return (node_p)->prev_p;
}

static INLINE bool dll_empty(dll_t *list_p)
{
    return ((list_p)->next_p == (list_p));
}

static INLINE bool dll_end(dll_t *list_p, dll_t *node_p)
{
    return (list_p == node_p);
}

/* inserts the node new_p "after" the node at_p */
static INLINE void dll_insert(dll_t *new_p, dll_t *at_p)
{
...
static INLINE void dll_append(dll_t *list_p, dll_t *node_p)
{
    dll_insert(node_p, dll_tail_p(list_p));
}

static INLINE void dll_prepend(dll_t *list_p, dll_t *node_p)
{
    dll_insert(node_p, list_p);
}

/* deletes a node from any list that it "may" be in, if at all. */
static INLINE void dll_delete(dll_t *node_p)
{
...
typedef struct _counter_tbl_t {
    ...
    uint32
        prev_log_print; /* Internal use. Timestamp of the previous log print */
    uint log_print_interval; /* Desired interval to print logs in ms */
    uint needed_cnt;         /* How many counters need to be used */
    uint32
        cnt[CNTR_TBL_MAX]; /* Counting entries to increase at desired places */
...
enum dump_dongle_e { DUMP_DONGLE_COREREG = 0, DUMP_DONGLE_D11MEM };

typedef struct {
    uint32
        type; /**< specifies e.g dump of d11 memory, use enum dump_dongle_e  */
...
extern int array_value_mismatch_count(uint8 value, uint8 *array,
                                      int array_size);
...
extern int verify_ordered_array_uint8(uint8 *array, int array_size,
                                      uint8 range_lo, uint8 range_hi);
/* Validate a int16 configuration array that need not be zero-terminated. Assert
 * if invalid. */
extern int verify_ordered_array_int16(int16 *array, int array_size,
                                      int16 range_lo, int16 range_hi);
/* Validate all values in an array are in range */
extern int verify_array_values(uint8 *array, int array_size, int range_lo,
