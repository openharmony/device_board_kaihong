 ...
 *              by the client or malloc'ed by the memory manager. The objects
 * are allocated out of a block of memory and freed back to the block.
 *
 *    heap:     The memory pool allocator uses the heap (malloc/free) for
 * memory. In this case, the pool allocator is just providing statistics and
 * instrumentation on top of the heap, without modifying the heap allocation
 * implementation.
 *
...
/*
 * Opaque type definition for the pool manager handle. This object is used for
 * global memory pool operations such as obtaining a new pool, deleting a pool,
 * iterating and instrumentation/debugging.
 */
struct bcm_mpm_mgr;
typedef struct bcm_mpm_mgr *bcm_mpm_mgr_h;

/*
 * Opaque type definition for an instance of a pool. This handle is used for
 * allocating and freeing memory through the pool, as well as
 * management/instrumentation on this specific pool.
 */
struct bcm_mp_pool;
 ...
 *
 * Parameters:
 *    osh:       INPUT  Operating system handle. Needed for heap memory
 * allocation. max_pools: INPUT Maximum number of mempools supported. mgr:
 * OUTPUT The handle is written with the new pools manager object/handle.
 *
 * Returns:
 *    BCME_OK     Object initialized successfully. May be used.
 *    BCME_NOMEM  Initialization failed due to no memory. Object must not be
 * used.
 */
...
/*
 * bcm_mpm_create_prealloc_pool() - Create a new pool for fixed size objects.
 * The pool uses a contiguous block of pre-alloced memory. The memory block may
 * either be provided by the client or dynamically allocated by the pool
 * manager.
 *
 * Parameters:
 ...
 *                     Must be >= sizeof(void *).
 *    nobj:     INPUT  Maximum number of concurrently existing objects to
 * support memstart  INPUT  Pointer to the memory to use, or NULL to malloc()
 *    memsize   INPUT  Number of bytes referenced from memstart (for error
 * checking). Must be 0 if 'memstart' is NULL. poolname  INPUT  For
 * instrumentation, the name of the pool newp:     OUTPUT The handle for the new
 * pool, if creation is successful
 *
 * Returns:
 ...
 *
 */
int bcm_mpm_create_prealloc_pool(bcm_mpm_mgr_h mgr, unsigned int obj_sz,
                                 int nobj, void *memstart, unsigned int memsize,
                                 const char poolname[BCM_MP_NAMELEN],
                                 bcm_mp_pool_h *newp);

/*
 * bcm_mpm_delete_prealloc_pool() - Delete a memory pool. This should only be
 * called after all memory objects have been freed back to the pool.
 *
 * Parameters:
...
/*
 * bcm_mpm_create_heap_pool() - Create a new pool for fixed size objects. The
 * memory pool allocator uses the heap (malloc/free) for memory. In this case,
 * the pool allocator is just providing statistics and instrumentation on top of
 * the heap, without modifying the heap allocation implementation.
 *
 * Parameters:
 *    mgr:      INPUT  The handle to the pool manager
 *    obj_sz:   INPUT  Size of objects that will be allocated by the new pool
...
/*
 * bcm_mpm_delete_heap_pool() - Delete a memory pool. This should only be called
 * after all memory objects have been freed back to the pool.
 *
 * Parameters:
 *    mgr:     INPUT The handle to the pools manager
 *    pool:    INPUT The handle of the  pool to delete
 ...
 *                          compensate for alignment requirements of the
 * objects. This function provides the padded object size. If clients
 *                          pre-allocate a memory slab for a memory pool, the
 *                          padded object size should be used by the client to
 * allocate the memory slab (in order to provide sufficent space for the maximum
 * number of objects).
 *
 * Parameters:
 *    mgr:            INPUT   The handle to the pools manager.
