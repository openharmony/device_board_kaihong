/*
 * bcmiov.h
 * Common iovar handling/parsing support - batching, parsing, sub-cmd dispatch
 * etc. To be used in firmware and host apps or dhd - reducing code size,
...
typedef int (*bcm_iov_cmd_validate_t)(const bcm_iov_cmd_digest_t *dig,
                                      uint32 actionid, const uint8 *ibuf,
...
typedef int (*bcm_iov_cmd_get_t)(const bcm_iov_cmd_digest_t *dig,
                                 const uint8 *ibuf, size_t ilen, uint8 *obuf,
                                 size_t *olen);
 ...
 * for each individual sub-command.
 */
...
enum {
    ...
    BCM_IOV_CMD_FLAG_STATUS_PRESENT =
        (1 << 0), /* status present at data start - output only */
    BCM_IOV_CMD_FLAG_XTLV_DATA = (1 << 1), /* data is a set of xtlvs */
    BCM_IOV_CMD_FLAG_HDR_IN_LEN =
        (1 << 2), /* length starts at version - non-bacthed only */
 ...
 * the tlv data is extracted and used. Multiple tlv support involves allocation
 * of tlp nodes, except the first, as required,
 */
...
typedef size_t (*bcm_iov_xtlv_get_len_t)(
    const bcm_iov_cmd_digest_t *dig, const bcm_iov_cmd_tlv_info_t *tlv_info);
...
typedef void (*bcm_iov_xtlv_pack_t)(const bcm_iov_cmd_digest_t *dig,
                                    ...
                                    uint8 *out_buf, const uint8 *in_data,
                                    size_t len);
...
int bcm_iov_free_parse_context(bcm_iov_parse_context_t **ctx,
                               bcm_iov_free_t free_fn);
 ...
 */
int bcm_iov_pack_xtlvs(const bcm_iov_cmd_digest_t *dig,
                       bcm_xtlv_opts_t xtlv_opts, uint8 *out_buf,
