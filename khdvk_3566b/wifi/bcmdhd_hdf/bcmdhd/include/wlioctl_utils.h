...
extern int wl_cntbuf_to_xtlv_format(void *ctx, void *cntbuf, int buflen,
                                    uint32 corerev);

extern const char *wl_get_reinit_rc_name(int rc);

/* Get data pointer of wlc layer counters tuple from xtlv formatted counters
 * IOVar buffer. */
#define GET_WLCCNT_FROM_CNTBUF(cntbuf)                                         \
    (const wl_cnt_wlc_t *)bcm_get_data_from_xtlv_buf(                          \
        ((const wl_cnt_info_t *)cntbuf)->data,                                 \
        ((const wl_cnt_info_t *)cntbuf)->datalen, WL_CNT_XTLV_WLC, NULL,       \
        BCM_XTLV_OPTION_ALIGN32)

#define CHK_CNTBUF_DATALEN(cntbuf, ioctl_buflen)                               \
    do {                                                                       \
        if (((wl_cnt_info_t *)cntbuf)->datalen +                               \
                OFFSETOF(wl_cnt_info_t, data) >                                \
            ioctl_buflen)                                                      \
