...
#define BCM_RING_IS_VALID(ring)                                                \
    (((ring) != BCM_RING_NULL) && ((ring)->self == (ring)))
#else /* ! BCM_RING_DEBUG */
#define RING_ASSERT(exp)                                                       \
    do {                                                                       \
    } while (0)
...
static INLINE void bcm_ring_sync_write(bcm_ring_t *peer,
                                       const bcm_ring_t *self);
...
/**
 * bcm_ring_cons_avail - fetch total number of available elements for
 * consumption.
 * @ring: pointer to a ring context
 * @ring_size: size of the ring
 */
