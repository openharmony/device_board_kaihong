...
typedef volatile struct {
    ...
    uint32
        spih_int_edge; /* 0x40 SPI Interrupt Edge Register (0=Level, 1=Edge) */
    uint32
        spih_int_pol; /* 0x44 SPI Interrupt Polarity Register (0=Active Low, */
                      /* 1=Active High) */
    ...
    uint32 spih_disp_sel;   /* 0x6c SPI 4-digit hex display mode select
                               (1=current) */
    ...
    uint32
        spih_xtal_freq; /* 0xC8 External Clock Frequency in units of 10000Hz */
    uint32 spih_clk_count; /* 0xCC External Clock Count Register */
} spih_regs_t;

typedef volatile struct {
    uint32
        cfg_space[0x40]; /* 0x000-0x0FF PCI Configuration Space (Read Only) */
    ...
    uint32 WB_CONF_SPC_BAR; /* 0x180 32 R WISHBONE Configuration Space Base
                               Address */
...
#define SPIH_PLL_NO_CLK                                                        \
    (1 << 1) /* Set to 1 if the PLL's input clock is lost. */
