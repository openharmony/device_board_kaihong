...
#define TRX_GZ_FILES                                                           \
    0x2 /* Contains up to TRX_MAX_OFFSET individual gzip files */
 ...
 * To make sure, other applications like "dhdl" which are yet to be enhanced to
 * support TRXV2 are not broken, new macro and structure defintion take effect
 * only when BCMTRXV2 is defined.
 */
struct trx_header {
    uint32 magic;        /* "HDR0" */
...
#ifndef BCMTRXV2
    uint32 offsets[TRX_MAX_OFFSET]; /* Offsets of partitions from start of
                                       header */
#else
...
#define SIZEOF_TRXHDR_V1                                                       \
    (sizeof(struct trx_header) + (TRX_V1_MAX_OFFSETS - 1) * sizeof(uint32))
#define SIZEOF_TRXHDR_V2                                                       \
    (sizeof(struct trx_header) + (TRX_V2_MAX_OFFSETS - 1) * sizeof(uint32))
