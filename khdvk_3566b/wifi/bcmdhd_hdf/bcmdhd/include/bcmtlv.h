...
#define BCM_TLV_SIZE(_tlv)                                                     \
    ((_tlv) ? (OFFSETOF(bcm_tlv_t, data) + (_tlv)->len) : 0)
...
#define BCM_XTLV_MAX_DATA_SIZE_EX(_opts)                                       \
    ((_opts & BCM_XTLV_OPTION_LENU8) ? 255 : 65535)
...
/* return the next xtlv element, and update buffer len (remaining). Buffer
 * length updated includes padding as specified by options
 */
bcm_xtlv_t *bcm_next_xtlv(const bcm_xtlv_t *elt, int *buf_len,
                          bcm_xtlv_opts_t opts);
...
/* unpack a single xtlv entry, advances buffer and copies data to dst_data on
 * match type and length match must be exact
 */
int bcm_unpack_xtlv_entry(const uint8 **buf, uint16 expected_type,
                          uint16 expected_len, uint8 *dst_data,
                          bcm_xtlv_opts_t opts);
...
typedef int(bcm_xtlv_unpack_cbfn_t)(void *ctx, const uint8 *buf, uint16 type,
                                    uint16 len);
...
/* callback to return next tlv id and len to pack, if there is more tlvs to come
 * and options e.g. alignment
 */
typedef bool (*bcm_pack_xtlv_next_info_cbfn_t)(void *ctx, uint16 *tlv_id,
                                               uint16 *tlv_len);

/* callback to pack the tlv into length validated buffer */
typedef void (*bcm_pack_xtlv_pack_next_cbfn_t)(void *ctx, uint16 tlv_id,
                                               uint16 tlv_len, uint8 *buf);
