...
typedef enum { TPUT_DIR_TX = 0, TPUT_DIR_RX } tput_dir_t;
...
/*
 * Increase the DHD_IOCTL_MAXLEN to 16K for supporting download of NVRAM files
 * of size > 8K. In the existing implementation when NVRAM is to be downloaded
 * via the "vars" DHD IOVAR, the NVRAM is copied to the DHD Driver memory. Later
 * on when "dwnldstate" is invoked with FALSE option, the NVRAM gets copied from
 * the DHD driver to the Dongle memory. The simple way to support this feature
 * without modifying the DHD application, driver logic is to increase the
 * DHD_IOCTL_MAXLEN size. This macro defines the "size" of the buffer in which
 * data is exchanged between the DHD App and DHD driver.
 */
...
typedef struct _dhd_maclist_t {
    ...
    uint16
        bytes_len;  /* Total bytes length of lists, XTLV headers and paddings */
