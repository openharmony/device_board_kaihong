 ...
 * and host addr associated with the rxfrag or shared pool buffer during
 * pktpool_reclaim().
 */
...
/** PCIe SPLITRX related: call back fn extension to populate host address in
 * pool pkt */
typedef int (*pktpool_cb_extn_t)(struct pktpool *pool, void *arg1, void *pkt,
                                 int arg2);
typedef struct {
...
typedef struct pktpool {
    ...
    uint16 max_pkt_bytes; /**< size of pkt buffer in [bytes], excluding
                             lbuf|lbuf_frag */
...
extern int pktpool_init(osl_t *osh, pktpool_t *pktp, int *n_pkts,
                        int max_pkt_bytes, bool istx, uint8 type);
...
extern int pktpool_avail_deregister(pktpool_t *pktp, pktpool_cb_t cb,
                                    void *arg);
...
extern int pktpool_hostaddr_fill_register(pktpool_t *pktp, pktpool_cb_extn_t cb,
                                          void *arg1);
extern int pktpool_rxcplid_fill_register(pktpool_t *pktp, pktpool_cb_extn_t cb,
                                         void *arg);
extern void pktpool_invoke_dmarxfill(pktpool_t *pktp);
extern int pkpool_haddr_avail_register_cb(pktpool_t *pktp, pktpool_cb_t cb,
                                          void *arg);
...
#define pktpool_tot_pkts(pp)                                                   \
    (POOLPTR(pp)->n_pkts) /**< n_pkts = avail + in_use <= max_pkts */
