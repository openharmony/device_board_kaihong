...
|      |   12 | TX status & timestamps   | Present only when pkt timestamp is
enabled
 ---------------------------------------------------------------------------
...
|      |      |                          | [host->firmware]
 ---------------------------------------------------------------------------
|  13  |   3  | (count, handle, prec_bmp)| One time request for packet to a
specific |      |      |                          | MAC destination.
 ---------------------------------------------------------------------------
|  15  |  12  | (pkttag, timestamps)     | Send TX timestamp at reception from
host
 ---------------------------------------------------------------------------
|  16  |  12  | (pkttag, timestamps)     | Send WLAN RX timestamp along with RX
frame
 ---------------------------------------------------------------------------
...
typedef enum {
    ...
    WLFC_CTL_TYPE_CLEAR_SUPPR =
        25, /* free the supression info in the flowring */
...
/* Reset the flags set for the corresponding flowring of the SCB which is
 * de-inited */
/* FLOW_RING_FLAG_LAST_TIM | FLOW_RING_FLAG_INFORM_PKTPEND |
...
#define WLFC_CTL_VALUE_LEN_REQUEST_CREDIT                                      \
    3 /* credit, MAC-handle, prec_bitmap */
#define WLFC_CTL_VALUE_LEN_REQUEST_PACKET                                      \
    3 /* credit, MAC-handle, prec_bitmap */
...
#define WL_TXSTATUS_SET_STATUS(x, status)                                      \
    ((x) = ((x) & ~(WL_TXSTATUS_STATUS_MASK << WL_TXSTATUS_STATUS_SHIFT)) |    \
           (((status)&WL_TXSTATUS_STATUS_MASK) << WL_TXSTATUS_STATUS_SHIFT))
#define WL_TXSTATUS_GET_STATUS(x)                                              \
    (((x) >> WL_TXSTATUS_STATUS_SHIFT) & WL_TXSTATUS_STATUS_MASK)

/**
 * Bit 31 of the 32-bit packet tag is defined as 'generation ID'. It is set by
 * the host to the "current" generation, and by the firmware to the "expected"
 * generation, toggling on suppress. The firmware accepts a packet when the
 * generation matches; on reset (startup) both "current" and "expected" are set
 * to 0.
 */
#define WL_TXSTATUS_GENERATION_MASK 1 /* allow 1 bit */
#define WL_TXSTATUS_GENERATION_SHIFT 31

#define WL_TXSTATUS_SET_GENERATION(x, gen)                                     \
    ((x) = ((x) &                                                              \
            ~(WL_TXSTATUS_GENERATION_MASK << WL_TXSTATUS_GENERATION_SHIFT)) |  \
           (((gen)&WL_TXSTATUS_GENERATION_MASK)                                \
            << WL_TXSTATUS_GENERATION_SHIFT))

#define WL_TXSTATUS_GET_GENERATION(x)                                          \
    (((x) >> WL_TXSTATUS_GENERATION_SHIFT) & WL_TXSTATUS_GENERATION_MASK)

#define WL_TXSTATUS_FLAGS_MASK 0xf /* allow 4 bits only */
#define WL_TXSTATUS_FLAGS_SHIFT 27

#define WL_TXSTATUS_SET_FLAGS(x, flags)                                        \
    ((x) = ((x) & ~(WL_TXSTATUS_FLAGS_MASK << WL_TXSTATUS_FLAGS_SHIFT)) |      \
           (((flags)&WL_TXSTATUS_FLAGS_MASK) << WL_TXSTATUS_FLAGS_SHIFT))
#define WL_TXSTATUS_GET_FLAGS(x)                                               \
    (((x) >> WL_TXSTATUS_FLAGS_SHIFT) & WL_TXSTATUS_FLAGS_MASK)

#define WL_TXSTATUS_FIFO_MASK 0x7 /* allow 3 bits for FIFO ID */
#define WL_TXSTATUS_FIFO_SHIFT 24

#define WL_TXSTATUS_SET_FIFO(x, flags)                                         \
    ((x) = ((x) & ~(WL_TXSTATUS_FIFO_MASK << WL_TXSTATUS_FIFO_SHIFT)) |        \
           (((flags)&WL_TXSTATUS_FIFO_MASK) << WL_TXSTATUS_FIFO_SHIFT))
#define WL_TXSTATUS_GET_FIFO(x)                                                \
    (((x) >> WL_TXSTATUS_FIFO_SHIFT) & WL_TXSTATUS_FIFO_MASK)

#define WL_TXSTATUS_PKTID_MASK 0xffffff /* allow 24 bits */
#define WL_TXSTATUS_SET_PKTID(x, num)                                          \
    ((x) = ((x) & ~WL_TXSTATUS_PKTID_MASK) | (num))
...
#define WL_TXSTATUS_SET_HSLOT(x, hslot)                                        \
    ((x) = ((x) & ~(WL_TXSTATUS_HSLOT_MASK << WL_TXSTATUS_HSLOT_SHIFT)) |      \
           (((hslot)&WL_TXSTATUS_HSLOT_MASK) << WL_TXSTATUS_HSLOT_SHIFT))
#define WL_TXSTATUS_GET_HSLOT(x)                                               \
    (((x) >> WL_TXSTATUS_HSLOT_SHIFT) & WL_TXSTATUS_HSLOT_MASK)

#define WL_TXSTATUS_FREERUNCTR_MASK 0xff /* allow 8 bits */

#define WL_TXSTATUS_SET_FREERUNCTR(x, ctr)                                     \
    ((x) = ((x) & ~(WL_TXSTATUS_FREERUNCTR_MASK)) |                            \
...
#define WL_SEQ_SET_AMSDU(x, val)                                               \
    ((x) = ((x) & ~(WL_SEQ_AMSDU_MASK << WL_SEQ_AMSDU_SHIFT)) |                \
           (((val)&WL_SEQ_AMSDU_MASK)                                          \
            << WL_SEQ_AMSDU_SHIFT)) /**< sets a single AMSDU bit */
...
#define WL_SEQ_SET_VALIDSUPPR(x, val)                                          \
    ((x) = ((x) & ~(WL_SEQ_VALIDSUPPR_MASK << WL_SEQ_VALIDSUPPR_SHIFT)) |      \
           (((val)&WL_SEQ_VALIDSUPPR_MASK) << WL_SEQ_VALIDSUPPR_SHIFT))
#define WL_SEQ_GET_VALIDSUPPR(x)                                               \
    (((x) >> WL_SEQ_VALIDSUPPR_SHIFT) & WL_SEQ_VALIDSUPPR_MASK)

#define WL_SEQ_FROMFW_MASK 0x1 /* allow 1 bit */
#define WL_SEQ_FROMFW_SHIFT 13
#define WL_SEQ_SET_FROMFW(x, val)                                              \
    ((x) = ((x) & ~(WL_SEQ_FROMFW_MASK << WL_SEQ_FROMFW_SHIFT)) |              \
...
/** returns TRUE if packet has been assigned a d11 seq number by the WL firmware
 * layer */
#define GET_WL_HAS_ASSIGNED_SEQ(x)                                             \
    (((x) >> WL_SEQ_FROMFW_SHIFT) & WL_SEQ_FROMFW_MASK)

/**
 * Proptxstatus related.
 *
 * When a packet is suppressed by WL or the D11 core, the packet has to be
 * retried. Assigning a new d11 sequence number for the packet when retrying
 * would cause the peer to be unable to reorder the packets within an AMPDU. So,
 * suppressed packet from bus layer (DHD for SDIO and pciedev for PCIE) is
 * re-using d11 seq number, so FW should not assign a new one.
 */
#define WL_SEQ_FROMDRV_MASK 0x1 /* allow 1 bit */
#define WL_SEQ_FROMDRV_SHIFT 12

/**
 * Proptxstatus, host or fw PCIe layer requests WL layer to reuse d11 seq no.
 * Bit is reset by WL subsystem when it reuses the seq number.
 */
#define WL_SEQ_SET_REUSE(x, val)                                               \
    ((x) = ((x) & ~(WL_SEQ_FROMDRV_MASK << WL_SEQ_FROMDRV_SHIFT)) |            \
...
#define IS_WL_TO_REUSE_SEQ(x)                                                  \
    (((x) >> WL_SEQ_FROMDRV_SHIFT) & WL_SEQ_FROMDRV_MASK)

#define WL_SEQ_NUM_MASK 0xfff /* allow 12 bit */
#define WL_SEQ_NUM_SHIFT 0
/** Proptxstatus, sets d11seq no in pkt tag, related to reuse of d11seq no when
 * retransmitting */
#define WL_SEQ_SET_NUM(x, val)                                                 \
    ((x) = ((x) & ~(WL_SEQ_NUM_MASK << WL_SEQ_NUM_SHIFT)) |                    \
           (((val)&WL_SEQ_NUM_MASK) << WL_SEQ_NUM_SHIFT))
/** Proptxstatus, gets d11seq no from pkt tag, related to reuse of d11seq no
 * when retransmitting */
#define WL_SEQ_GET_NUM(x) (((x) >> WL_SEQ_NUM_SHIFT) & WL_SEQ_NUM_MASK)

#define WL_SEQ_AMSDU_SUPPR_MASK                                                \
...
#define WLFC_PKTFLAG_SET_PKTREQUESTED(x)                                       \
    (x) |= (WLFC_PKTFLAG_PKT_REQUESTED << WL_TXSTATUS_FLAGS_SHIFT)

#define WLFC_PKTFLAG_CLR_PKTREQUESTED(x)                                       \
    (x) &= ~(WLFC_PKTFLAG_PKT_REQUESTED << WL_TXSTATUS_FLAGS_SHIFT)
...
#define WLHOST_REORDERDATA_TOTLEN                                              \
    (WLHOST_REORDERDATA_LEN + 1 + 1) /* +tag +len */
...
/* transaction id data len byte 0: rsvd, byte 1: seqnumber, byte 2-5 will be
 * used for timestampe */
...
#define WLFC_SET_AFQ(x, val)                                                   \
    ((x) = ((x) & ~(1 << WLFC_MODE_AFQ_SHIFT)) |                               \
...
#define WLFC_SET_REUSESEQ(x, val)                                              \
    ((x) = ((x) & ~(1 << WLFC_MODE_REUSESEQ_SHIFT)) |                          \
           (((val)&1) << WLFC_MODE_REUSESEQ_SHIFT))

/** returns TRUE if 'd11 sequence reuse' has been agreed upon between host and
 * dongle */
...
#define WLFC_SET_REORDERSUPP(x, val)                                           \
    ((x) = ((x) & ~(1 << WLFC_MODE_REORDERSUPP_SHIFT)) |                       \
           (((val)&1) << WLFC_MODE_REORDERSUPP_SHIFT))
/** returns TRUE if 'reorder suppress' has been agreed upon between host and
 * dongle */
