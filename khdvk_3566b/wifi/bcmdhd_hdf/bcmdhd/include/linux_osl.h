...
#define ASSERT(exp)                                                            \
    do {                                                                       \
        if (!(exp))                                                            \
            osl_assert(#exp, __FILE__, __LINE__);                              \
    } while (0)
...
static inline void bcm_prefetch_32B(const uint8 *addr, const int cachelines_32B)
{
    ...
    switch (cachelines_32B) {
        case 0x4:
            __asm__ __volatile__("pld\t%a0" ::"p"(addr + 96) : "cc");
        case 0x3:
            __asm__ __volatile__("pld\t%a0" ::"p"(addr + 64) : "cc");
        case 0x2:
            __asm__ __volatile__("pld\t%a0" ::"p"(addr + 32) : "cc");
        case 1:
            __asm__ __volatile__("pld\t%a0" ::"p"(addr + 0) : "cc");
    }
#endif // endif
}
...
extern void osl_dma_flush(osl_t *osh, void *va, uint size, int direction,
                          void *p, hnddma_seg_map_t *txp_dmah);
extern dmaaddr_t osl_dma_map(osl_t *osh, void *va, uint size, int direction,
                             void *p, hnddma_seg_map_t *txp_dmah);
...
#define OSL_WRITE_REG(osh, r, v)                                               \
    (bcmsdh_reg_write(osl_get_bus_handle(osh), (uintptr)(r), sizeof(*(r)), (v)))
#define OSL_READ_REG(osh, r)                                                   \
    (bcmsdh_reg_read(osl_get_bus_handle(osh), (uintptr)(r), sizeof(*(r))))
...
#define PKTBUFSZ                                                               \
    2048 /* largest reasonable packet buffer, driver uses for ethernet MTU */
    ...
    (SELECT_BUS_READ(osh, ( {                                                  \
                         ...
                         switch (sizeof(*(r))) {                               \
                             ...
                             case sizeof(uint32):                              \
                                 ...
                                 break;                                        \
                             case sizeof(uint64):                              \
                                 __osl_v = readq((volatile uint64 *)(r));      \
                                 break;                                        \
                         }                                                     \
                         __osl_v;                                              \
                     }),                                                       \
...
#define W_REG(osh, r, v)                                                       \
    do {                                                                       \
        SELECT_BUS_WRITE(                                                      \
            ...
            switch (sizeof(*(r))) {                                            \
                case sizeof(uint8):                                            \
                    writeb((uint8)(v), (volatile uint8 *)(r));                 \
                    break;                                                     \
                case sizeof(uint16):                                           \
                ...
                case sizeof(uint32):                                           \
                    ...
                    break;                                                     \
                case sizeof(uint64):                                           \
                    writeq((uint64)(v), (volatile uint64 *)(r));               \
                    break;                                                     \
            },                                                                 \
...
#else
#define REG_MAP(pa, size)                                                      \
    ioremap_nocache((unsigned long)(pa), (unsigned long)(size))
#endif
#else
...
#define SECURE_DMA_MAP(osh, va, size, direction, p, dmah, pcma, offset)        \
    osl_sec_dma_map((osh), (va), (size), (direction), (p), (dmah), (pcma),     \
                    (offset))
...
#define SECURE_DMA_MAP_TXMETA(osh, va, size, direction, p, dmah, pcma)         \
    osl_sec_dma_map_txmeta((osh), (va), (size), (direction), (p), (dmah),      \
                           (pcma))
#define SECURE_DMA_UNMAP(osh, pa, size, direction, p, dmah, pcma, offset)      \
    osl_sec_dma_unmap((osh), (pa), (size), (direction), (p), (dmah), (pcma),   \
                      (offset))
#define SECURE_DMA_UNMAP_ALL(osh, pcma) osl_sec_dma_unmap_all((osh), (pcma))
...
extern dma_addr_t osl_sec_dma_map(osl_t *osh, void *va, uint size,
                                  int direction, void *p,
                                  hnddma_seg_map_t *dmah, void *ptr_cma_info,
                                  uint offset);
extern dma_addr_t osl_sec_dma_dd_map(osl_t *osh, void *va, uint size,
                                     int direction, void *p,
...
extern dma_addr_t osl_sec_dma_map_txmeta(osl_t *osh, void *va, uint size,
                                         ...
                                         hnddma_seg_map_t *dmah,
                                         void *ptr_cma_info);
extern void osl_sec_dma_unmap(osl_t *osh, dma_addr_t dma_handle, uint size,
                              int direction, void *p, hnddma_seg_map_t *map,
...
#define PKTLIST_ENQ(x, y)                                                      \
    skb_queue_head((struct sk_buff_head *)(x), (struct sk_buff *)(y))
#define PKTLIST_DEQ(x) skb_dequeue((struct sk_buff_head *)(x))
#define PKTLIST_UNLINK(x, y)                                                   \
    skb_unlink((struct sk_buff *)(y), (struct sk_buff_head *)(x))
...
extern osl_timer_t *osl_timer_init(osl_t *osh, const char *name,
                                   void (*fn)(void *arg), void *arg);
extern void osl_timer_add(osl_t *osh, osl_timer_t *t, uint32 ms, bool periodic);
extern void osl_timer_update(osl_t *osh, osl_timer_t *t, uint32 ms,
                             bool periodic);
...
extern uint32 osl_do_gettimediff(struct osl_timespec *cur_ts,
