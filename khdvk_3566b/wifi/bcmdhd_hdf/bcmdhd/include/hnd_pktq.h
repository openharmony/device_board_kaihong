...
typedef struct {
    ...
    uint32 selfsaved; /**< packets saved,
                             because an older packet from the same queue has
                         been dropped
                       */
    uint32
        full_dropped;  /**< packets dropped,
                              because pktq is full with higher precedence packets
                        */
    uint32 dropped;    /**< packets dropped because pktq per that precedence is
                          full */
    uint32 sacrificed; /**< packets dropped,
                              in order to save one from a queue of a highest
                          priority
                        */
    uint32 busy;  /**< packets droped because of hardware/transmission error */
    uint32 retry; /**< packets re-sent because they were not received */
    uint32
        ps_retry; /**< packets retried again prior to moving power save mode */
    uint32 suppress;   /**< packets which were suppressed and not transmitted */
    uint32 retry_drop; /**< packets finally dropped after retry limit */
    uint32 max_avail; /**< the high-water mark of the queue capacity for packets
                         - goes to zero as queue fills
                       */
    uint32 max_used;  /**< the high-water mark of the queue utilisation for
                         packets -  increases with use ('inverse' of max_avail)
                       */
    ...
    uint32 txrate_succ; /**< running total of phy rate of packets sent
                           successfully */
    uint32
        txrate_main;   /**< running totoal of primary phy rate of all packets */
...
#define PKTQ_COMMON                                                            \
    ...
    uint16 n_pkts_tot; /**< total (cummulative over all precedences) number of \
                          packets */
...
#define PKTQ_PREC_ITER(pq, prec)                                               \
    for (prec = (pq)->num_prec - 1; prec >= 0; prec--)
...
typedef enum {
    PKT_FILTER_NOACTION =
        0,                 /**< restore the pkt to its position in the queue */
 ...
 */
typedef void (*flip_epoch_t)(void *ctx, void *pkt, uint8 *flipEpoch,
                             uint8 *lastEpoch);
...
/** filter a particular precedence in pktq, using the caller supplied filter
 * function */
extern void pktq_pfilter(struct pktq *pq, int prec, pktq_filter_t fn, void *arg,
                         defer_free_pkt_fn_t defer, void *defer_ctx,
                         flush_free_pkt_fn_t flush, void *flush_ctx);
/** filter a simple non-precedence in spktq, using the caller supplied filter
 * function */
...
#else
#define pktqprec_avail_pkts(pq, prec)                                          \
    ((pq)->q[prec].max_pkts - (pq)->q[prec].n_pkts)
...
extern void *pktq_pdeq_with_fn(struct pktq *pq, int prec, ifpkt_cb_t fn,
                               int arg);
...
#define pktqfilter(pq, fltr, fltr_ctx, defer, defer_ctx, flush, flush_ctx)     \
    pktq_pfilter((pq), 0, (fltr), (fltr_ctx), (defer), (defer_ctx), (flush),   \
                 (flush_ctx))
...
#define spktqfilter(spq, fltr, fltr_ctx, defer, defer_ctx, flush, flush_ctx)   \
    spktq_filter((spq), (fltr), (fltr_ctx), (defer), (defer_ctx), (flush),     \
                 (flush_ctx))
