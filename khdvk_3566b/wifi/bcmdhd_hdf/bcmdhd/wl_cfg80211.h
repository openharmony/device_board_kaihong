...
#define WL_ERR_MSG(x, args...)                                                 \
    do {                                                                       \
        if (wl_dbg_level & WL_DBG_ERR) {                                       \
            ...
            DHD_LOG_DUMP_WRITE("[%s] %s: ", dhd_log_dump_get_timestamp(),      \
                               __func__);                                      \
...
#define WL_ERR_EX(args)                                                        \
    do {                                                                       \
        if (wl_dbg_level & WL_DBG_ERR) {                                       \
            printk(KERN_INFO CFG80211_ERROR_TEXT "%s : " x, __func__, ##args); \
            DHD_LOG_DUMP_WRITE_EX("[%s] %s: ", dhd_log_dump_get_timestamp(),   \
                                  __func__);                                   \
...
#define WL_INFORM_MSG(x, args...)                                              \
    do {                                                                       \
        if (wl_dbg_level & WL_DBG_INFO) {                                      \
            printk(KERN_INFO DHD_LOG_PREFIXS "CFG80211-INFO) %s : " x,         \
                   __func__, ##args);                                          \
        }                                                                      \
    } while (0)
...
#define WL_SCAN_MSG(x, args...)                                                \
    do {                                                                       \
        if (wl_dbg_level & WL_DBG_SCAN) {                                      \
            printk(KERN_INFO DHD_LOG_PREFIXS "CFG80211-SCAN) %s :" x,          \
                   __func__, ##args);                                          \
        }                                                                      \
    } while (0)
...
#define WL_TRACE_MSG(x, args...)                                               \
    do {                                                                       \
        if (wl_dbg_level & WL_DBG_TRACE) {                                     \
            printk(KERN_INFO DHD_LOG_PREFIXS "CFG80211-TRACE) %s :" x,         \
                   __func__, ##args);                                          \
        }                                                                      \
    } while (0)
...
#define WL_DBG_MSG(x, args...)                                                 \
    do {                                                                       \
        if (wl_dbg_level & WL_DBG_DBG) {                                       \
            printk(KERN_INFO DHD_LOG_PREFIXS "CFG80211-DEBUG) %s :" x,         \
                   __func__, ##args);                                          \
        }                                                                      \
    } while (0)
...
struct bcm_cfg80211 {
    ...
    struct wireless_dev
        *p2p_wdev;              /* representing cfg cfg80211 device for P2P */
    ...
    spinlock_t
        net_list_sync;      /* to protect scan status (and others if needed) */
...
static inline wl_bss_info_t *next_bss(struct wl_scan_results *list,
                                      wl_bss_info_t *bss)
{
    return bss = bss ? (wl_bss_info_t *)((uintptr)bss + dtoh32(bss->length))
                     : list->bss_info;
}
...
static inline struct net_info *wl_get_netinfo_by_wdev(struct bcm_cfg80211 *cfg,
                                                      struct wireless_dev *wdev)
{
    struct net_info *_net_info, *next, *info = NULL;
    unsigned long int flags;
...
#define wl_escan_set_type(a, b)                                                \
    ((a)->escan_info                                                           \
         .escan_type[((a)->escan_info.cur_sync_id) % SCAN_BUF_CNT] = (b))
static inline wl_scan_results_t *wl_escan_get_buf(struct bcm_cfg80211 *cfg,
                                                  bool aborted)
{
    u8 index;
    if (aborted) {
...
extern s32 wl_cfg80211_get_chanspecs_2g(struct net_device *ndev, void *buf,
                                        s32 buflen);
extern s32 wl_cfg80211_get_chanspecs_5g(struct net_device *ndev, void *buf,
                                        s32 buflen);
...
wl_cfg80211_register_static_if(struct bcm_cfg80211 *cfg, u16 iftype,
                               char *ifname, int static_ifidx);
...
wl_cfg80211_post_static_ifcreate(struct bcm_cfg80211 *cfg,
                                 wl_if_event_info *event, u8 *addr,
                                 s32 iface_type, int static_ifidx);
