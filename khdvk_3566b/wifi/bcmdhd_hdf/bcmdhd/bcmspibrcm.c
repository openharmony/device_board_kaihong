...
/* these are for the older cores... for newer cores we have control for each of
 * them */
...
/* 128bytes buffer is enough to clear data-not-available and program
 * response-delay F0 bits assuming we will not exceed F0 response delay > 100
 * bytes at 48MHz.
 */
...
#define SPISWAP_WD2(x)                                                         \
    (bcmswap16(x & 0xffff)) | (bcmswap16((x & 0xffff0000) >> 16) << 16);
#else
...
static int bcmspi_card_regread_fixedaddr(sdioh_info_t *sd, int func,
                                         uint32 regaddr, int regsize,
                                         uint32 *data);
...
extern sdioh_info_t *sdioh_attach(osl_t *osh, void *bar0, uint irq)
{
    ...
    /* Spi device default is 16bit mode, change to 4 when device is changed to
     * 32bit mode
     */
    sd->wordlen = 0x2;
...
extern uint32 sdioh_get_dstatus(sdioh_info_t *sd)
{
    return sd->card_dstatus;
}

extern void sdioh_chipinfo(sdioh_info_t *sd, uint32 chip, uint32 chiprev)
{
    sd->chip = chip;
    sd->chiprev = chiprev;
}

extern void sdioh_dwordmode(sdioh_info_t *sd, bool set)
{
    uint8 reg = 0;
    int status;

    if ((status = sdioh_request_byte(sd, SDIOH_READ, SPI_FUNC_0,
                                     SPID_STATUS_ENABLE, &reg)) != SUCCESS) {
    ...
    if (set) {
        ...
        sd->dwordmode = TRUE;
        sd->client_block_size[SPI_FUNC_2] =
            0x1000; /* h2spi's limit is 4KB, we support 8KB */
    } else {
        reg &= ~DWORD_PKT_LEN_EN;
        sd->dwordmode = FALSE;
        sd->client_block_size[SPI_FUNC_2] = 0x800;
    }
...
const bcm_iovar_t sdioh_iovars[] = {
    ...
    {"sd_blocksize", IOV_BLOCKSIZE, 0, IOVT_UINT32,
     0}, /* ((fn << 16) | size) */
    ...
    {"spi_errstats", IOV_SPIERRSTATS, 0, IOVT_BUFFER,
        ...
        case IOV_SVAL(IOV_RESP_DELAY_ALL):
            ...
            if (si->resp_delay_all) {
                ...
                if (bcmspi_card_regwrite(si, SPI_FUNC_0, SPID_RESPONSE_DELAY, 1,
                    ...
                    sd_err(
                        ("%s: Unable to set response delay.\n", __FUNCTION__));
                    bcmerror = BCME_SDIO_ERROR;
                    break;
                }
            }

            if (bcmspi_card_regwrite(si, SPI_FUNC_0, SPID_STATUS_ENABLE, 1,
                                     int_val) != SUCCESS) {
                                   ...
                                   uint32 length)
{
    ...
    status =
        bcmspi_card_bytewrite(sd, SDIO_FUNC_1, SBSDIO_FUNC1_SBADDRLOW, &data);
    if (status == SUCCESS) {
        data = (bar0 >> 0x10) & SBSDIO_SBADDRMID_MASK;
    ...
    if (status == SUCCESS) {
        data = (bar0 >> 0x18) & SBSDIO_SBADDRHIGH_MASK;
        status = bcmspi_card_bytewrite(sd, SDIO_FUNC_1, SBSDIO_FUNC1_SBADDRHIGH,
                                       &data);
    } else {
        sd_err(("%s: Unable to set sb-addr-windows\n", __FUNCTION__));
        spi_unlock(sd);
    ...
    for (count = 0; count < length / 0x2; count++) {
        if (bcmspi_card_regread(sd, SDIO_FUNC_1, offset, 0x2, &cis_byte) < 0) {
                                         ...
                                         void *pkt)
{
    ...
    ASSERT(reg_width == 0x4);
    ASSERT(buflen_u < (1 << 0x1E));
    ASSERT(sd->client_block_size[func]);

    sd_data(("%s: %c len %d r_cnt %d t_cnt %d, pkt @0x%p\n", __FUNCTION__,
             rw == SDIOH_READ ? 'R' : 'W', buflen_u, sd->r_cnt, sd->t_cnt,
             pkt));

    /* Break buffer down into blocksize chunks. */
    while (buflen > 0) {
        len = MIN(sd->client_block_size[func], buflen);
        if (bcmspi_card_buf(sd, rw, func, fifo, addr, len, (uint32 *)buffer) !=
            SUCCESS) {
            sd_err(("%s: bcmspi_card_buf %s failed\n", __FUNCTION__,
                    rw == SDIOH_READ ? "Read" : "Write"));
            spi_unlock(sd);
...
/* This function allows write to gspi bus when another rd/wr function is deep
 * down the call stack. Its main aim is to have simpler spi writes rather than
 * recursive writes. e.g. When there is a need to program response delay on the
 * fly after detecting the SPI-func this call will allow to program the response
 * delay.
 */
static int bcmspi_card_byterewrite(sdioh_info_t *sd, int func, uint32 regaddr,
                                   uint8 byte)
{
    uint32 cmd_arg;
    uint32 datalen = 1;
    ...
    /* Set up and issue the SPI command.  MSByte goes out on bus first. Increase
     * datalen according to the wordlen mode(16/32bit) the device is in.
     */
    ASSERT(sd->wordlen == 0x4 || sd->wordlen == 0x2);
    datalen = ROUNDUP(datalen, sd->wordlen);

    /* Start by copying command in the spi-outbuffer */
    if (sd->wordlen == 0x4) { /* 32bit spid */
        *(uint32 *)spi_outbuf2 = SPISWAP_WD4(cmd_arg);
        if (datalen & 0x3) {
            datalen += (0x4 - (datalen & 0x3));
        }
    } else if (sd->wordlen == 0x2) { /* 16bit spid */
...
static int bcmspi_prog_resp_delay(sdioh_info_t *sd, int func, uint8 resp_delay)
{
    ...
    /* Remember function for which to avoid reprogramming resp-delay in next
     * iteration */
    sd->prev_fun = func;
...
/* A resync pattern is a 32bit MOSI line with all zeros. Its a special command
 * in gSPI. It resets the spi-bkplane logic so that all F1 related ping-pong
 * buffer logic is synchronised and all queued resuests are cancelled.
 */
static int bcmspi_resync_f1(sdioh_info_t *sd)
{
    ...
    *(uint32 *)&spi_outbuf2[CMDLEN] = data;
    ...
    /* +4 for cmd, +4 for dstatus */
    spi_sendrecv(sd, spi_outbuf2, spi_inbuf2, datalen + 0x8);
    ...
    if (sd->card_dstatus) {
        sd_trace(
            ("dstatus after resync pattern write = 0x%x\n", sd->card_dstatus));
    }
...
static int bcmspi_update_stats(sdioh_info_t *sd, uint32 cmd_arg)
{
    ...
    if (dstatus & STATUS_DATA_NOT_AVAILABLE) {
        ...
        /* Clear dna bit */
        bcmspi_card_byterewrite(sd, SPI_FUNC_0, SPID_INTR_REG,
                                DATA_UNAVAILABLE);
    }
    ...
    if (dstatus & STATUS_OVERFLOW) {
        ...
        sd_err(
            ("FIFO overflow happened due to current (F1/F2) write command.\n"));
    ...
    if (dstatus & STATUS_F2_INTR) {
        ...
        sd_trace(("Interrupt from F2.  SW should clear corresponding IntStatus "
                  "bits\n"));
    }

    if (dstatus & STATUS_F3_INTR) {
        spierrstats->f3interrupt++;
        sd_err(("Interrupt from F3.  SW should clear corresponding IntStatus "
                "bits\n"));
    }

    if (dstatus & STATUS_HOST_CMD_DATA_ERR) {
        spierrstats->hostcmddataerr++;
        sd_err(("Error in CMD or Host data, detected by CRC/Checksum "
                "(optional)\n"));
    }
    ...
    if (dstatus & STATUS_F2_PKT_AVAILABLE) {
        ...
        sd_trace(("Packet length = %d\n",
                  sd->dwordmode ? ((dstatus & STATUS_F2_PKT_LEN_MASK) >>
                                   (STATUS_F2_PKT_LEN_SHIFT - 0x2))
 ...
 */
static int bcmspi_host_init(sdioh_info_t *sd)
{
...
static int get_client_blocksize(sdioh_info_t *sd)
{
    uint32 regdata[2];
    int status;

    /* Find F1/F2/F3 max packet size */
    if ((status = bcmspi_card_regread(sd, 0, SPID_F1_INFO_REG, 0x8, regdata)) !=
        SUCCESS) {
        return status;
    }
    ...
    sd->client_block_size[1] = (regdata[0] & F1_MAX_PKT_SIZE) >> 0x2;
    sd_trace(("Func1 blocksize = %d\n", sd->client_block_size[1]));
    ASSERT(sd->client_block_size[1] == BLOCK_SIZE_F1);

    sd->client_block_size[0x2] = ((regdata[0] >> 0x10) & F2_MAX_PKT_SIZE) >> 0x2;
    sd_trace(("Func2 blocksize = %d\n", sd->client_block_size[0x2]));
    ASSERT(sd->client_block_size[0x2] == BLOCK_SIZE_F2);

    sd->client_block_size[0x3] = (regdata[1] & F3_MAX_PKT_SIZE) >> 0x2;
    sd_trace(("Func3 blocksize = %d\n", sd->client_block_size[0x3]));
    ASSERT(sd->client_block_size[0x3] == BLOCK_SIZE_F3);
...
static int bcmspi_client_init(sdioh_info_t *sd)
{
    ...
    /* check to see if the response delay needs to be programmed properly */
    {
        ...
        if ((f1_respdelay == 0) || (f1_respdelay == 0xFF)) {
             ...
             */
            sd_err(("older corerev < 4 so use the same resp delay for all "
                    "funcs\n"));
            ...
            sd_err(("new corerev >= 4 so set the resp delay for each of the "
                    "funcs\n"));
            ...
            ret_val = bcmspi_card_regwrite(sd, SPI_FUNC_0, SPID_RESP_DELAY_F0,
                                           1, GSPI_F0_RESP_DELAY);
            if (ret_val != SUCCESS) {
                sd_err(("%s: Unable to set response delay for F0\n",
                        __FUNCTION__));
                return ERROR;
            }
            ret_val = bcmspi_card_regwrite(sd, SPI_FUNC_0, SPID_RESP_DELAY_F1,
                                           1, GSPI_F1_RESP_DELAY);
            if (ret_val != SUCCESS) {
                sd_err(("%s: Unable to set response delay for F1\n",
                        __FUNCTION__));
                return ERROR;
            }
            ret_val = bcmspi_card_regwrite(sd, SPI_FUNC_0, SPID_RESP_DELAY_F2,
                                           1, GSPI_F2_RESP_DELAY);
            if (ret_val != SUCCESS) {
                sd_err(("%s: Unable to set response delay for F2\n",
                        __FUNCTION__));
                return ERROR;
            }
            ret_val = bcmspi_card_regwrite(sd, SPI_FUNC_0, SPID_RESP_DELAY_F3,
                                           1, GSPI_F3_RESP_DELAY);
            if (ret_val != SUCCESS) {
                sd_err(("%s: Unable to set response delay for F2\n",
                        __FUNCTION__));
                return ERROR;
...
static int bcmspi_set_highspeed_mode(sdioh_info_t *sd, bool hsmode)
{
    ...
    if ((status = bcmspi_card_regread(sd, 0, SPID_CONFIG, 0x4, &regdata)) !=
        SUCCESS) {
        return status;
    }
    ...
    if (hsmode == TRUE) {
        ...
        if (regdata & HIGH_SPEED_MODE) {
            ...
            sd_trace(
                ("Writing %08x to device at %08x\n", regdata, SPID_CONFIG));
            if ((status = bcmspi_card_regwrite(sd, 0, SPID_CONFIG, 0x4,
                                               regdata)) != SUCCESS) {
                return status;
            }
        }
...
#define bcmspi_find_curr_mode(sd)                                              \
    {                                                                          \
        sd->wordlen = 2;                                                       \
        status =                                                               \
            bcmspi_card_regread_fixedaddr(sd, 0, SPID_TEST_READ, 4, &regdata); \
        regdata &= 0xff;                                                       \
        if ((regdata == 0xad) || (regdata == 0x5b) || (regdata == 0x5d) ||     \
            (regdata == 0x5a))                                                 \
            break;                                                             \
        sd->wordlen = 4;                                                       \
        status =                                                               \
...
static bool bcmspi_host_device_init_adapt(sdioh_info_t *sd)
{
    ...
    if ((sd->dwordmode) || (sd->wordlen == 0x4)) {
        bcmspi_card_regwrite(sd, 0, SPID_RESET_BP, 1,
                             RESET_ON_WLAN_BP_RESET | RESET_SPI);
        ...
        sd_trace(("dwordmode = %d, wordlen = %d, resp_delay_all = %d\n",
                  sd->dwordmode, sd->wordlen, sd->resp_delay_all));
    ...
    if (sd->wordlen == 0x4) {
        if ((status = bcmspi_card_regread(sd, 0, SPID_TEST_READ, 0x4,
                                          &regdata)) != SUCCESS) {
            return FALSE;
        }
...
#define CTRL_REG_DEFAULT 0x00010430 /* according to the host m/c */
        ...
        bcmspi_card_regwrite(sd, 0, SPID_CONFIG, 0x4, wrregdata);
#endif /* HSMODE */
        ...
        sd_trace(("(we are still in 16bit mode) 32bit Write LE reg-ctrl-data = "
                  "0x%x\n",
                  wrregdata));
        ...
        if (regdata == TEST_RO_DATA_32BIT_LE) {
            ...
            sd_err(("Stale spid reg values read as it was kept powered. Value "
                    "read ="
                    "0x%x\n",
                    regdata));
            return FALSE;
...
static bool bcmspi_test_card(sdioh_info_t *sd)
{
    ...
    if ((status = bcmspi_card_regread(sd, 0, SPID_TEST_READ, 0x4, &regdata)) !=
        SUCCESS) {
        return FALSE;
    }
...
#define RW_PATTERN2 0x4B5B6B7B
    ...
    regdata = RW_PATTERN1;
    if ((status = bcmspi_card_regwrite(sd, 0, SPID_TEST_RW, 0x4, regdata)) !=
        SUCCESS) {
        return FALSE;
    }
    regdata = 0;
    if ((status = bcmspi_card_regread(sd, 0, SPID_TEST_RW, 0x4, &regdata)) !=
        SUCCESS) {
        return FALSE;
    }
    if (regdata != RW_PATTERN1) {
        sd_err(
            ("Write-Read spid failed. Value wrote = 0x%x, Value read = 0x%x\n",
             RW_PATTERN1, regdata));
        return FALSE;
    } else {
                               ...
                               int regsize, uint32 *data)
{
    ...
    if (func == 0x2) {
        sd_trace(("Reg access on F2 will generate error indication in dstatus "
                  "bits.\n"));
    }
    ...
    if ((status = bcmspi_cmd_issue(sd, sd->sd_use_dma, cmd_arg, data,
                                   regsize)) != SUCCESS) {
        return status;
    }
...
void bcmspi_cmd_getdstatus(sdioh_info_t *sd, uint32 *dstatus_buffer)
{
                            ...
                            uint32 *data, uint32 datalen)
{
    ...
    if (GFIELD(cmd_arg, SPI_RW_FLAG) == 1) {
        /* We send len field of hw-header always a mod16 size, both from host
         * and dongle */
        if (datalen != 0) {
            for (i = 0; i < datalen / 0x4; i++) {
                if (sd->wordlen == 0x4) { /* 32bit spid */
                    *(uint32 *)&spi_outbuf[i * 0x4 + CMDLEN] =
                        SPISWAP_WD4(data[i]);
                } else if (sd->wordlen == 0x2) { /* 16bit spid */
                    *(uint32 *)&spi_outbuf[i * 0x4 + CMDLEN] =
                        SPISWAP_WD2(data[i]);
    ...
    hostlen = datalen + 0x8 + resp_delay;
    hostlen += dstatus_idx;
#ifdef BCMSPI_ANDROID
    if (hostlen % 0x4) {
    ...
    if (datalen != 0) {
        if (GFIELD(cmd_arg, SPI_RW_FLAG) == 0) { /* if read cmd */
            for (j = 0; j < datalen / 0x4; j++) {
                if (sd->wordlen == 0x4) { /* 32bit spid */
                    data[j] = SPISWAP_WD4(
                        *(uint32 *)&spi_inbuf[j * 0x4 + CMDLEN + resp_delay]);
                } else if (sd->wordlen == 0x2) { /* 16bit spid */
                    data[j] = SPISWAP_WD2(
                        *(uint32 *)&spi_inbuf[j * 0x4 + CMDLEN + resp_delay]);
                }
                           ...
                           uint32 addr, int nbytes, uint32 *data)
{
        ...
        if (write) {
            ...
            if ((dstatus & STATUS_F2_RX_READY) == 0) {
                ...
                while (retries-- && !enable) {
                    OSL_DELAY(WAIT_F2RXFIFORDY_DELAY * 0x3E8);
                    bcmspi_card_regread(sd, SPI_FUNC_0, SPID_STATUS_REG, 0x4,
                                        &dstatus);
    ...
    if ((func == 0x2) && (fifo == 1)) {
        sd_data(
            ("%s: %s func %d, %s, addr 0x%x, len %d bytes, r_cnt %d t_cnt %d\n",
             __FUNCTION__, write ? "Wr" : "Rd", func, "INCR", addr, nbytes,
             sd->r_cnt, sd->t_cnt));
    }

    sd_trace(("%s cmd_arg = 0x%x\n", __FUNCTION__, cmd_arg));
    sd_data(("%s: %s func %d, %s, addr 0x%x, len %d bytes, r_cnt %d t_cnt %d\n",
             __FUNCTION__, write ? "Wd" : "Rd", func, "INCR", addr, nbytes,
             sd->r_cnt, sd->t_cnt));
    ...
    if ((func == 0x2) && (rw == SDIOH_WRITE) && (sd->dwordmode == FALSE)) {
        ASSERT((uint16)sd->data_xfer_count == (uint16)(*data & 0xffff));
        ASSERT((uint16)sd->data_xfer_count ==
               (uint16)(~((*data & 0xffff0000) >> 0x10)));
    }
...
bool sdioh_gpioin(sdioh_info_t *sd, uint32 gpio)
{
    return FALSE;
}
