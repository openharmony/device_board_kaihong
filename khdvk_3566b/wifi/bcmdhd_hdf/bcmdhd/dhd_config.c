...
#define CONFIG_ERROR(x, args...)                                               \
    do {                                                                       \
        if (config_msg_level & CONFIG_ERROR_LEVEL) {                           \
            printk(KERN_ERR DHD_LOG_PREFIXS "CONFIG-ERROR) %s : " x, __func__, \
                   ##args);                                                    \
        }                                                                      \
    } while (0)
#define CONFIG_TRACE(x, args...)                                               \
    do {                                                                       \
        if (config_msg_level & CONFIG_TRACE_LEVEL) {                           \
            printk(KERN_INFO DHD_LOG_PREFIXS "CONFIG-TRACE) %s : " x,          \
                   __func__, ##args);                                          \
        }                                                                      \
    } while (0)
...
void dhd_conf_set_hw_oob_intr(bcmsdh_info_t *sdh, struct si_pub *sih)
{
    ...
    if (CHIPID(sih->chip) == BCM43362_CHIP_ID) {
        ...
        gpiocontrol = bcmsdh_reg_read(sdh, addr, 0x4);
        gpiocontrol |= 0x2;
        bcmsdh_reg_write(sdh, addr, 0x4, gpiocontrol);
...
void dhd_conf_get_otp(dhd_pub_t *dhd, bcmsdh_info_t *sdh, si_t *sih)
{
    ...
    if (!memcmp(&ether_null, &dhd->conf->otp_mac, ETHER_ADDR_LEN)) {
        ptr = cis;
        /* Special OTP */
        if (bcmsdh_reg_read(sdh, SI_ENUM_BASE(sih), 0x4) == 0x16044330) {
            for (i = 0; i < SBSDIO_CIS_SIZE_LIMIT; i++) {
                if (!memcmp(mac_header, ptr, 0x3)) {
                    memcpy(&dhd->conf->otp_mac, ptr + 0x3, ETHER_ADDR_LEN);
                    break;
                }
                ptr++;
            }
...
void dhd_conf_set_fw_name_by_mac(dhd_pub_t *dhd, char *fw_path)
{
    ...
    oui = (mac[0] << 0x10) | (mac[1] << 0x8) | (mac[0x2]);
    nic = (mac[0x3] << 0x10) | (mac[0x4] << 0x8) | (mac[0x5]);
    ...
    for (i = 0; i < fw_num; i++) {
        ...
        for (j = 0; j < mac_num; j++) {
            if (oui == mac_range[j].oui) {
                if (nic >= mac_range[j].nic_start &&
                    nic <= mac_range[j].nic_end) {
                                     ...
                                     uint32 cis_start_addr, uint32 cis_max_cnt)
{
    ...
    for (bp_idx = 0; bp_idx < ARRAYSIZE(boardtype_backplane_addr); bp_idx++) {
        ...
        if (si_backplane_access(
                sih, boardtype_backplane_addr[bp_idx], sizeof(int),
        ...
        if (si_backplane_access(sih, boardtype_backplane_addr[bp_idx],
                                sizeof(int), &boardtype_backplane_data[bp_idx],
                                FALSE) != BCME_OK) {
            CONFIG_ERROR("invalid size/addr combination\n");
            return BCME_ERROR;
        }
...
int dhd_conf_get_otp(dhd_pub_t *dhd, si_t *sih)
{
    ...
    for (i = 0; i < sizeof(chip_cisaddr_map) / sizeof(chip_cisaddr_map[0]);
         i++) {
...
const module_name_map_t *dhd_conf_match_module(dhd_pub_t *dhd)
{
    ...
    for (i = 0; i < sizeof(module_name_map) / sizeof(module_name_map[0]); i++) {
        ...
        if (row->devid == devid && row->chip == chip &&
            row->chiprev == chiprev &&
    ...
    for (i = 0; i < sizeof(module_name_map) / sizeof(module_name_map[0]); i++) {
        const module_name_map_t *row = &module_name_map[i];
        if (row->devid == devid && row->chip == chip &&
            row->chiprev == chiprev && row->svid == svid && row->ssid == ssid) {
            return row;
        }
    }
#endif
...
void dhd_conf_tput_monitor(dhd_pub_t *dhd)
{
    ...
    if (conf->tput_monitor_ms && conf->data_drop_mode >= FW_DROP) {
        if (conf->tput_ts.tv_sec == 0 && conf->tput_ts.tv_nsec == 0) {
            ...
            diff_ms = osl_do_gettimediff(&cur_ts, &conf->tput_ts) / 0x3E8;
            if (diff_ms >= conf->tput_monitor_ms) {
                diff_bytes = dhd->dstats.tx_bytes - conf->last_tx;
                tput_tx =
                    (int32)((diff_bytes / 0x400 / 0x400) * 0x8) * 0x3E8 / diff_ms;
                if (tput_tx == 0) {
                    tput_tx =
                        (int32)(diff_bytes * 0x8 / 0x400 / 0x400) * 0x3E8 / diff_ms;
                    tput_tx_kb =
                        (int32)(diff_bytes * 0x8 * 0x3E8 / 0x400) / diff_ms;
                    tput_tx_kb = tput_tx_kb % 0x3E8;
                }
                ...
                CONFIG_TRACE("xmit=%3d.%d%d%d Mbps, tx=%3d.%d%d%d Mbps, "
                             "rx=%3d.%d%d%d Mbps\n",
                             tput_net, (tput_net_kb / 100) % 10,
                             (tput_net_kb / 10) % 10, (tput_net_kb) % 10,
                             tput_tx, (tput_tx_kb / 100) % 10,
                             (tput_tx_kb / 10) % 10, (tput_tx_kb) % 10, tput_rx,
                             (tput_rx_kb / 100) % 10, (tput_rx_kb / 10) % 10,
                             (tput_rx_kb) % 10);
            }
        }
    }
...
void dhd_conf_set_tput_patch(dhd_pub_t *dhd)
{
    ...
    if (conf->tput_patch) {
        ...
        conf->rps_cpus = TRUE;
#endif /* SET_RPS_CPUS */
        conf->orphan_move = 0x3;
        conf->flow_ring_queue_threshold = 0x800;
#endif /* BCMPCIE */
#ifdef DHDTCPACK_SUPPRESS
        conf->tcpack_sup_ratio = 0xF;
        conf->tcpack_sup_delay = 0xA;
#endif /* DHDTCPACK_SUPPRESS */
    } else {
        conf->mtu = 0;
                          ...
                          int val, int def, bool down)
{
    ...
    if (val >= def) {
        ...
        if (cmd == WLC_SET_VAR) {
            ...
            bcm_mkiovar(name, (char *)&val, sizeof(val), iovbuf,
                        sizeof(iovbuf));
            ...
            if ((ret = dhd_wl_ioctl_cmd(dhd, cmd, &val, sizeof(val), TRUE, 0)) <
                0) {
                CONFIG_ERROR("%s setting failed %d\n", name, ret);
            }
        }
                              ...
                              char *buf, int len)
{
    ...
    if (cmd == WLC_GET_VAR) {
        if (bcm_mkiovar(name, NULL, 0, iovbuf, sizeof(iovbuf))) {
            ret = dhd_wl_ioctl_cmd(dhd, cmd, iovbuf, sizeof(iovbuf), FALSE,
                                   ifidx);
            if (!ret) {
                memcpy(buf, iovbuf, len);
            } else {
...
int dhd_conf_scan_mac(dhd_pub_t *dhd, char *cmd, char *buf)
{
    ...
    if (!strcmp(sub_cmd, "enable")) {
        ...
        if (iovar_len > 0) {
            ret =
                dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, iovar_len, TRUE, 0);
        } else {
            ret = BCME_BUFTOOSHORT;
        }
...
static bool dhd_conf_set_wl_cmd(dhd_pub_t *dhd, char *data, bool down)
{
    ...
    while (pick_tmp && (pick_tmp2 = bcmstrtok(&pick_tmp, ",", 0)) != NULL) {
        ...
        ret = iovar_tpl_parse(iovar_tpl_list, ARRAY_SIZE(iovar_tpl_list), dhd,
                              cmd, name, pch);
        if (ret) {
...
int dhd_conf_map_country_list(dhd_pub_t *dhd, wl_country_t *cspec)
{
    ...
    while (country != NULL) {
        if (!strncmp("**", country->cspec.country_abbrev, 0x2)) {
            if (!strncmp("**", country->cspec.ccode, 0x2)) {
                cspec->rev = 0;
                bcmerror = 0;
                break;
            }
            ...
            break;
                            ...
                            country->cspec.country_abbrev, 0x2)) {
            memcpy(cspec->ccode, country->cspec.ccode, WLC_CNTRY_BUF_SZ);
            cspec->rev = country->cspec.rev;
            bcmerror = 0;
            break;
...
int dhd_conf_set_country(dhd_pub_t *dhd, wl_country_t *cspec)
{
    ...
    bcmerror =
        dhd_conf_set_bufiovar(dhd, 0, WLC_SET_VAR, "country", (char *)cspec,
...
int dhd_conf_fix_country(dhd_pub_t *dhd)
{
    ...
    if ((bcmerror =
             dhd_wl_ioctl_cmd(dhd, WLC_GET_VALID_CHANNELS, valid_chan_list,
                         ...
                         edcf_acparam_t *acparam_cur, int aci)
{
     ...
     */
    if (mode == 0) {
        dhd_conf_set_bufiovar(dhd, ifidx, WLC_SET_VAR, "wme_ac_sta",
                              (char *)acp, sizeof(edcf_acparam_t), FALSE);
    } else {
...
#define MACS "%02x%02x%02x%02x%02x%02x"
    ...
    if (dhd->conf->magic_pkt_filter_add) {
        ...
        for (j = 0; j < 0x10; j++) {
            strncat(dhd->conf->magic_pkt_filter_add, str, 0xC);
        }
    ...
    if (dhd->conf->insuspend & (NO_TXCTL_IN_SUSPEND | WOWL_IN_SUSPEND)) {
        ...
        if (dhd->hostsleep && (!*hostsleep_set || *hostsleep_val)) {
            ...
            *ret = BCME_EPERM;
            ...
            if (dhd->hostsleep == 0x2) {
                *ret = 0;
                goto exit;
            }
        } else if (dhd->hostsleep == 0x2 && !*hostsleep_val) {
            CONFIG_TRACE("hostsleep %d => %d\n", dhd->hostsleep,
                         *hostsleep_val);
            dhd->hostsleep = *hostsleep_val;
            if (dhd->conf->insuspend & NO_TXDATA_IN_SUSPEND) {
                                 ...
                                 char *data)
{
    ...
    if (data) {
        ...
        sscanf(data, "%d %d %d %d %s %s", &id, &type, &polarity, &offset,
               mask_tmp, pattern);
        masksize = strlen(mask_tmp) - 0x2;
        CONFIG_TRACE("0 mask_tmp=%s, masksize=%d\n", mask_tmp, masksize);

        // add pading
        if (masksize % 0x10) {
            pad_len = (0x10 - masksize % 0x10);
        }
        for (i = 0; i < pad_len; i++) {
            strcat(mask_tmp, "0");
        }
        ...
        j = 0;
        pmask_tmp = &mask_tmp[0x2];
        for (i = 0; i < masksize / 0x2; i++) {
            if (strncmp(&pmask_tmp[i * 0x2], "00", 0x2)) {
                pmask_tmp[j] = '1';
            } else {
                pmask_tmp[j] = '0';
            }
        ...
        pmask_tmp[j] = '\0';
        masksize = masksize / 0x2;
        CONFIG_TRACE("2 mask_tmp=%s, masksize=%d\n", mask_tmp, masksize);

        // reorder per 8bits
        pmask_tmp = &mask_tmp[0x2];
        for (i = 0; i < masksize / 0x8; i++) {
            char c;
            for (j = 0; j < 0x4; j++) {
                c = pmask_tmp[i * 0x8 + j];
                pmask_tmp[i * 0x8 + j] = pmask_tmp[(i + 1) * 0x8 - j - 1];
                pmask_tmp[(i + 1) * 0x8 - j - 1] = c;
            }
        }
        ...
        patternsize = (strlen(pattern) - 0x2) / 0x2;
    ...
    if (period >= 0) {
        ...
        if (packet && strlen(packet)) {
            ...
            if (!ret) {
                ...
                memcpy(pdata + 0x6, iovar_buf, ETHER_ADDR_LEN);
            }
        }
...
void dhd_conf_set_garp(dhd_pub_t *dhd, int ifidx, uint32 ipa, bool enable)
{
    ...
    if (enable) {
        ...
        // Sender IP Addr.
        len +=
            snprintf(packet + len, total_len, "%02x%02x%02x%02x", ipa & 0xff,
                     (ipa >> 0x8) & 0xff, (ipa >> 0x10) & 0xff, (ipa >> 0x18) & 0xff);
    ...
    if (suspend) {
        ...
        if (memcmp(&ether_null, &conf->bssid_insuspend, ETHER_ADDR_LEN)) {
            ...
            dhd_wl_ioctl_cmd(dhd, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN, FALSE,
                             ifidx);
            if (memcmp(&ether_null, &bssid, ETHER_ADDR_LEN)) {
                dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_PM, "WLC_SET_PM", 0,
                                      0, FALSE);
                ...
                dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_PM, "WLC_SET_PM", pm,
                                      0, FALSE);
            } else {
...
static void dhd_conf_set_suspend_event(dhd_pub_t *dhd, int suspend)
{
    ...
    if (suspend) {
        ...
        dhd_conf_set_bufiovar(dhd, 0, WLC_SET_VAR, "event_msgs",
                              suspend_eventmask, sizeof(suspend_eventmask),
                              FALSE);
    } else {
        dhd_conf_set_bufiovar(dhd, 0, WLC_SET_VAR, "event_msgs",
                              conf->resume_eventmask,
    ...
    if (suspend) {
        ...
        if ((insuspend & WOWL_IN_SUSPEND) && dhd_master_mode) {
            ...
            for (i = 0; i < conf->pkt_filter_add.count; i++) {
                dhd_conf_wowl_pattern(dhd, ifidx, TRUE,
                                      conf->pkt_filter_add.filter[i]);
            }
            dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_VAR, "wowl", conf->wowl,
                                  0, FALSE);
            dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_VAR, "wowl_activate", 1,
                                  0, FALSE);
            dhd_conf_wowl_wakeind(dhd, ifidx, TRUE);
        }
#endif
    } else {
        dhd_conf_get_iovar(dhd, 0, WLC_GET_PM, "WLC_GET_PM", (char *)&pm,
                           sizeof(pm));
        ...
        if (insuspend & WOWL_IN_SUSPEND) {
            ...
            dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_VAR, "wowl_activate", 0,
                                  0, FALSE);
            dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_VAR, "wowl", 0, 0, FALSE);
            dhd_conf_wowl_pattern(dhd, ifidx, FALSE, "clr");
        }
#endif
        dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_VAR, "bcn_li_dtim", 0, 0,
                              FALSE);
        dhd_conf_set_intiovar(dhd, ifidx, WLC_SET_VAR, "roam_off",
...
static int dhd_conf_suspend_resume_ap(dhd_pub_t *dhd, int ifidx, int suspend)
{
    ...
    if (suspend) {
        if (insuspend & AP_DOWN_IN_SUSPEND) {
            dhd_conf_set_intiovar(dhd, ifidx, WLC_DOWN, "WLC_DOWN", 1, 0,
                                  FALSE);
        }
    } else {
        if (insuspend & AP_DOWN_IN_SUSPEND) {
...
static int dhd_conf_suspend_resume_bus(dhd_pub_t *dhd, int suspend)
{
    ...
    if (suspend) {
        if (insuspend & (WOWL_IN_SUSPEND | NO_TXCTL_IN_SUSPEND)) {
            ...
            hostsleep = 1;
#endif
            dhd_conf_set_intiovar(dhd, 0, WLC_SET_VAR, "hostsleep", hostsleep,
                                  0, FALSE);
#ifdef BCMSDIO
        ...
        if (insuspend & (WOWL_IN_SUSPEND | NO_TXCTL_IN_SUSPEND)) {
            dhd_conf_set_intiovar(dhd, 0, WLC_SET_VAR, "hostsleep", 0, 0,
                                  FALSE);
        }
    }
...
int dhd_conf_set_suspend_resume(dhd_pub_t *dhd, int suspend)
{
    ...
    if (suspend) {
        ...
        if (insuspend & ROAM_OFFLOAD_IN_SUSPEND) {
            dhd_conf_enable_roam_offload(dhd, 0x2);
        }
#ifdef SUSPEND_EVENT
...
void dhd_conf_read_wme_ac_value(wme_param_t *wme, char *pick, int ac_val)
{
    ...
    if (pch) {
        wme->aifsn[ac_val] =
            (int)simple_strtol(pch + strlen("aifsn "), NULL, 0);
    ...
    if (pch) {
        wme->ecwmin[ac_val] =
            (int)simple_strtol(pch + strlen("ecwmin "), NULL, 0);
    ...
    if (pch) {
        wme->ecwmax[ac_val] =
            (int)simple_strtol(pch + strlen("ecwmax "), NULL, 0);
                                 ...
                                 uint len_param)
{
    ...
    if (!strncmp("force_wme_ac=", full_param, len_param)) {
        conf->force_wme_ac = (int)simple_strtol(data, NULL, 0xA);
    ...
    if (!strncmp("fw_by_mac=", full_param, len_param)) {
        ...
        for (i = 0; i < conf->fw_by_mac.count; i++) {
            ...
            CONFIG_MSG("name=%s, mac_count=%d\n", mac_list[i].name,
                       mac_list[i].count);
            if (!(mac_range =
                      kmalloc(sizeof(wl_mac_range_t) * mac_list[i].count,
                              GFP_KERNEL))) {
                mac_list[i].count = 0;
                CONFIG_ERROR("kmalloc failed\n");
                break;
...
bool dhd_conf_read_country(dhd_pub_t *dhd, char *full_param, uint len_param)
{
    ...
    if (!strncmp("ccode=", full_param, len_param)) {
        ...
        conf->cspec.rev = (int32)simple_strtol(data, NULL, 0xA);
        CONFIG_MSG("regrev = %d\n", conf->cspec.rev);
        ...
        for (i = 0; i < CONFIG_COUNTRY_LIST_SIZE; i++) {
            ...
            memcpy(country->cspec.country_abbrev, pch, 0x2);
            pch = bcmstrtok(&pick_tmp2, "/", 0);
            if (!pch) {
                kfree(country);
                break;
            }
            memcpy(country->cspec.ccode, pch, 0x2);
            pch = bcmstrtok(&pick_tmp2, "/", 0);
            if (!pch) {
                kfree(country);
                break;
            }
            country->cspec.rev = (int32)simple_strtol(pch, NULL, 0xA);
            count++;
...
bool dhd_conf_read_pkt_filter(dhd_pub_t *dhd, char *full_param, uint len_param)
{
    ...
    if (!strncmp("dhd_master_mode=", full_param, len_param)) {
        ...
        i = 0;
        while (pch != NULL && i < DHD_CONF_FILTER_MAX) {
            conf->pkt_filter_del.id[i] = (uint32)simple_strtol(pch, NULL, 0xA);
            pch = bcmstrtok(&pick_tmp, " ,.-", 0);
            i++;
        }
...
#ifdef IDHCP
bool dhd_conf_read_dhcp_params(dhd_pub_t *dhd, char *full_param, uint len_param)
{
    struct dhd_conf *conf = dhd->conf;
    char *data = full_param + len_param;
    ...
    if (!strncmp("dhcpc_enable=", full_param, len_param)) {
        conf->dhcpc_enable = (int)simple_strtol(data, NULL, 0xA);
        CONFIG_MSG("dhcpc_enable = %d\n", conf->dhcpc_enable);
    } else if (!strncmp("dhcpd_enable=", full_param, len_param)) {
        conf->dhcpd_enable = (int)simple_strtol(data, NULL, 0xA);
    ...
    if (!strncmp("dhd_doflow=", full_param, len_param)) {
        ...
        if (conf->txglom_ext) {
            if ((conf->chip == BCM43362_CHIP_ID) ||
                (conf->chip == BCM4330_CHIP_ID)) {
                conf->txglom_bucket_size = 0x690;
            } else if (conf->chip == BCM43340_CHIP_ID ||
                       conf->chip == BCM43341_CHIP_ID ||
                       conf->chip == BCM4334_CHIP_ID ||
                       conf->chip == BCM4324_CHIP_ID) {
                conf->txglom_bucket_size = 0x694;
            }
        }
        ...
        conf->deferred_tx_len = (int)simple_strtol(data, NULL, 0xA);
        ...
        conf->tx_max_offset = (int)simple_strtol(data, NULL, 0xA);
...
bool dhd_conf_read_pcie_params(dhd_pub_t *dhd, char *full_param, uint len_param)
{
    ...
    if (!strncmp("bus:deepsleep_disable=", full_param, len_param)) {
        ...
        conf->flow_ring_queue_threshold = (int)simple_strtol(data, NULL, 0xA);
...
bool dhd_conf_read_pm_params(dhd_pub_t *dhd, char *full_param, uint len_param)
{
    ...
    if (!strncmp("deepsleep=", full_param, len_param)) {
        ...
        conf->pm = (int)simple_strtol(data, NULL, 0xA);
        CONFIG_MSG("PM = %d\n", conf->pm);
    } else if (!strncmp("pm_in_suspend=", full_param, len_param)) {
        conf->pm_in_suspend = (int)simple_strtol(data, NULL, 0xA);
        ...
        conf->suspend_bcn_li_dtim = (int)simple_strtol(data, NULL, 0xA);
...
int bcm_str2hex(const char *p, char *ea, int size)
{
    ...
    for (;;) {
        ea[i++] = (char)bcm_strtoul(p, &ep, 0x10);
        p = ep;
        if (!*p++ || i == size) {
            break;
        }
...
bool dhd_conf_read_others(dhd_pub_t *dhd, char *full_param, uint len_param)
{
    ...
    if (!strncmp("dhd_poll=", full_param, len_param)) {
        ...
        dhd_watchdog_ms = (int)simple_strtol(data, NULL, 0xA);
        ...
        while (pch != NULL && i < WL_NUMCHANNELS) {
            conf->channels.channel[i] = (uint32)simple_strtol(pch, NULL, 0xA);
            pch = bcmstrtok(&pick_tmp, " ,.-", 0);
            i++;
        }
        ...
        conf->keep_alive_period = (uint)simple_strtol(data, NULL, 0xA);
        ...
        if (config_msg_level & CONFIG_TRACE_LEVEL) {
            ...
            for (i = 0; i < sizeof(macpad); i++) {
                printk(KERN_CONT "0x%02x, ",
                       conf->hw_ether[ETHER_ADDR_LEN + i]);
                if ((i + 1) % 0x8 == 0) {
                    printk(KERN_CONT "\n");
                }
            }
...
void dhd_conf_postinit_ioctls(dhd_pub_t *dhd)
{
    ...
    dhd_conf_set_intiovar(dhd, 0, WLC_SET_BAND, "WLC_SET_BAND", conf->band, 0,
                          FALSE);
    dhd_conf_set_intiovar(dhd, 0, WLC_SET_VAR, "bcn_timeout", conf->bcn_timeout,
                          0, FALSE);
...
#ifdef IDHCP
    dhd_conf_set_intiovar(dhd, 0, WLC_SET_VAR, "dhcpc_enable",
                          conf->dhcpc_enable, 0, FALSE);
    ...
    if (conf->chip == BCM4354_CHIP_ID || conf->chip == BCM4356_CHIP_ID ||
        ...
        conf->chip == BCM43569_CHIP_ID || conf->chip == BCM43751_CHIP_ID ||
...
int dhd_conf_preinit(dhd_pub_t *dhd)
{
    ...
    if (conf->chip == BCM43362_CHIP_ID || conf->chip == BCM4330_CHIP_ID) {
        ...
        conf->cspec.rev = 0;
               ...
               conf->chip == BCM4359_CHIP_ID || conf->chip == BCM4375_CHIP_ID) {
    ...
    conf->roam_off = 1;
    conf->roam_off_suspend = 1;
    conf->roam_trigger[0] = -0x41;
    conf->roam_trigger[1] = WLC_BAND_ALL;
    conf->roam_scan_period[0] = 0xA;
    conf->roam_scan_period[1] = WLC_BAND_ALL;
    conf->roam_delta[0] = 0xA;
    conf->roam_delta[1] = WLC_BAND_ALL;
    conf->fullroamperiod = 0x14;
    conf->keep_alive_period = 0x7530;
#ifdef ARP_OFFLOAD_SUPPORT
    conf->garp = FALSE;
#endif
    conf->force_wme_ac = 0;
...
#endif
    conf->orphan_move = 0;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(0x4, 1, 0))
    conf->tsq = 0xA;
#else
    conf->tsq = 0;
#endif
...
#endif
    conf->pktprio8021x = -1;
    conf->ctrl_resched = 0x2;
    conf->rxcnt_timeout = 0x3;
    ...
    conf->scan_busy_thresh = 0xA;
    conf->scan_busy_tmo = 0x78;
    conf->scan_tput_thresh = 0x5;
    if (conf->scan_tput_thresh > 0) {
        conf->tput_monitor_ms = 0x3E8;
    }
#endif
#ifdef DHD_TPUT_PATCH
    conf->tput_patch = FALSE;
...
#endif
#if defined(SDIO_ISR_THREAD)
    if (conf->chip == BCM43012_CHIP_ID || conf->chip == BCM4335_CHIP_ID ||
        conf->chip == BCM4339_CHIP_ID || conf->chip == BCM43454_CHIP_ID ||
        conf->chip == BCM4345_CHIP_ID || conf->chip == BCM4354_CHIP_ID ||
        conf->chip == BCM4356_CHIP_ID || conf->chip == BCM4345_CHIP_ID ||
        conf->chip == BCM4371_CHIP_ID || conf->chip == BCM4359_CHIP_ID ||
...
#endif
    if ((conf->chip == BCM43430_CHIP_ID && conf->chiprev == 0x2) ||
        conf->chip == BCM43012_CHIP_ID || conf->chip == BCM4335_CHIP_ID ||
        conf->chip == BCM4339_CHIP_ID || conf->chip == BCM43454_CHIP_ID ||
        conf->chip == BCM4345_CHIP_ID || conf->chip == BCM4354_CHIP_ID ||
        conf->chip == BCM4356_CHIP_ID || conf->chip == BCM4345_CHIP_ID ||
        conf->chip == BCM4371_CHIP_ID || conf->chip == BCM43569_CHIP_ID ||
        conf->chip == BCM4359_CHIP_ID || conf->chip == BCM43751_CHIP_ID ||
        conf->chip == BCM43752_CHIP_ID || conf->chip == BCM4375_CHIP_ID) {
#ifdef DHDTCPACK_SUPPRESS
#ifdef BCMSDIO
    ...
    if (conf->chip == BCM43362_CHIP_ID || conf->chip == BCM4330_CHIP_ID) {
        conf->txglom_bucket_size = 0x690; // fixed value, don't change
        conf->txglomsize = 0x6;
    }
    if (conf->chip == BCM4334_CHIP_ID || conf->chip == BCM43340_CHIP_ID ||
        conf->chip == BCM43341_CHIP_ID || conf->chip == BCM4324_CHIP_ID) {
        conf->txglom_bucket_size = 0x694; // fixed value, don't change
        conf->txglomsize = 0x10;
    }
#endif
    if (conf->txglomsize > SDPCM_MAXGLOM_SIZE) {
        conf->txglomsize = SDPCM_MAXGLOM_SIZE;
