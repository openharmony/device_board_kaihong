...
extern sdioh_info_t *sdioh_attach(osl_t *osh, struct sdio_func *func)
{
...
#endif
    sd->func[0x2] = func->card->sdio_func[1];
#ifdef GLOBAL_SDMMC_INSTANCE
    ...
    sd->client_block_size[0] = 0x40;
    sd->use_rxchain = CUSTOM_RXCHAIN;
    if (sd->func[1] == NULL || sd->func[0x2] == NULL) {
    ...
    sdio_claim_host(sd->func[0x2]);
    if ((func->device == BCM43362_CHIP_ID || func->device == BCM4330_CHIP_ID) &&
        sd_f2_blocksize > 0x80) {
        sd_f2_blocksize = 0x80;
    }
    sd->client_block_size[0x2] = sd_f2_blocksize;
    printf("%s: set sd_f2_blocksize %d\n", __FUNCTION__, sd_f2_blocksize);
    err_ret = sdio_set_block_size(sd->func[0x2], sd_f2_blocksize);
    sdio_release_host(sd->func[0x2]);
    if (err_ret) {
...
extern SDIOH_API_RC sdioh_enable_func_intr(sdioh_info_t *sd)
{
    ...
    if (err) {
        sd_err(
            ("%s: error for write SDIO_CCCR_IENx : 0x%x\n", __FUNCTION__, err));
        return SDIOH_API_RC_FAIL;
    }
                                             ...
                                             void *argh)
{
    ...
    if (fn == NULL) {
        sd_err(
            ("%s: interrupt handler is NULL, not registering\n", __FUNCTION__));
        return SDIOH_API_RC_FAIL;
    }
#if !defined(OOB_INTR_ONLY)
    sd->intr_handler = fn;
    ...
    if (sd->func[0x2]) {
        sdio_claim_host(sd->func[0x2]);
        sdio_claim_irq(sd->func[0x2], IRQHandlerF2);
        sdio_release_host(sd->func[0x2]);
    }
    ...
    switch (actionid) {
        ...
        case IOV_SVAL(IOV_DIVISOR):
            ...
            if (int_val && !(int_val & (int_val - 1))) {
                ...
                DHD_ERROR(
                    ("%s: Invalid sd_divisor value, should be power of 2!\n",
                     __FUNCTION__));
            }
            break;
                                       ...
                                       uint regaddr, uint8 *byte)
{
                    ...
                    if (sd->func[0x3]) {
                        ...
                        if (*byte & SDIO_FUNC_ENABLE_3) {
                            ...
                            sd_info(("bcmsdh_sdmmc F3: enable F3 fn %p\n",
                                     sd->func[0x3]));
                            err_ret = sdio_enable_func(sd->func[0x3]);
                            if (err_ret) {
                                sd_err(("bcmsdh_sdmmc: enable F3 err:%d\n",
                                        err_ret));
                            }

                            sdio_release_host(sd->func[0x3]);
                            ...
                            sd_info(("bcmsdh_sdmmc F3: disable F3 fn %p\n",
                                     sd->func[0x3]));
                            err_ret = sdio_disable_func(sd->func[0x3]);
                            if (err_ret) {
    ...
    if (sunxi_mmc_check_r1_ready(sd->func[func]->card->host, 0x3E8) != 0) {
    ...
    if (err_ret) {
        if ((regaddr == 0x1001F) &&
            ((err_ret == -ETIMEDOUT) || (err_ret == -EILSEQ) ||
             (err_ret == -EIO))) {
        } else {
            sd_err(
                ("bcmsdh_sdmmc: Failed to %s byte F%d:@0x%05x=%02x, Err: %d\n",
                                       ...
                                       uint nbytes)
{
    ...
    if (err_ret) {
#if defined(MMC_SDIO_ABORT)
        /* Any error on CMD53 transaction should abort that function using
         * function 0. */
        while (sdio_abort_retry--) {
            if (sd->func[0]) {
                sdio_claim_host(sd->func[0]);
...
#ifdef CUSTOMER_HW_ALLWINNER
                // AW judge sdio read write timeout, 1s
                if (sunxi_mmc_check_r1_ready(sd->func[func]->card->host,
                                             0x3E8) != 0) {
...
#endif /* MMC_SDIO_ABORT */
        {
            sd_err(("bcmsdh_sdmmc: Failed to %s word F%d:@0x%05x=%02x, Err: "
                    "0x%08x\n",
    ...
    if (sd_msglevel & SDH_COST_VAL) {
        ...
        sd_cost(("%s: rw=%d, len=%d cost = %3dms %3dus\n", __FUNCTION__, rw,
                 nbytes, diff_us / 0x3E8, diff_us % 0x3E8));
    }
                                               ...
                                               void *pkt)
{
    ...
    if (sd->txglom_mode == SDPCM_TXGLOM_MDESC) {
        while (pnext != NULL) {
             ...
             * the max data we can transfer with one command 53. blocks per
             * command is limited by host max_req_size and 9-bit max block
             * number. when the total length of this packet chain is bigger than
             * max_req_size, use multiple SD_IO_RW_EXTENDED commands (each
             * transfer is still block aligned)
             */
            while (pnext != NULL && ttl_len < max_req_size) {
                 ...
                 * NOT something we can handle here, but in case it happens,
                 * PLEASE put a restriction on max tx/glom count (based on
                 * host->max_segs).
                 */
                if (sg_count >= ARRAYSIZE(sd->sg_list)) {
                    sd_err(("%s: sg list entries(%u) exceed limit(%zu),"
                            ...
                            (size_t)ARRAYSIZE(sd->sg_list), blk_size));
                    return (SDIOH_API_RC_FAIL);
                }
                pdata += pkt_offset;
            ...
            mmc_cmd.opcode = 0x35; /* SD_IO_RW_EXTENDED */
            mmc_cmd.arg = write ? 1 << 0x1F : 0;
            mmc_cmd.arg |= (func & 0x7) << 0x1C;
            mmc_cmd.arg |= 1 << 0x1B;
            mmc_cmd.arg |= fifo ? 0 : 1 << 0x1A;
            mmc_cmd.arg |= (addr & 0x1FFFF) << 0x9;
        ...
        txglomfail:
            /* Align Patch */
            if (!write || pkt_len < 0x20) {
                pkt_len = (pkt_len + 0x3) & 0xFFFFFFFC;
            ...
            if ((write) && (!fifo)) {
                ...
                err_ret =
                    sdio_memcpy_fromio(sd->func[func], buf, addr, pkt_len);
            }

            if (err_ret) {
                sd_err(
                    ("%s: %s FAILED %p[%d], addr=0x%05x, pkt_len=%d, ERR=%d\n",
                     __FUNCTION__, (write) ? "TX" : "RX", pnext, sg_count, addr,
                     pkt_len, err_ret));
            } else {
                sd_trace(("%s: %s xfr'd %p[%d], addr=0x%05x, len=%d\n",
                          __FUNCTION__, (write) ? "TX" : "RX", pnext, sg_count,
                          addr, pkt_len));
            }
...
#endif
    {
        ...
        sd_cost(("%s: rw=%d, ttl_len=%4d cost = %3dms %3dus\n", __FUNCTION__,
                 write, ttl_len, diff_us / 0x3E8, diff_us % 0x3E8));
#ifdef PKT_STATICS
        if (write && (func == 0x2)) {
                                            ...
                                            uint8 *buf, uint len)
{
    ...
    if (err_ret) {
        sd_err(("%s: %s FAILED %p, addr=0x%05x, pkt_len=%d, ERR=%d\n",
                __FUNCTION__, (write) ? "TX" : "RX", buf, addr, len, err_ret));
    } else {
    ...
    if (sd_msglevel & SDH_COST_VAL) {
        ...
        sd_cost(("%s: rw=%d, len=%4d cost = %3dms %3dus\n", __FUNCTION__, write,
                 len, diff_us / 0x3E8, diff_us % 0x3E8));
    }
...
extern SDIOH_API_RC sdioh_request_buffer(sdioh_info_t *sd, uint pio_dma,
                                         ...
                                         uint buf_len, uint8 *buffer, void *pkt)
{
    SDIOH_API_RC status;
    void *tmppkt;
    struct osl_timespec now, before;
    ...
    if (pkt) {
        ...
        if (PKTNEXT(sd->osh, pkt)) {
            return sdioh_request_packet_chain(sd, fix_inc, write, func, addr,
                                              pkt);
        }
#endif /* BCMSDIOH_TXGLOM */
    ...
    /* buffer and length are aligned, use it directly so we can avoid memory
     * copy */
    if (((ulong)buffer & DMA_ALIGN_MASK) == 0 &&
        (buf_len & DMA_ALIGN_MASK) == 0) {
    ...
    if (sd_msglevel & SDH_COST_VAL) {
        ...
        diff_us = osl_do_gettimediff(&now, &before);
        sd_cost(("%s: rw=%d, len=%d cost = %3dms %3dus\n", __FUNCTION__, write,
                 buf_len, diff_us / 0x3E8, diff_us % 0x3E8));
    }
                             ...
                             int regsize, uint32 *data)
{
    if ((func == 0) || (regsize == 1)) {
        ...
        *data &= 0xff;
        ...
        if (sdioh_request_word(sd, 0, SDIOH_READ, func, regaddr, data,
                               regsize)) {
    ...
    */
    if (sd->func[0]) {
        if (stage == 0) {
            ...
            */
            /* sdio_reset_comm() - has been fixed in latest kernel/msm.git for
               Linux 2.6.27. The implementation prior to that is buggy, and
               needs broadcom's patch for it
            */
#if defined(ENABLE_INSMOD_NO_FW_LOAD) && !defined(BUS_POWER_RESTORE)
...
#endif
            {
                ...
                if (sd->func[1]) {
                    ...
                    sd->client_block_size[1] = 0x40;
                    ret = sdio_set_block_size(sd->func[1], 0x40);
                    if (ret) {
                ...
                if (sd->func[0x2]) {
                    ...
                    printf("%s: set sd_f2_blocksize %d\n", __FUNCTION__,
                           sd_f2_blocksize);
                    ret = sdio_set_block_size(sd->func[0x2], sd_f2_blocksize);
                    if (ret) {
