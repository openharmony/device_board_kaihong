                              ...
                              nan_event_data_t *tlv_data)
{
    ...
    if (tlv_data->sde_control_flag & NAN_SDE_CF_SVC_UPD_IND_PRESENT) {
        ...
        tlv_data->sde_svc_info.dlen = (p_attr[1] | (p_attr[0x2] << 0x8));
        WL_TRACE(
            ("> sdea svc info len: 0x%02x\n", tlv_data->sde_svc_info.dlen));
        ...
        if (tlv_data->sde_svc_info.dlen > 0) {
            tlv_data->sde_svc_info.data =
                MALLOCZ(osh, tlv_data->sde_svc_info.dlen);
            ...
            p_attr += offset;
            len -= offset;
            ret = memcpy_s(tlv_data->sde_svc_info.data,
                           tlv_data->sde_svc_info.dlen, p_attr,
                           tlv_data->sde_svc_info.dlen);
            if (ret != BCME_OK) {
                             ...
                             nan_event_data_t *tlv_data)
{
    ...
    if (svc_control & NAN_SC_MATCHING_FILTER_PRESENT) {
        ...
        ret = memcpy_s(tlv_data->tx_match_filter.data,
                       tlv_data->tx_match_filter.dlen, p_attr,
                       tlv_data->tx_match_filter.dlen);
        if (ret != BCME_OK) {
    ...
    if (svc_control & NAN_SC_SR_FILTER_PRESENT) {
        ...
        if (!tlv_data->rx_match_filter.dlen ||
            ...
            WL_ERR(
                ("%s: sr matching filter length is invalid\n", __FUNCTION__));
            ret = BCME_BADLEN;
            goto fail;
        }
        ...
        ret = memcpy_s(tlv_data->rx_match_filter.data,
                       tlv_data->rx_match_filter.dlen, p_attr,
                       tlv_data->rx_match_filter.dlen);
        if (ret != BCME_OK) {
                                        ...
                                        nan_event_data_t *tlv_data, uint16 type)
{
    ...
    if (type == WL_NAN_XTLV_SD_DISC_RESULTS) {
        ...
        /*
         * SDF followed by nan2_pub_act_frame_t and
         * wifi_nan_svc_descriptor_attr_t, and svc controls are optional.
         */
        ...
        WL_TRACE(("nan oui: %2x-%2x-%2x\n", nan_pub_af->oui[0],
                  nan_pub_af->oui[1], nan_pub_af->oui[0x2]));
...
int wl_cfgnan_set_vars_cbfn(void *ctx, const uint8 *data, uint16 type,
                            uint16 len)
{
    ...
    switch (type) {
        ...
        case WL_NAN_XTLV_SD_DISC_RESULTS: {
            ret = wl_cfgnan_parse_sd_attr_data(ctx_tlv_data->cfg->osh, len,
                                               data, tlv_data, type);
            break;
        }
                             ...
                             uint16 *subcmd_len)
{
    ...
    if (subcmd_len != NULL) {
        *subcmd_len =
            OFFSETOF(bcm_iov_batch_subcmd_t, data) + ALIGN_SIZE(data_size, 0x4);
                               ...
                               bool disable_events)
{
    ...
    (void)memset_s(event_mask, WL_NAN_EVMASK_EXTN_VER, 0,
                   WL_NAN_EVMASK_EXTN_VER);
    evmask_cmd_len =
        OFFSETOF(wl_nan_evmask_extn_t, evmask) + WL_NAN_EVMASK_EXTN_LEN;
    ...
    if (disable_events) {
        ...
        if (unlikely(ret) || unlikely(status)) {
            WL_ERR(
                ("get nan event mask failed ret %d status %d \n", ret, status));
            goto fail;
        }
                                   ...
                                   uint8 avail_type)
{
    ...
    if (status == BCME_NOTFOUND) {
        ...
        if (cmd_data->duration != NAN_BAND_INVALID) {
            entry->flags |=
                (0x3 << WL_AVAIL_ENTRY_USAGE_SHIFT) |
        ...
        if (avail_type == WL_AVAIL_LOCAL) {
            ...
            if (cfg->support_5g) {
                entry->u.channel_info = htod32(wf_channel2chspec(
                    WL_AVAIL_CHANNEL_5G, WL_AVAIL_BANDWIDTH_5G));
            } else {
                entry->u.channel_info = htod32(wf_channel2chspec(
                    WL_AVAIL_CHANNEL_2G, WL_AVAIL_BANDWIDTH_2G));
            }
        ...
        if (avail_type == WL_AVAIL_NDC) {
            ret =
                memcpy_s(&avail->addr, ETHER_ADDR_LEN, ndc_id, ETHER_ADDR_LEN);
            if (ret != BCME_OK) {
...
static int wl_cfgnan_config_control_flag(struct net_device *ndev,
                                         struct bcm_cfg80211 *cfg, uint32 flag,
                                         uint32 *status, bool set)
{
    ...
    NAN_DBG_ENTER();
    WL_INFORM_MEM(
        ("%s: Modifying nan ctrl flag %x val %d", __FUNCTION__, flag, set));
    nan_buf = MALLOCZ(cfg->osh, nan_buf_size);
    if (!nan_buf) {
        WL_ERR(("%s: memory allocation failed\n", __func__));
        ret = BCME_NOMEM;
    ...
    ret = wl_cfg_nan_check_cmd_len(nan_iov_data->nan_iov_len, sizeof(cfg_ctrl),
                                   &subcmd_len);
    if (unlikely(ret)) {
        WL_ERR(("nan_sub_cmd check failed\n"));
        goto fail;
    ...
    ret =
        memcpy_s(sub_cmd->data, sizeof(cfg_ctrl), &cfg_ctrl, sizeof(cfg_ctrl));
    if (ret != BCME_OK) {
                                     ...
                                     struct ether_addr *if_addr)
{
    ...
    if (p_buf != NULL) {
        ...
        ret = wl_cfg_nan_check_cmd_len(*nan_buf_size, sizeof(*if_addr),
                                       &subcmd_len);
        if (unlikely(ret)) {
            WL_ERR(("nan_sub_cmd check failed\n"));
            goto fail;
...
static int wl_cfgnan_get_ver(struct net_device *ndev, struct bcm_cfg80211 *cfg)
{
    bcm_iov_batch_buf_t *nan_buf = NULL;
    s32 ret = BCME_OK;
    uint16 nan_buf_size = NAN_IOCTL_BUF_SIZE;
...
static int wl_cfgnan_set_if_addr(struct bcm_cfg80211 *cfg)
{
    ...
    nan_buf_size -= OFFSETOF(bcm_iov_batch_buf_t, cmds[0]);
    if (rand_mac) {
        RANDOM_BYTES(if_addr.octet, 0x6);
         ...
         * NAN NMI I/F
         */
        if (wl_get_vif_macaddr(cfg, WL_IF_TYPE_NAN_NMI, if_addr.octet) !=
            BCME_OK) {
            ret = -EINVAL;
    ...
    ret = memcpy_s(cfg->nan_nmi_mac, ETH_ALEN, if_addr.octet, ETH_ALEN);
    if (ret != BCME_OK) {
...
static int wl_cfgnan_init_handler(void *p_buf, uint16 *nan_buf_size, bool val)
{
    ...
    if (p_buf != NULL) {
        ...
        sub_cmd->u.options = htol32(BCM_XTLV_OPTION_ALIGN32);
        ret = memcpy_s(sub_cmd->data, sizeof(uint8), (uint8 *)&val,
                       sizeof(uint8));
        if (ret != BCME_OK) {
                                         ...
                                         uint32 nan_attr_mask)
{
    ...
    if ((!cmd_data->metrics.master_pref) ||
        ...
        WL_TRACE(("Master Pref is 0 or greater than 254, hence sending random "
                  "value\n"));
        /* Master pref for mobile devices can be from 1 - 127 as per Spec
         * AppendixC */
        metrics->master_pref =
            (RANDOM32() % (NAN_MAXIMUM_MASTER_PREFERENCE / 0x2)) + 1;
    } else {
                                        ...
                                        uint32 nan_attr_mask)
{
    ...
    sub_cmd->len =
        htod16(sizeof(sub_cmd->u.options) + sizeof(*rssi_notif_thld));
...
static int check_for_valid_5gchan(struct net_device *ndev, uint8 chan)
{
    s32 ret = BCME_OK;
    uint bitmap;
                                       ...
                                       uint32 nan_attr_mask)
{
    ...
    if (cmd_data->support_5g) {
        if (nan_attr_mask & NAN_ATTR_5G_CHAN_CONFIG) {
            soc_chans->soc_chan_5g = cmd_data->chanspec[0x2];
        } else {
...
static int wl_cfgnan_set_nan_scan_params(struct net_device *ndev,
                                         struct bcm_cfg80211 *cfg,
                                         nan_config_cmd_data_t *cmd_data,
                                         uint8 band_index, uint32 nan_attr_mask)
{
    bcm_iov_batch_buf_t *nan_buf = NULL;
    s32 ret = BCME_OK;
    uint16 nan_iov_start, nan_iov_end;
                                    ...
                                    wl_nan_iov_t *nan_iov_data)
{
    ...
    cmd_data->clus_id.octet[0x2] = 0x9A;
    cmd_data->clus_id.octet[0x3] = 0x01;
    WL_TRACE(
        ...
        if (nan_attr_mask & NAN_ATTR_5G_DW_CONFIG) {
            ...
            if (!awake_dws->dw_interval_5g) {
                ...
                ret = wl_cfgnan_config_control_flag(
                    ndev, cfg, WL_NAN_CTRL_DISC_BEACON_TX_5G,
                    &(cmd_data->status), 0);
                ...
                ret = wl_cfgnan_config_control_flag(
                    ndev, cfg, WL_NAN_CTRL_SYNC_BEACON_TX_5G,
                    &(cmd_data->status), 0);
                if (unlikely(ret) || unlikely(cmd_data->status)) {
                    WL_ERR((" nan control set config handler,"
                            " ret = %d status = %d \n",
                            ...
                            uint32 nan_attr_mask)
{
    ...
    for (i = 0; i < NAN_MAX_NDI; i++) {
        ...
        if (cfg->nancfg.ndi[i].in_use && !cfg->nancfg.ndi[i].created) {
            ret = wl_cfgnan_data_path_iface_create_delete_handler(
                ndev, cfg, cfg->nancfg.ndi[i].ifname,
    ...
    ret = wl_cfgnan_set_nan_soc_chans(ndev, cmd_data, nan_iov_data,
                                      nan_attr_mask);
    if (unlikely(ret)) {
    ...
    if ((cmd_data->support_2g) &&
        ...
        /* setting scan params */
        ret = wl_cfgnan_set_nan_scan_params(ndev, cfg, cmd_data, 0,
                                            nan_attr_mask);
        if (unlikely(ret)) {
    ...
    if ((cmd_data->support_5g) &&
        ...
        /* setting scan params */
        ret = wl_cfgnan_set_nan_scan_params(
            ndev, cfg, cmd_data, cmd_data->support_5g, nan_attr_mask);
        if (unlikely(ret)) {
    ...
    if (!ETHER_ISNULLADDR(&cmd_data->clus_id.octet)) {
        if (cmd_data->clus_id.octet[0x4] == cmd_data->clus_id.octet[0x5]) {
            ...
            if (unlikely(ret) || unlikely(cmd_data->status)) {
                WL_ERR(
                    (" nan control set config handler, ret = %d status = %d \n",
                     ret, cmd_data->status));
                goto fail;
            }
        }
    ...
    ret =
        wl_cfgnan_set_rssi_mid_or_close(cmd_data, nan_iov_data, nan_attr_mask);
    if (unlikely(ret)) {
    ...
    if ((nan_attr_mask & NAN_ATTR_SID_BEACON_CONFIG) ||
        ...
        ret =
            wl_cfgnan_set_sid_beacon_val(cmd_data, nan_iov_data, nan_attr_mask);
        if (unlikely(ret)) {
    ...
    ret = wl_cfgnan_set_awake_dws(ndev, cmd_data, nan_iov_data, cfg,
                                  nan_attr_mask);
    if (unlikely(ret)) {
    ...
    if (unlikely(ret)) {
        WL_ERR(
            ("Failed to config disc ind flag in event_mask, ret = %d\n", ret));
        goto fail;
    }
    ...
    if ((ret = wl_cfgnan_get_capablities_handler(ndev, cfg, &capabilities)) ==
        ...
        cfg->nancfg.nan_ndp_peer_info = MALLOCZ(
            cfg->osh, cfg->nancfg.max_ndp_count * sizeof(nan_ndp_peer_t));
...
int wl_cfgnan_disable(struct bcm_cfg80211 *cfg)
{
    s32 ret = BCME_OK;
    dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);

    NAN_DBG_ENTER();
    if ((cfg->nan_init_state == TRUE) && (cfg->nan_enable == TRUE)) {
...
fail:
    ...
    cfg->nancfg.inst_id_start = 0;
    memset(cfg->nancfg.svc_inst_id_mask, 0,
           sizeof(cfg->nancfg.svc_inst_id_mask));
                             ...
                             uint32 nan_attr_mask)
{
    ...
    if (cfg->nan_enable) {
        ...
        /* setting rssi proximaty values for 2.4GHz and 5GHz */
        ret =
            wl_cfgnan_set_rssi_proximity(cmd_data, nan_iov_data, nan_attr_mask);
        if (unlikely(ret)) {
            WL_ERR(("2.4GHz/5GHz rssi proximity threshold set failed\n"));
            goto fail;
        } else {
        ...
        if ((!cmd_data->bmap) ||
            ...
            ret =
                wl_cfgnan_set_nan_avail(bcmcfg_to_prmry_ndev(cfg), cfg,
                                    ...
                                    uint16 cmd_id, bool update)
{
    ...
    if (cmd_id == WL_NAN_CMD_SD_SUBSCRIBE) {
        ...
        if ((cmd_data->flags & WL_NAN_SUB_ACTIVE) &&
            ...
            ret = memcpy_s(svc_info->tx_match_filter,
                           sizeof(svc_info->tx_match_filter),
    ...
    for (i = 0; i < NAN_MAX_RANGING_INST; i++) {
        ...
        if (ranging_inst->range_id &&
            ...
            if (ranging_inst->range_status == NAN_RANGING_IN_PROGRESS) {
                ret = wl_cfgnan_cancel_ranging(
                    ndev, cfg, ranging_inst->range_id,
 ...
 */
static void
wl_cfgnan_terminate_all_obsolete_ranging_sessions(struct bcm_cfg80211 *cfg)
{
                                         ...
                                         struct ether_addr *peer_addr)
{
    ...
    if (ranging_inst->range_status != NAN_RANGING_IN_PROGRESS) {
        WL_DBG(("Trigger range request with first svc in svc list of range "
                "inst\n"));
        ...
        if (ret != BCME_OK) {
            ...
            if (ret == BCME_BUSY) {
                ...
                geofence_target = dhd_rtt_get_geofence_target(
                    dhd, &ranging_inst->peer_addr, &index);
                if (geofence_target) {
                                        ...
                                        nan_event_data_t *nan_event_data)
{
    ...
    if (svc && svc->ranging_required) {
        nan_ranging_inst_t *ranging_inst;
        ranging_inst = wl_cfgnan_get_ranging_inst(
            cfg, &nan_event_data->remote_nmi, NAN_RANGING_ROLE_INITIATOR);
        if (!ranging_inst) {
            ret = BCME_NORESOURCE;
            goto exit;
        }
                                      ...
                                      struct bcm_cfg80211 *cfg)
{
    ...
    for (i = 0; i < NAN_MAX_RANGING_INST; i++) {
        ...
        if (ranging_inst->range_status == NAN_RANGING_IN_PROGRESS &&
            ...
            ret = wl_cfgnan_cancel_ranging(
                bcmcfg_to_prmry_ndev(cfg), cfg, ranging_inst->range_id,
                NAN_RNG_TERM_FLAG_IMMEDIATE, &status);
            if (unlikely(ret) || unlikely(status)) {
                                        ...
                                        wl_nan_ev_rng_req_ind_t *rng_ind)
{
    ...
    if (ranging_inst) {
        if (ranging_inst->range_type == RTT_TYPE_NAN_GEOFENCE ||
            ranging_inst->range_role == NAN_RANGING_ROLE_RESPONDER) {
            WL_INFORM_MEM(
                ("Already responder/geofence for the Peer, cancel current"
            ...
            if (ranging_inst->range_type == RTT_TYPE_NAN_GEOFENCE &&
                ...
                wl_cfgnan_suspend_geofence_rng_session(
                    ndev, &(rng_ind->peer_m_addr),
                    RTT_GEO_SUSPN_PEER_RTT_TRIGGER, cancel_flags);
            } else {
                ret = wl_cfgnan_cancel_ranging(
                    ndev, cfg, ranging_inst->range_id, cancel_flags, &status);
                if (unlikely(ret)) {
                              ...
                              uint8 range_cmd, bool accept_req)
{
    ...
    memset_s(&cmd_data, sizeof(cmd_data), 0, sizeof(cmd_data));
    ...
    if (range_cmd == NAN_RANGE_REQ_CMD) {
        ret =
            process_resp_buf(resp_buf + WL_NAN_OBUF_DATA_OFFSET,
                                       ...
                                       uint16 *nan_buf_size)
{
    ...
    if ((cmd_data->svc_hash.dlen == WL_NAN_SVC_HASH_LEN) &&
        ...
        ret =
            memcpy_s((uint8 *)sd_params->svc_hash, sizeof(sd_params->svc_hash),
    ...
    *nan_buf_size -= sub_cmd->len;
    ...
    if (cmd_data->svc_info.data && cmd_data->svc_info.dlen) {
        ...
        if (unlikely(ret)) {
            WL_ERR(
                ("%s: fail to pack WL_NAN_XTLV_SD_SVC_INFO\n", __FUNCTION__));
            goto fail;
        }
    }
    ...
    if (cmd_data->tx_match.dlen) {
        WL_TRACE(("optional tx match filter presnet (len=%d)\n",
                  cmd_data->tx_match.dlen));
        ret = bcm_pack_xtlv_entry(
            &pxtlv, nan_buf_size, WL_NAN_XTLV_CFG_MATCH_TX,
    ...
    if (cmd_data->life_count) {
        ...
        ret = bcm_pack_xtlv_entry(
            &pxtlv, nan_buf_size, WL_NAN_XTLV_CFG_SVC_LIFE_COUNT,
        ...
        if (!ETHER_ISNULLADDR(&cmd_data->mac_list.list) &&
            ...
            if (cmd_data->srf_type == SRF_TYPE_SEQ_MAC_ADDR) {
                ...
                ret = bcm_pack_xtlv_entry(&pxtlv, nan_buf_size,
                                          WL_NAN_XTLV_CFG_SR_FILTER, srf_size,
                                          srf_mac, BCM_XTLV_OPTION_ALIGN32);
                if (unlikely(ret)) {
                ...
                if (sd_params->instance_id == NAN_ID_RESERVED) {
                    WL_ERR(
                        ("Invalid instance id: %d\n", sd_params->instance_id));
                    ret = BCME_BADARG;
                    goto fail;
                }
                if (bloom_idx == 0xFFFFFFFF) {
                    bloom_idx = sd_params->instance_id % 0x4;
                } else {
                    WL_ERR(("Invalid bloom_idx\n"));
                    ret = BCME_BADARG;
                    goto fail;
                }
                srf_control |= bloom_idx << 0x2;
                ...
                ret = bcm_pack_xtlv_entry(
                    &pxtlv, nan_buf_size, WL_NAN_XTLV_CFG_SR_FILTER,
    ...
    if (cmd_data->rx_match.dlen) {
        ...
        ret = bcm_pack_xtlv_entry(
            &pxtlv, nan_buf_size, WL_NAN_XTLV_CFG_MATCH_RX,
    ...
    if (cmd_data->csid) {
        ...
        ret = bcm_pack_xtlv_entry(
            &pxtlv, nan_buf_size, WL_NAN_XTLV_CFG_SEC_CSID,
    ...
    if (cmd_data->ndp_cfg.security_cfg) {
        if ((cmd_data->key_type == NAN_SECURITY_KEY_INPUT_PMK) ||
            ...
            if (cmd_data->key.data && cmd_data->key.dlen) {
                ...
                ret = bcm_pack_xtlv_entry(
                    &pxtlv, nan_buf_size, WL_NAN_XTLV_CFG_SEC_PMK,
    ...
    if (cmd_data->scid.data && cmd_data->scid.dlen) {
        ...
        ret = bcm_pack_xtlv_entry(&pxtlv, nan_buf_size,
                                  WL_NAN_XTLV_CFG_SEC_SCID, cmd_data->scid.dlen,
                                  cmd_data->scid.data, BCM_XTLV_OPTION_ALIGN32);
        if (unlikely(ret)) {
            WL_ERR(
                ("%s: fail to pack WL_NAN_XTLV_CFG_SEC_SCID\n", __FUNCTION__));
            goto fail;
        }
    }
    ...
    uint16 *data_size, nan_discover_cmd_data_t *cmd_data)
{
    ...
    if (cmd_data->svc_info.dlen) {
        *data_size +=
            ALIGN_SIZE(cmd_data->svc_info.dlen + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->sde_svc_info.dlen) {
        *data_size +=
            ALIGN_SIZE(cmd_data->sde_svc_info.dlen + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->tx_match.dlen) {
        *data_size +=
            ALIGN_SIZE(cmd_data->tx_match.dlen + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->rx_match.dlen) {
        *data_size +=
            ALIGN_SIZE(cmd_data->rx_match.dlen + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->use_srf) {
        if (cmd_data->srf_type == SRF_TYPE_SEQ_MAC_ADDR) {
        ...
        *data_size += ALIGN_SIZE(*data_size + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->csid) {
        *data_size +=
            ALIGN_SIZE(sizeof(nan_sec_csid_e) + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->key.dlen) {
        *data_size += ALIGN_SIZE(cmd_data->key.dlen + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->scid.dlen) {
        *data_size += ALIGN_SIZE(cmd_data->scid.dlen + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->sde_control_config) {
        *data_size += ALIGN_SIZE(sizeof(uint16) + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    if (cmd_data->life_count) {
        *data_size +=
            ALIGN_SIZE(sizeof(cmd_data->life_count) + NAN_XTLV_ID_LEN_SIZE, 0x4);
    }
    return ret;
}

static int
wl_cfgnan_aligned_data_size_of_opt_dp_params(uint16 *data_size,
                                             nan_datapath_cmd_data_t *cmd_data)
{
    ...
    *data_size += ALIGN_SIZE(WL_NAN_SVC_HASH_LEN + NAN_XTLV_ID_LEN_SIZE, 0x4);
    return ret;
}
                              ...
                              nan_discover_cmd_data_t *cmd_data)
{
    ...
    WL_INFORM_MEM(
        ("[NAN] Service published for instance id:%d\n", cmd_data->pub_id));
                                ...
                                nan_discover_cmd_data_t *cmd_data)
{
    ...
    WL_INFORM_MEM(
        ("[NAN] Service subscribed for instance id:%d\n", cmd_data->sub_id));
                               ...
                               nan_discover_cmd_data_t *cmd_data)
{
    ...
    data_size = ALIGN_SIZE(data_size, 0x4);
    ret = wl_cfgnan_aligned_data_size_of_opt_disc_params(&data_size, cmd_data);
    if (unlikely(ret)) {
        WL_ERR(("Failed to get alligned size of optional params\n"));
        goto fail;
    ...
    if (cmd_data->svc_info.data && cmd_data->svc_info.dlen) {
        ...
        ret = bcm_pack_xtlv_entry(
            &pxtlv, &nan_buf_size, WL_NAN_XTLV_SD_SVC_INFO,
                                    ...
                                    nan_hal_capabilities_t *capabilities)
{
    ...
    capabilities->max_total_match_filter_len =
        fw_cap->max_total_match_filter_len;
                                      ...
                                      nan_hal_capabilities_t *capabilities)
{
    ...
    if (cfg->nan_init_state) {
        ...
        if (ret != BCME_OK) {
            WL_ERR(
                ("NAN init state: %d, failed to get capability from FW[%d]\n",
...
int wl_cfgnan_init(struct bcm_cfg80211 *cfg)
{
    s32 ret = BCME_OK;
    uint16 nan_buf_size = NAN_IOCTL_BUF_SIZE;
    uint32 status;
    ...
    cfg->nan_disc_cache = MALLOCZ(cfg->osh, NAN_MAX_CACHE_DISC_RESULT *
...
void wl_cfgnan_deinit_cleanup(struct bcm_cfg80211 *cfg)
{
    ...
    memset_s(cfg->svc_info, NAN_MAX_SVC_INST * sizeof(nan_svc_info_t), 0,
             NAN_MAX_SVC_INST * sizeof(nan_svc_info_t));
    memset_s(cfg->nan_ranging_info,
             NAN_MAX_RANGING_INST * sizeof(nan_ranging_inst_t), 0,
             NAN_MAX_RANGING_INST * sizeof(nan_ranging_inst_t));
#endif /* WL_NAN_DISC_CACHE */
    return;
}

int wl_cfgnan_deinit(struct bcm_cfg80211 *cfg, uint8 busstate)
{
    s32 ret = BCME_OK;
    uint16 nan_buf_size = NAN_IOCTL_BUF_SIZE;
    uint32 status;
                                                    ...
                                                    uint8 busstate)
{
    ...
    if (busstate != DHD_BUS_DOWN) {
        if (type == NAN_WIFI_SUBCMD_DATA_PATH_IFACE_CREATE) {
            ...
            wdev =
                wl_cfg80211_add_if(cfg, ndev, WL_IF_TYPE_NAN, ifname, mac_addr);
            if (!wdev) {
                ret = -ENODEV;
            ...
            if (ret == BCME_OK) {
                if (wl_cfgnan_del_ndi_data(cfg, ifname) < 0) {
                    WL_ERR(
                        ("Failed to find matching data for ndi:%s\n", ifname));
                }
            } else if (ret == -ENODEV) {
        ...
        ret = -ENODEV;
        WL_ERR(
            ("Bus is already down, no dev found to remove, ret = %d\n", ret));
    }
fail:
    NAN_DBG_EXIT();
    return ret;
 ...
 * NULL if not found
 */
nan_ndp_peer_t *wl_cfgnan_data_get_peer(struct bcm_cfg80211 *cfg,
                                        struct ether_addr *peer_addr)
{
    uint8 i;
...
bool wl_cfgnan_data_dp_exists(struct bcm_cfg80211 *cfg)
{
    ...
    if ((cfg->nan_init_state == FALSE) || (cfg->nan_enable == FALSE)) {
        goto exit;
    }
                                        ...
                                        uint8 *ndp_instance_id)
{
    ...
    if (cmd_data->ndp_cfg.security_cfg) {
        ...
        if ((cmd_data->svc_hash.dlen == WL_NAN_SVC_HASH_LEN) &&
            ...
            ret = bcm_pack_xtlv_entry(
                &pxtlv, &nan_buf_size, WL_NAN_XTLV_CFG_SVC_HASH,
            ...
            cache = wl_cfgnan_get_disc_result(cfg, datareq->pub_id,
                                              &datareq->peer_mac);
            if (!cache) {
                ret = BCME_ERROR;
                WL_ERR(("invalid svc hash data or length = %d\n",
            ...
            ret = bcm_pack_xtlv_entry(
                &pxtlv, &nan_buf_size, WL_NAN_XTLV_CFG_SVC_HASH,
                WL_NAN_SVC_HASH_LEN, cache->svc_hash, BCM_XTLV_OPTION_ALIGN32);
            if (ret != BCME_OK) {
                WL_ERR(("%s: fail to pack WL_NAN_XTLV_CFG_SVC_HASH\n",
                        __FUNCTION__));
                goto fail;
                                         ...
                                         nan_datapath_cmd_data_t *cmd_data)
{
    ...
    if (!ETHER_ISNULLADDR(&cfg->initiator_ndi.octet)) {
        ret = memcpy_s(&dataresp->mac_addr, ETHER_ADDR_LEN, &cfg->initiator_ndi,
                       ETHER_ADDR_LEN);
        if (ret != BCME_OK) {
    ...
    if (dataresp->status != WL_NAN_DP_STATUS_REJECTED) {
        /* Retrieve mac from given iface name */
        wdev =
            wl_cfg80211_get_wdev_from_ifname(cfg, (char *)cmd_data->ndp_iface);
        if (!wdev || ETHER_ISNULLADDR(wdev->netdev->dev_addr)) {
            ret = -EINVAL;
            WL_ERR(("Failed to retrieve wdev/dev addr for ndp_iface = %s\n",
                    (char *)cmd_data->ndp_iface));
    ...
    if (unlikely(ret) || unlikely(cmd_data->status)) {
        WL_ERR(
            ("nan data path response handler failed, error = %d, status %d\n",
             ret, cmd_data->status));
        goto fail;
    }
                               ...
                               nan_hal_resp_t *nan_req_resp)
{
    ...
    if (cmd_data->pub_id && !ETHER_ISNULLADDR(&cmd_data->mac_addr)) {
        disc_cache = wl_cfgnan_get_disc_result(cfg, cmd_data->pub_id,
                                               &cmd_data->mac_addr);
    ...
    if (cmd_data->ndp_instance_id) {
        ...
        /* Always send NMI */
        (void)memcpy_s(nan_req_resp->pub_nmi, ETHER_ADDR_LEN, cfg->nan_nmi_mac,
                       ETHER_ADDR_LEN);
        ret = BCME_OK;
    }
fail:
                                      ...
                                      osl_t *osh)
{
    ...
    if (cache->svc_info.dlen && cache->svc_info.data) {
        ...
        ret = memcpy_s(nan_event_data->svc_info.data,
                       nan_event_data->svc_info.dlen, cache->svc_info.data,
                       cache->svc_info.dlen);
        if (ret != BCME_OK) {
    ...
    if (cache->tx_match_filter.dlen && cache->tx_match_filter.data) {
        ...
        ret =
            memcpy_s(nan_event_data->tx_match_filter.data,
                                        ...
                                        struct ether_addr *peer, int reason)
{
    ...
    if (rng_inst) {
        if (rng_inst->range_type == RTT_TYPE_NAN_GEOFENCE) {
            err = wl_cfgnan_suspend_geofence_rng_session(ndev, peer, reason, 0);
        } else {
            if (rng_inst->range_type == RTT_TYPE_NAN_DIRECTED) {
                dhd_rtt_handle_nan_rtt_session_end(dhdp, peer);
            }
            /* responder */
            err =
                wl_cfgnan_cancel_ranging(ndev, cfg, rng_inst->range_id,
                                         NAN_RNG_TERM_FLAG_IMMEDIATE, &status);
        ...
        if (event_num == WL_NAN_EVENT_PEER_DATAPATH_IND) {
            ...
            *hal_event_id = GOOGLE_NAN_EVENT_DATA_END;
            ...
            if (ev_dp->role != NAN_DP_ROLE_INITIATOR) {
                 ...
                 */
                svc_info =
                    wl_cfgnan_get_svc_inst(cfg, 0, nan_event_data->ndp_id);
                if (svc_info) {
                    for (i = 0; i < NAN_MAX_SVC_INST; i++) {
                                         ...
                                         uint32 prev_distance)
{
    ...
    if (svc_ind ==
        (NAN_RANGE_INDICATION_INGRESS | NAN_RANGE_INDICATION_EGRESS)) {
        ...
        if (svc_info) {
            if (nan_event_data->ranging_result_present) {
                notify_svc = wl_cfgnan_check_ranging_cond(
                    svc_info, distance, &ranging_ind,
                    rng_inst->prev_distance_mm);
...
#ifdef RTT_SUPPORT
static int32 wl_cfgnan_handle_directed_rtt_report(struct bcm_cfg80211 *cfg,
                                                  nan_ranging_inst_t *rng_inst,
                                                  uint8 rng_id)
{
    int ret = BCME_OK;
    ...
    ret = wl_cfgnan_cancel_ranging(bcmcfg_to_prmry_ndev(cfg), cfg, rng_id,
                                   NAN_RNG_TERM_FLAG_IMMEDIATE, &status);
    if (unlikely(ret) || unlikely(status)) {
        WL_ERR(("%s:nan range cancel failed ret = %d status = %d\n",
                __FUNCTION__, ret, status));
                                      ...
                                      int sched_reason)
{
    ...
    WL_INFORM_MEM(
        ("wl_cfgnan_reset_geofence_ranging, sched_reason = %d, cur_idx = %d, "
         "geofence_interval = %d\n",
    ...
    if ((sched_reason == RTT_SCHED_RTT_RETRY_GEOFENCE) &&
        ...
        rng_inst = wl_cfgnan_get_ranging_inst(cfg, &geofence_target->peer_addr,
     ...
     * NDP with Peer
     */
    if ((geofence_state == TRUE) || (!RTT_IS_STOPPED(rtt_status)) ||
...
static bool wl_check_range_role_concurrency(dhd_pub_t *dhd,
                                            nan_ranging_inst_t *rng_inst)
{
    ASSERT(rng_inst);
                                ...
                                const wl_event_msg_t *event, void *event_data)
{
    ...
    if (!cfg->nan_init_state) {
        WL_ERR(
            ("nan is not in initialized state, dropping nan related events\n"));
        ret = BCME_OK;
        goto exit;
    }
    ...
    switch (event_num) {
        ...
        case WL_NAN_EVENT_TERMINATED: {
            ...
            if (pev->reason != NAN_TERM_REASON_USER_REQ) {
                wl_cfgnan_clear_svc_from_all_ranging_inst(cfg,
                                                          pev->instance_id);
        ...
        case WL_NAN_EVENT_TXS: {
            ...
            if (txs->type == WL_NAN_FRM_TYPE_FOLLOWUP) {
                ...
                if (txs->opt_tlvs_len && xtlv->id == WL_NAN_XTLV_SD_TXS) {
                    ...
                    WL_ERR(
                        ("Invalid params in TX status for trasnmit followup"));
                    ret = -EINVAL;
                    goto exit;
                }
        ...
        case WL_NAN_EVENT_DISC_CACHE_TIMEOUT: {
            ...
            if (xtlv->id == WL_NAN_XTLV_SD_DISC_CACHE_TIMEOUT) {
                xtlv_len = xtlv_len - OFFSETOF(wl_nan_ev_disc_cache_timeout_t,
                                               cache_exp_list);
                while ((entry_idx < cache_data->count) &&
                    ...
                    wl_cfgnan_ranging_clear_publish(
                        cfg, &cache_entry->r_nmi_addr, cache_entry->l_sub_id);
                    /* Invalidate local cache info */
        ...
        case WL_NAN_EVENT_RNG_TERM_IND: {
            ...
            if (rng_inst) {
                ...
                if (rng_inst->range_type == RTT_TYPE_NAN_DIRECTED) {
                    ...
                    if (wl_cfgnan_geofence_retry_check(rng_inst, range_term->reason_code)) {
                        ...
                        if (geofence_target) {
                            dhd_rtt_remove_geofence_target(
                                dhd, &geofence_target->peer_addr);
                        }
                    }
                    /* Set geofence RTT in progress state to false */
                    dhd_rtt_set_geofence_rtt_state(dhd, FALSE);
        ...
        case WL_NAN_EVENT_DATAPATH_END: {
            ret = wl_nan_dp_cmn_event_data(
                cfg, event_data, data_len, &tlvs_offset, &nan_opts_len,
        ...
        default:
            WL_ERR_RLMT(
                ("WARNING: unimplemented NAN APP EVENT = %d\n", event_num));
            ret = BCME_ERROR;
            goto exit;
    }
    ...
    if (nan_opts_len) {
        ...
        ret = bcm_unpack_xtlv_buf((void *)&nan_event_ctx,
                                  (const uint8 *)tlv_buf, nan_opts_len,
                                  xtlv_opt, wl_cfgnan_set_vars_cbfn);
        if (ret != BCME_OK) {
    ...
    if (hal_event_id == GOOGLE_NAN_EVENT_SUBSCRIBE_MATCH) {
        ...
        if (nan_event_data->sde_control_flag & NAN_SDE_CF_RANGING_REQUIRED) {
            ...
            if (svc_info && svc_info->ranging_required &&
                ...
                wl_cfgnan_ranging_clear_publish(
                    cfg, &nan_event_data->remote_nmi, nan_event_data->sub_id);
            }
        }
    ...
    *disc_cache_update_flags = 0;
    ...
    for (i = 0; i < NAN_MAX_CACHE_DISC_RESULT; i++) {
        ...
        if (!memcmp(&disc_res[i].peer, &disc->remote_nmi, ETHER_ADDR_LEN) &&
            !memcmp(disc_res[i].svc_hash, disc->svc_name,
                    WL_NAN_SVC_HASH_LEN)) {
    ...
    ret = memcpy_s(&disc_res[add_index].peer, ETHER_ADDR_LEN, &disc->remote_nmi,
                   ETHER_ADDR_LEN);
    if (ret != BCME_OK) {
        WL_ERR(("Failed to copy remote nmi\n"));
        goto done;
...
void wl_cfgnan_update_dp_info(struct bcm_cfg80211 *cfg, bool add,
...
s32 wl_cfgnan_get_ndi_idx(struct bcm_cfg80211 *cfg)
{
    int i;
    for (i = 0; i < NAN_MAX_NDI; i++) {
...
s32 wl_cfgnan_add_ndi_data(struct bcm_cfg80211 *cfg, s32 idx, char *name)
{
    u16 len;
...
s32 wl_cfgnan_del_ndi_data(struct bcm_cfg80211 *cfg, char *name)
{
    u16 len;
    int i;
    if (!name) {
    ...
    for (i = 0; i < NAN_MAX_NDI; i++) {
        if (strncmp(cfg->nancfg.ndi[i].ifname, name, len) == 0) {
            memset_s(&cfg->nancfg.ndi[i].ifname, IFNAMSIZ, 0x0, IFNAMSIZ);
...
struct wl_ndi_data *wl_cfgnan_get_ndi_data(struct bcm_cfg80211 *cfg, char *name)
{
    u16 len;
    int i;
    if (!name) {
...
s32 wl_cfgnan_delete_ndp(struct bcm_cfg80211 *cfg, struct net_device *nan_ndev)
{
    ...
    for (i = 0; i < NAN_MAX_NDI; i++) {
        if (cfg->nancfg.ndi[i].in_use && cfg->nancfg.ndi[i].created &&
             ...
             */
            if (wl_cfgnan_del_ndi_data(cfg, (char *)cfg->nancfg.ndi[i].ifname) <
                0) {
