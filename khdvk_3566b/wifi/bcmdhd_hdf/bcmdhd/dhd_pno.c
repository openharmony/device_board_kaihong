...
int dhd_pno_clean(dhd_pub_t *dhd)
{
    ...
    if (err < 0) {
        DHD_ERROR(
            ("%s : failed to execute pfn(error : %d)\n", __FUNCTION__, err));
        goto exit;
    }
...
static uint64 convert_fw_rel_time_to_systime(struct osl_timespec *ts,
                                             uint32 fw_ts_ms)
{
    return ((uint64)(TIMESPEC_TO_US(*ts)) - (uint64)(fw_ts_ms * 0x3E8));
}
                              ...
                              wl_pfn_gscan_cfg_t *pfncfg_gscan_param, int size)
{
    ...
    if (err < 0) {
        DHD_ERROR(
            ("%s : failed to execute pfncfg_gscan_param\n", __FUNCTION__));
        goto exit;
    }
exit:
    return err;
...
static int _dhd_pno_suspend(dhd_pub_t *dhd)
{
    ...
    if (err < 0) {
        DHD_ERROR(
            ("%s : failed to suspend pfn(error :%d)\n", __FUNCTION__, err));
        goto exit;
    }
                        ...
                        dhd_pno_mode_t mode)
{
    ...
    if (mode == DHD_PNO_LEGACY_MODE) {
        ...
        if ((pno_params->params_legacy.pno_repeat != 0) ||
            ...
            pfn_param.exp =
                (uchar)(pno_params->params_legacy.pno_freq_expo_max);
        }
        /* set up pno scan fr */
    ...
    if (mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
        ...
        if (mode == DHD_PNO_BATCH_MODE) {
            ...
            if (pno_params->params_hotlist.scan_fr) {
                pfn_param.scan_freq =
                    htod32(pno_params->params_hotlist.scan_fr);
            }
            pfn_param.bestn = 0;
            pfn_param.repeat = 0;
            /* enable broadcast scan */
        ...
        if (combined_scan) {
            ...
            if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
                ...
                _params =
                    &(_pno_state->pno_params_arr[INDEX_OF_HOTLIST_PARAMS]);
                if (_params->params_hotlist.scan_fr) {
                    pfn_param.scan_freq =
                        htod32(_params->params_hotlist.scan_fr);
                }
                pfn_param.bestn = 0;
                pfn_param.repeat = 0;
            }
    ...
    if (mode & DHD_PNO_GSCAN_MODE) {
        ...
        if (_pno_state->pno_mode & DHD_PNO_LEGACY_MODE) {
            ...
            if ((params->params_legacy.pno_repeat != 0) ||
                ...
                pfn_param.exp =
                    (uchar)(params->params_legacy.pno_freq_expo_max);
            }
        }
...
#endif /* GSCAN_SUPPORT */
    {
        ...
        err = dhd_iovar(dhd, 0, "pfnmem", (char *)&_tmp, sizeof(_tmp), NULL, 0,
                        TRUE);
        if (err < 0) {
                             ...
                             int nssid)
{
    ...
    err = dhd_iovar(dhd, 0, "pfn_add", (char *)pfn_elem_buf, mem_needed, NULL,
                    0, TRUE);
    if (err < 0) {
        DHD_ERROR(("%s : failed to execute pfn_add\n", __FUNCTION__));
    }
                                   ...
                                   char *buf, int nbufsize)
{
    ...
    DHD_PNO(("%s scancount %d\n", __FUNCTION__,
             params_batch->get_batch.expired_tot_scan_cnt));
    /* preestimate scan count until which scan result this report is going to
     * end */
    GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
        ...
        while (phead != NULL) {
            ...
            list_for_each_entry_safe(iter, next, &phead->entry_list, list)
            {
                ...
                bp += nreadsize = snprintf(
                    ...
                    bcm_ether_ntoa((const struct ether_addr *)&iter->BSSID,
                                   eabuf));
                nleftsize -= nreadsize;
                /* SSID */
                bp += nreadsize =
                    snprintf(bp, nleftsize, "ssid=%s\n", iter->SSID);
                nleftsize -= nreadsize;
                /* channel */
                bp += nreadsize =
                                      ...
                                      dhd_pno_mode_t mode)
{
    ...
    switch (mode) {
        ...
        case DHD_PNO_BATCH_MODE: {
            ...
            INIT_LIST_HEAD(
                &params->params_batch.get_batch.expired_scan_results_list);
            break;
        }
        case DHD_PNO_HOTLIST_MODE: {
            ...
            if (params->params_hotlist.nbssid > 0) {
                GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
                list_for_each_entry_safe(
                    iter, next, &params->params_hotlist.bssid_list, list)
                {
                    GCC_DIAGNOSTIC_POP();
                    list_del(&iter->list);
                    MFREE(dhd->osh, iter, sizeof(struct dhd_pno_ssid));
    ...
    if (_pno_state->pno_mode & (DHD_PNO_BATCH_MODE | DHD_PNO_HOTLIST_MODE)) {
        ...
        if (_pno_state->pno_mode & DHD_PNO_BATCH_MODE) {
            ...
            if (_params2->params_hotlist.nchan > 0 && tot_nchan > 0) {
                err =
                    _dhd_pno_chan_merge(_chan_list, &tot_nchan,
                          ...
                          struct dhd_pno_batch_params *batch_params)
{
    ...
    if (!WLS_SUPPORTED(_pno_state)) {
        DHD_ERROR(
            ("%s : wifi location service is not supported\n", __FUNCTION__));
    ...
    if (batch_params->band == WLC_BAND_2G ||
        ...
        err = _dhd_pno_get_channels(
            dhd, &_params->params_batch.chan_list[batch_params->nchan],
        ...
        sort(_params->params_batch.chan_list, _params->params_batch.nchan,
             sizeof(_params->params_batch.chan_list[0]), _dhd_pno_cmpfunc,
             NULL);
    }
#ifdef PNO_DEBUG
    {
                               ...
                               bool set)
{
    ...
    if (err != BCME_OK) {
        DHD_ERROR(
            ("%s : Failed to execute pfn_ssid_cfg %d\n", __FUNCTION__, err));
    }
    return err;
}
...
int dhd_pno_set_epno(dhd_pub_t *dhd)
{
    ...
    if (gscan_params->epno_cfg.num_epno_ssid) {
        ...
        if ((err =
                 _dhd_pno_add_ssid(dhd, &gscan_params->epno_cfg.epno_ssid_list,
                                    ...
                                    uint8 flags)
{
    ...
    if (flags & GSCAN_FLUSH_EPNO_CFG) {
        ...
        if (epno_cfg->num_epno_ssid > 0) {
            GCC_DIAGNOSTIC_PUSH_SUPPRESS_CAST();
            list_for_each_entry_safe(iter, next, &epno_cfg->epno_ssid_list,
                                     list)
            {
                GCC_DIAGNOSTIC_POP();
                list_del(&iter->list);
...
int dhd_wait_batch_results_complete(dhd_pub_t *dhd)
{
    ...
    if (_params->params_gscan.get_batch_flag ==
        ...
        if ((_pno_state->pno_mode & DHD_PNO_GSCAN_MODE) && !num_results) {
            DHD_PNO(
                ("%s: No results cached, getting from FW..\n", __FUNCTION__));
            ...
            if (ptr->nchannel_buckets <= GSCAN_MAX_CH_BUCKETS) {
                ...
                for (i = 0; i < ptr->nchannel_buckets; i++) {
                    ...
                    for (k = 0; k < ptr->channel_bucket[i].num_channels; k++) {
                        ch_bucket[i].chan_list[k] = wf_mhz2channel(
                            ptr->channel_bucket[i].chan_list[k], 0);
                    }
                    ch_bucket[i].band = 0;
    ...
    for (i = 0; i < gscan_params->nchannel_buckets; i++) {
        if (!gscan_params->channel_bucket[i].band) {
            for (k = 0; k < gscan_params->channel_bucket[i].num_channels; k++) {
                if (gscan_params->channel_bucket[i].chan_list[k] >
                    CHANNEL_5G_MAX) {
                                 ...
                                 struct dhd_pno_gscan_params *gscan_params)
{
    ...
    if (gscan_params->nbssid_hotlist) {
        ...
        err =
            _dhd_pno_add_bssid(dhd, p_pfn_bssid, gscan_params->nbssid_hotlist);
        if (err < 0) {
    ...
    *num_buckets_to_fw = 0;
    ...
    for (i = 0; i < _params->params_gscan.nchannel_buckets && nchan; i++) {
        if (!gscan_buckets[i].band) {
            ...
            /* get a valid channel list based on band B or A */
            err = _dhd_pno_get_channels(
                dhd, ptr, &nchan, (gscan_buckets[i].band & GSCAN_ABG_BAND_MASK),
    ...
    /* Legacy PNO maybe running, which means we need to create a legacy PNO
     * bucket Get GCF of Legacy PNO and Gscan scanfreq
     */
    if (is_pno_legacy_running) {
...
static int _dhd_pno_get_gscan_batch_from_fw(dhd_pub_t *dhd)
{
     ...
     * net_info and subnet_info structures are compatible in size and SSID
     * offset, allowing v1 to be safely used in the code below except for
     * lscanresults fields themselves (status, count, offset to netinfo).
     */
    ...
    timediff = gscan_params->scan_fr * 0x3E8;
    timediff = timediff >> 1;
        ...
        if (plbestnet_v1->version == PFN_LBEST_SCAN_RESULT_VERSION_V1) {
            ...
            for (i = 0; i < num_scans_in_cur_iter; i++) {
                iter = (gscan_results_cache_t *)MALLOCZ(
                    dhd->osh,
                    ((nAPs_per_scan[i] - 1) * sizeof(wifi_gscan_result_t)) +
                        sizeof(gscan_results_cache_t));
                if (!iter) {
                    DHD_ERROR(("%s :Out of memory!! Cant malloc %d bytes\n",
                ...
                for (j = 0; j < nAPs_per_scan[i]; j++, plnetinfo++) {
                    ...
                    result->ts = convert_fw_rel_time_to_systime(
                        &tm_spec, plnetinfo->timestamp);
            ...
            for (i = 0, count = 0; i < fwcount; i++, count++, plnetinfo_v2++) {
                ...
                if (TIME_DIFF_MS(timestamp, plnetinfo_v2->timestamp) >
                    timediff) {
                    nAPs_per_scan[num_scans_in_cur_iter] = count;
                    count = 0;
                    num_scans_in_cur_iter++;
            ...
            for (i = 0; i < num_scans_in_cur_iter; i++) {
                ...
                for (j = 0; j < nAPs_per_scan[i]; j++, plnetinfo_v2++) {
                    ...
                    result->channel = wf_channel2mhz(
                        ...
                        (plnetinfo_v2->pfnsubnet.channel <= CH_MAX_2G_CHANNEL
                             ? WF_CHAN_FACTOR_2_4_G
                             : WF_CHAN_FACTOR_5_G));
                    ...
                    result->ts = convert_fw_rel_time_to_systime(
                        &tm_spec, plnetinfo_v2->timestamp);
...
static void *dhd_get_gscan_batch_results(dhd_pub_t *dhd, uint32 *len)
{
    ...
    *len = ((num_results << 0x10) | (num_scan_ids));
    return results;
}
                        ...
                        void *info, uint32 *len)
{
    ...
    switch (type) {
        case DHD_PNO_GET_CAPABILITIES:
            ...
            ptr->max_scan_reporting_threshold = 0x64;
                                  ...
                                  int reason)
{
     ...
     * structures are compatible in size and SSID offset, allowing v1 to be
     * safely used in the code below except for lscanresults fields themselves
    ...
    if (buf && bufsize) {
        if (!list_empty(
            ...
            DHD_PNO(
                ("%s: have cashed batching data in Driver\n", __FUNCTION__));
    ...
    /* create dhd_pno_scan_results_t whenever we got event
    ...
    /* add the element into list unless total node cnt is less than MAX_NODE_
     * CNT */
    ...
    do {
        ...
        if (plbestnet_v1->version == PFN_LBEST_SCAN_RESULT_VERSION_V1) {
            ...
            for (i = 0; i < fwcount; i++) {
                pbestnet_entry = (dhd_pno_bestnet_entry_t *)MALLOC(
                ...
                timestamp = plnetinfo->timestamp;
                if (allocate_header) {
                    pbestnetheader = (dhd_pno_best_header_t *)MALLOC(
                        dhd->osh, BEST_HEADER_SIZE);
            ...
            for (i = 0; i < fwcount; i++) {
                ...
                /* check whether the new generation is started or not */
                if (timestamp &&
                    (TIME_DIFF(timestamp, plnetinfo_v2->timestamp) >
                     TIME_MIN_DIFF)) {
                    allocate_header = TRUE;
                }
                timestamp = plnetinfo_v2->timestamp;
    ...
    convert_format:
        err =
            _dhd_pno_convert_format(dhd, &_params->params_batch, buf, bufsize);
        if (err < 0) {
...
int dhd_pno_get_for_batch(dhd_pub_t *dhd, char *buf, int bufsize, int reason)
{
...
#endif // endif
    {
        if (!(_pno_state->pno_mode & DHD_PNO_BATCH_MODE)) {
            DHD_ERROR(
                ("%s: Batching SCAN mode is not enabled\n", __FUNCTION__));
                            ...
                            struct dhd_pno_hotlist_params *hotlist_params)
{
    ...
    if (hotlist_params->band == WLC_BAND_2G ||
        ...
        sort(_params->params_hotlist.chan_list, _params->params_hotlist.nchan,
             sizeof(_params->params_hotlist.chan_list[0]), _dhd_pno_cmpfunc,
             NULL);
    }
#ifdef PNO_DEBUG
    {
                                    ...
                                    uint32 len, int *size)
{
    ...
    *size = 0;
    ...
    result->fixed.channel = wf_channel2mhz(
        channel, (channel <= CH_MAX_2G_CHANNEL ? WF_CHAN_FACTOR_2_4_G
                                               : WF_CHAN_FACTOR_5_G));
    ...
    if (event == WLC_E_PFN_NET_FOUND || event == WLC_E_PFN_NET_LOST) {
        ...
        if (pfn_result->version == PFN_SCANRESULT_VERSION_V1) {
            ...
            for (i = 0; i < count; i++) {
                ...
                results[i].channel =
                    wf_channel2mhz(net->pfnsubnet.channel,
    ...
    if (results_v1->version == PFN_SCANRESULTS_VERSION_V1) {
        ...
        *buf_len = malloc_size;
        ...
        for (i = 0; i < fwcount; i++, pnetinfo++) {
            ...
            hotlist_found_array->ts = convert_fw_rel_time_to_systime(
                &tm_spec, (pnetinfo->timestamp * 0x3E8));
        ...
        *buf_len = malloc_size;
        ...
        for (i = 0; i < fwcount; i++, pnetinfo_v2++) {
            ...
            hotlist_found_array->ts = convert_fw_rel_time_to_systime(
                &tm_spec, (pnetinfo_v2->timestamp * 0x3E8));
