                     ...
                     cca_congest_channel_req_t *avg, bool percent)
{
    ...
    if (percent) {
        ...
        avg->secs[0].congest_ibss = totals.congest_ibss * 0x64 / totals.duration;
        avg->secs[0].congest_obss = totals.congest_obss * 0x64 / totals.duration;
        avg->secs[0].interference = totals.interference * 0x64 / totals.duration;
    } else {
        avg->secs[0].duration = totals.duration / avg->num_secs;
...
/*
    ...
    Don't forget that channels with low duration times may not have accurate
   readings. For the moment, do not overwrite input array.
*/
                ...
                chanspec_t *answer)
{
    ...
    if (flags & CCA_FLAGS_PREFER_1_6_11) {
        ...
        for (i = 0; i < num_chans; i++) {
            if (is2g && tmp_channel != 1 && tmp_channel != 0x6 &&
                tmp_channel != 0xB) {
                clrbit(bitmap, i);
            }
        }
...
#define IDX_IN_WL_CNT_VER_6_T(cntmember)                                       \
    ((OFFSETOF(wl_cnt_ver_6_t, cntmember) -                                    \
      OFFSETOF(wl_cnt_ver_6_t, txframe)) /                                     \
     sizeof(uint32))
                             ...
                             uint32 corerev)
{
    ...
    xtlv_desc[0x2].type = 0;
    xtlv_desc[0x2].len = 0;
    xtlv_desc[0x2].ptr = NULL;
