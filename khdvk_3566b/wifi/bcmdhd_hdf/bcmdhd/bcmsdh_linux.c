                   ...
                   uint bus_type, uint bus_num, uint slot_num)
{
    ...
    bcmsdh_osinfo->oob_irq_num = wifi_platform_get_irq_number(
        adapter_info, &bcmsdh_osinfo->oob_irq_flags);
    ...
    bcmsdh_osinfo->context =
        drvinfo.probe((vendevid >> 0x10), (vendevid & 0xFFFF), bus_num, slot_num,
                      0, bus_type, (void *)regs, osh, bcmsdh);
                             ...
                             void *oob_irq_handler_context)
{
    ...
    if (device_may_wakeup(bcmsdh_osinfo->dev)) {
        ...
        if (err) {
            SDLX_ERR(
                ("%s: enable_irq_wake failed with %d\n", __FUNCTION__, err));
        } else {
