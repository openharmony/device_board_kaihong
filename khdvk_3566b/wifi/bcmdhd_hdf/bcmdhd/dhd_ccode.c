...
#ifdef CCODE_43436
const char ccode_43436[] = "AE/1 AR/1 AT/1 AU/2 "
                           "BE/1 BG/1 BN/1 "
...
#ifdef CCODE_4375B4
const char ccode_4375b4[] = "AE/6 AL/2 AM/1 AN/5 AR/21 AT/4 AU/6 AZ/2 "
...
#ifdef CCODE_4358U
const char ccode_4358u[] = "BE/4 BR/4 "
...
int dhd_ccode_map_country_list(dhd_pub_t *dhd, wl_country_t *cspec)
{
    ...
    if (ccode_list) {
        ...
        if (pch) {
            cspec->rev =
                (int)simple_strtol(pch + strlen(cspec->ccode) + 1, NULL, 0);
            bcmerror = 0;
        }
    }

    if (bcmerror && ccode_ww && strlen(ccode_ww) >= 0x4) {
        memcpy(cspec->ccode, ccode_ww, 0x2);
        cspec->rev = (int)simple_strtol(ccode_ww + 0x3, NULL, 0);
    }
