...
GetLinuxInfByNetDevice(const struct NetDevice *netDevice);
...
int BDH6InitNetdev(struct NetDevice *netDevice, int private_data_size, int type,
                   int ifidx);
...
int32_t WalRemainOnChannel(struct NetDevice *netDev, WifiOnChannel *onChannel)
{
    ...
    HDF_LOGE("%s: ifname=%s, freq=%u, duration=%u", __func__, hnetdev->name,
             onChannel->freq, onChannel->duration);
    ...
    ret = wl_cfg80211_ops.remain_on_channel(wiphy, cfgdev, channel, duration,
                                            &p2p_cookie);
    OsalMemFree(channel);
    return ret;
}
...
int32_t WalAddIf(struct NetDevice *netDev, WifiIfAdd *ifAdd)
{
    ...
    if (g_hdf_infmap[HDF_INF_P2P1].hnetdev != NULL) {
        HDF_LOGE("%s: ifidx=%d was used, failed add if", __func__,
                 HDF_INF_P2P1);
        return -1;
    }

    ret = BDH6InitNetdev(netDev, get_dhd_priv_data_size(), ifAdd->type,
                         HDF_INF_P2P1);
    if (ret != 0) {
    ...
    wdev = wl_cfg80211_ops.add_virtual_intf(wiphy, p2p_hnetdev->name,
                                            NET_NAME_USER, ifAdd->type, NULL);
    if (wdev == NULL || wdev == ERR_PTR(-ENODEV)) {
        HDF_LOGE("%s:create wdev for %s %d failed", __func__, p2p_hnetdev->name,
                 ifAdd->type);
        return HDF_FAILURE;
    }
    HDF_LOGE("%s:%s wdev->netdev=%p, %p", __func__, p2p_hnetdev->name,
             wdev->netdev, p2p_netdev);
    p2p_hnetdev->ieee80211Ptr = p2p_netdev->ieee80211_ptr;

    // update mac addr to NetDevice object
    memcpy_s(p2p_hnetdev->macAddr, MAC_ADDR_SIZE, p2p_netdev->dev_addr,
             p2p_netdev->addr_len);
    HDF_LOGE("%s: %s mac: %02x:%02x:%02x:%02x:%02x:%02x", __func__,
             p2p_hnetdev->name, p2p_hnetdev->macAddr[0],
             p2p_hnetdev->macAddr[1], p2p_hnetdev->macAddr[0x2],
             p2p_hnetdev->macAddr[0x3], p2p_hnetdev->macAddr[0x4],
             p2p_hnetdev->macAddr[0x5]);
...
int32_t WalRemoveIf(struct NetDevice *netDev, WifiIfRemove *ifRemove)
{
    ...
    HDF_LOGE("%s: ifname=%s, primary netdev %s, remove ifname=%s", __func__,
    ...
    for (; i < HDF_INF_MAX; i++) {
        ...
        if (strcmp(p2p_hnetdev->name, ifRemove->ifName) == 0) {
            // check safely
            if (i == HDF_INF_WLAN0) {
                HDF_LOGE("%s: don't remove master interface %s", __func__,
                         ifRemove->ifName);
                continue;
            }
            if (i != HDF_INF_P2P1) {
                HDF_LOGE("%s: remove %s is not p2p interface (%d %d)", __func__,
                         ifRemove->ifName, i, HDF_INF_P2P1);
            }

            wdev = (struct wireless_dev *)p2p_hnetdev->ieee80211Ptr;
            ret = (int32_t)wl_cfg80211_ops.change_virtual_intf(
                wiphy, g_hdf_infmap[i].netdev, NL80211_IFTYPE_STATION, NULL);
            HDF_LOGE("%s: change %s mode %d --> %d, ret=%d", __func__,
                     g_hdf_infmap[i].netdev->name, wdev->iftype,
                     NL80211_IFTYPE_STATION, ret);

            rtnl_lock();
            // clear private object
            DestroyEapolData(p2p_hnetdev);
            p2p_hnetdev->ieee80211Ptr = NULL;
            // This func free wdev object and call unregister_netdevice() and
            // NetDeviceDeInit()
...
int hdf_start_p2p_device(void)
{
    ...
    // create wdev object for p2p-dev-wlan0 device, refer
    ...
    wdev = wl_cfg80211_ops.add_virtual_intf(
        wiphy, "p2p-dev-wlan0", NET_NAME_USER, NL80211_IFTYPE_P2P_DEVICE, NULL);
    if (wdev == NULL) {
        HDF_LOGE("%s:create wdev for p2p-dev-wlan0 %d failed", __func__,
...
int32_t WalGetDriverFlag(struct NetDevice *netDev, WifiGetDrvFlags **params)
{
    ...
    switch (iftype) {
        ...
        case NL80211_IFTYPE_P2P_DEVICE:
            getDrvFlag->drvFlags =
                (unsigned int)(HISI_DRIVER_FLAGS_P2P_DEDICATED_INTERFACE |
    ...
    *params = getDrvFlag;
    ...
    HDF_LOGE("%s: %s iftype=%d, drvflag=%lu", __func__, netDev->name, iftype,
             getDrvFlag->drvFlags);
    return HDF_SUCCESS;
}
